...
extern int SendOpenModifyItem(smWINSOCK* lpsmSock);
		...
		if (lpChar->MotionInfo->State != CHRMOTION_STATE_DEAD && lpChar->MotionInfo->State != CHRMOTION_STATE_WARP) {
			...
			else
			{
				...
				if ((abs((long)lpPlayInfo->dwLastAttackTime - (long)dwTime) > 1500) || Type == 33)
				{
					...
					if (lpChar->smMonsterInfo.SocketSystemNPC)
						SendOpenSocketItem(lpsmSock);
					if (lpChar->smMonsterInfo.elementSocketNPC)
						SendOpenElementItem(lpsmSock);
					if (lpChar->smMonsterInfo.BattleTime)
					{
						cBattleArena.SendOpenWindow(lpPlayInfo);
					}
					...
					if (lpChar->smMonsterInfo.ModifyNpc)
					{
						SendOpenModifyItem(lpsmSock);
					}

			

					if (lpChar->smMonsterInfo.NPCCODE)
					{
						SendOpenNPCCODE(lpsmSock);
					}

					

					if (lpChar->smMonsterInfo.SorteioCoins)
					{
						Sorteio_->OpenNpcSorteio(lpPlayInfo);
					...
					if (lpChar->smMonsterInfo.ItemShopOld)
					{
						SendCoinShop(lpsmSock);
					}

					if (lpChar->smMonsterInfo.BattleEvent)
					{
						SendOpenBattleEvent(lpsmSock);
		...
		if (lpPlayInfo2->Position.Area == rsCASTLE_FIELD) {
			if ((rand() % 100) < (int)lpPlayInfo2->dwSiegeItem_Scroll_Param) {
				...
				if (lpPlayInfo2->AdminMode > 2) {
					wsprintf(szDispDamage, "> %s Esquiva Adicional ( Pergaminho de Evasão[%d] )", lpPlayInfo->smCharInfo.szName, lpPlayInfo2->dwSiegeItem_Scroll_Param);
...
#ifdef DISP_DAMAGE
				if (lpPlayInfo2->AdminMode > 2) {
					wsprintf(szDispDamage, "> %s Evasão Adicional ( Vague[%d] )", lpPlayInfo->smCharInfo.szName, Vague_EvasionPercent[lpPlayInfo2->dwSkill_Vague_Param]);
					rsDisplayDamgeToClient(lpPlayInfo2, szDispDamage);
				}
#endif
...
#ifdef DISP_DAMAGE
				if (lpPlayInfo2->AdminMode > 2) {
					wsprintf(szDispDamage, "> %s Evasão Adicional ( Vague Phoenix[%d] )", lpPlayInfo->smCharInfo.szName, Evasion_Mastery_AddPercent[lpPlayInfo2->dwSkill_EvasionMastery_Param]);
					rsDisplayDamgeToClient(lpPlayInfo2, szDispDamage);
				}
#endif
...
#ifdef DISP_DAMAGE
				if (lpPlayInfo2->AdminMode > 2) {
					wsprintf(szDispDamage, "> %s Evasão Adicional ( Hall of Valhalla [%d] )", lpPlayInfo->smCharInfo.szName, lpPlayInfo2->wSkill_HallOfValhalla_Param[0]);
					rsDisplayDamgeToClient(lpPlayInfo2, szDispDamage);
				}
#endif
				//Ãß°¡È¸ÇÇ Á¤º¸ Å¬¶ó¸®¾ðÆ®¿¡ Ãâ·Â
...
#ifdef DISP_DAMAGE
				if (lpPlayInfo2->AdminMode > 2) {
					wsprintf(szDispDamage, "> %s Evasão Adicional ( Summon Muspell [%d] )", lpPlayInfo->smCharInfo.szName, Summon_Muspell_BlockPercent[lpPlayInfo2->dwSkill_SummonMuspell_Param]);
					rsDisplayDamgeToClient(lpPlayInfo2, szDispDamage);
				}
#endif
				//Ãß°¡È¸ÇÇ Á¤º¸ Å¬¶ó¸®¾ðÆ®¿¡ Ãâ·Â
			...
			if (lpChar && lpChar->Flag && lpChar->smCharInfo.State && lpChar->MotionInfo->State != CHRMOTION_STATE_DEAD)
			{
				for (cnt = 0; cnt < lpTransSkilAttackData->TargetCount; cnt++)
				{
					if (lpChar->dwObjectSerial == lpTransSkilAttackData->dwTarObjectSerial[cnt] &&
						...
						!STRINGCOMPARE(lpChar->smCharInfo.szName, "Valento")) // Não usa no valento
					{
...
int rsSetSkillAttack(rsPLAYINFO* lpPlayInfo, smCHAR* lpChar, TRANS_ATTACKDATA* lpTransAttackData, TRANS_ATTACKDATA2* lpTransAttackData2)
{
	...
	case SKILL_PLAY_HOLY_INCANTATION:
		...
		if (lpChar && lpChar->smCharInfo.Brood == smCHAR_MONSTER_UNDEAD && lpChar->smCharInfo.wPlayClass[0] != MONSTER_CLASS_BOSS) // Ajuste não funcionar em boss
		{
			// Retirado resistêcnia do mob na chance
...
int rsRecvAttackData(rsPLAYINFO* lpPlayInfo, TRANS_ATTACKDATA2* lpTransAttackData)
{
	...
	*/
	...
	if (lpChar) {
		...
		if (lpPlayInfo2) 
		{
			// Evita ataque somente 1 modo pk ativo
			if (lpPlayInfo->PkMode && !lpPlayInfo2->PkMode)
				return FALSE;
...
int rsSetSkillRangeAttack(rsPLAYINFO* lpPlayInfo, smCHAR* lpChar, TRANS_ATTACKDATA* lpTransAttackData, TRANS_SKIL_ATTACKDATA* lpTransSkillAttackData, TRANS_SKIL_ATTACKDATA2* lpTransRangeAttack)
{
	...
	case SKILL_PLAY_SHIELD_STRIKE:
		...
		// Correção soul sucker funcionando
		...
		if (lpChar)
		{
			for (cnt = 0; cnt < lpTransRangeAttack->TargetCount; cnt++)
			{
				if (lpTransRangeAttack->dwTarObjectSerial[cnt])
				{
					...
					if (monster)
					{
						if (monster->smCharInfo.wPlayClass[0] != MONSTER_CLASS_BOSS) // Não funciona em boss
...
int	rsBillingItemSkill(rsPLAYINFO* lpPlayInfo, smCHAR* lpChar, int HitMonsters, int Power, int SkillCode)
{
	...
	if (lpPlayInfo->dwTime_PrimeItem_VampCuspid > 0) {
		fp1 = float(lpPlayInfo->smCharInfo.Strength + (((float)Power) / 100.0f)) / 200.0f;
		fp2 = (1.0f + (((float)HitMonsters) / 500.0f));
		fps = fp1 * fp2 * HitMonsters + 100.0f;
	...
	{
		fp1 = float(lpPlayInfo->smCharInfo.Strength + (((float)Power) / 100.0f)) / 150.0f;
		fp2 = (1.0f + (((float)HitMonsters) / 70.0f));
		fps = fp1 * fp2 * HitMonsters + 100.0f;

...
bool CLI::run(bool* done)
{
	...
	cout << "Servidor Carregado ;)" << endl;
	cout << "Tempo Real: " << ctime(&tt) << endl;
	cout << "Version: 1.0.0.0 - " << "ALucard" << "!" << endl;
	cout << "Alterado by Alucard " << endl << endl;
	...
	while (!(*this->done))
	{
		...
		if (input.back() != ';')
		{
			cout << "todos comandos devem terminar em;" << endl;
		}
		else
		{
			process(input);
...
bool CLI::process(const std::string& cmd)
{
	...
	if (cmd.compare("ajuda;") == 0)
		handleHelpCommand();
	else if (cmd.compare("sair;") == 0)
		handleExitCommand();
	else if (cmd.compare("desligar;") == 0)
		handleShutdownCommand();
	else if (cmd.compare("limpar;") == 0)
		system("cls");
	else if (cmd.compare("conectados;") == 0)
...
{
	rsShutDown();
	cout << "Servidor será desligado em 8 minutos" << endl;
}

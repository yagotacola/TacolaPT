...
public:
	...
	int GetPTDistanceXZY(const Point3D* psCompare, int& xzy, int& xz, int& x, int& z, int& y) const;

	BOOL WithinPTDistance(const Point3D* psCompare, int iDistance, BOOL bFlatY = FALSE) const;
	BOOL WithinXZDistance(const Point3D* psCompare, int iDistanceXZ, int iDistanceY = 300) const;
...
public:
	...
	D3DVECTOR4DF() : fX(0), fY(0), fZ(0), fW(0) {};

	D3DVECTOR4DF(float x, float y, float z, float w) : fX(x), fY(y), fZ(z), fW(w) {};

	float Dot(const D3DVECTOR4DF& c) const;
	...
	D3DVECTOR4DF operator-(const D3DVECTOR4DF& c) const;
	D3DVECTOR4DF operator+(const D3DVECTOR4DF& c) const;
	void operator=(const D3DVECTOR4DF& c);
	D3DVECTOR4DF operator*(const float c) const;

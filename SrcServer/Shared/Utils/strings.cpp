...
unsigned int ChatCommand(std::string pszCommand, std::string pszChat)
{
	auto vec = split(pszChat, ' ');
	pszChat = vec[0];

	if ((pszCommand.compare(pszChat)) == 0)
		return pszChat.length();
	else
		return FALSE;
}
...
std::string GetFileExtension(const std::string& strFilePath)
{
	...
	if (stDotIndex != std::string::npos)
	{
		...
		std::string fileExtension(szFileExtension);

		return rtrim(fileExtension);
	}
...
void tolower_(const char* pszIn, char* pszOut)
{
	int i = 0;
...
std::string tolower_(const std::string& s)
{
	std::string r;
	r.reserve(s.length());

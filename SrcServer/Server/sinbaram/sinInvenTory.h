...
public:
	...
	void*	lpPotionBack;
	void*	lpChangeABInfo[2];
	void*	lpExitInfo;
	void*	lpWeightInfo;
	void*	lpItemABInfo[2];

	void*	lpShowWeaponClass[4];

	void*	lpIndiShopIcon;


	void*   lpPuzzleInven;     //퍼즐 칸
	void*   lpPuzzlewidth;     //가로퍼즐 잡을때 마우스이미지 
	void*   lpPuzzleHeight;    //세로퍼즐 잡을때 마우스이미지 
...
public:
	...
	POINT CheckImageSize(void* lpdd); //그림의 사이즈를 구해서 리턴한다 

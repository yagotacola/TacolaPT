...
int SetServerConfig(char* decode, char* lpPoint)
{
	...
	{
		char szBuf[32] = { 0 };
		LeIniStr("Event", "AgingFree", "Server\\Config\\Server.ini", szBuf);
	...
	{
		char szBuf[32] = { 0 };
		LeIniStr("Event", "MixFree", "Server\\Config\\Server.ini", szBuf);
		if (lstrcmpi(szBuf, "True") == 0)
		{
			MixEvento = TRUE;
		}
...
int SetServerSelectList(char* decode, char* lpPoint)
{
		...
		lstrcmpi(decode, "*GAME_SERVER") == 0)
	{
		...
		if (strBuff[0])
		{
			lstrcpy(TransServerList.ServerInfo[cnt].szServerName, strBuff);			//¼­¹ö ÀÌ¸§
		}

		pb = p; p = GetWord(strBuff, p); if (strBuff[0] == 34)p = GetString(strBuff, pb);
		if (strBuff[0])
		{
			lstrcpy(TransServerList.ServerInfo[cnt].szServerIp1, strBuff);			//¼­¹ö ÀÌ¸§
		...
		if (strBuff[0])
		{
			lstrcpy(TransServerList.ServerInfo[cnt].szServerIp2, strBuff);			//¼­¹ö ÀÌ¸§ ( ¿¹ºñ )
		...
		if (strBuff[0])
		{
			lstrcpy(TransServerList.ServerInfo[cnt].szServerIp3, strBuff);			//¼­¹ö ÀÌ¸§ ( ¿¹ºñ )
...
smCONFIG* smConfigReg()
{
	...
	{
		#ifndef _W_SERVER
		#ifdef _LANGUAGE_CHINESE//(Áß±¹ ·¹Áö°ª ÀÓ½Ã·Î µî·Ï(Å×½ºÆ®))
				SetRegString( HKEY_LOCAL_MACHINE, InfoRegPath , "FullMode" , "off");
		#endif
		#endif
	}
	*/
...
//Á¤º¸ ÆÄÀÏÀ» ÇØµ¶ÇÏ¿© µ¹·ÁÁØ´Ù (Info¿¡ÀúÀå)
smCONFIG* smConfigDecode(char* iniFile)
{
	...
	Config->TextureQuality = 0;		//±âº» 1
	Config->NetworkQuality = 2;		//±âº» 2
	Config->BGM_Mode = 1;
	Config->WeatherSwitch = 0;	// pluto ÇØ¿Ü ºñ
	...
	//return smConfigReg();					//·¹Áö½ºÆ®¸® ¼³Á¤ Àû¿ë

	rsServerConfig.Enable_PKField = 0;
	...
	while (!feof(fp))//  feof: file end±îÁö ÀÐ¾î¶ó 
	{
		...
		if (lpMonInfo && !NextFlag) {
			...
			if (!NextFlag && lstrcmp(decode, "*°æÇèÄ¡") == 0) {
				p = GetWord(strBuff, p);
				lpMonInfo->GetExp = std::stoll(strBuff);
			...
			if (!NextFlag && lstrcmp(decode, "*¾ÆÀÌÅÛ¸ðµÎ") == 0) {	//¶¥¿¡ ¶³¾îÁú¶§ ¸ðµÎ¿¡°Ô ¾ÆÀÌÅÛ º¸ÀÓ
			...
			if (!NextFlag && lstrcmp(decode, "*BattleTime") == 0) {
				lpMonInfo->BattleTime = true;
				NextFlag = TRUE;
			}

			if (!NextFlag && lstrcmp(decode, "*ModifyItem") == 0)
			{
				lpMonInfo->ModifyNpc = TRUE;
				NextFlag = TRUE;
			}

			if (!NextFlag && lstrcmp(decode, "*BattlePass") == 0)
			{
				lpMonInfo->BattlePass = TRUE;
				NextFlag = TRUE;
			}

		

			if (!NextFlag && lstrcmp(decode, "*NPCCODE") == 0)
			{
				lpMonInfo->NPCCODE = TRUE;
				NextFlag = TRUE;
			}

			if (!NextFlag && lstrcmp(decode, "*SorteioCoins") == 0)
			{
				lpMonInfo->SorteioCoins = TRUE;
				NextFlag = TRUE;
			}
			...
			if (!NextFlag && lstrcmp(decode, "*ShopCoin") == 0)
			{
				lpMonInfo->ItemShop = TRUE;
				NextFlag = TRUE;
			}

			if (!NextFlag && lstrcmp(decode, "*ShopBP") == 0)
			{
				lpMonInfo->BPShop = TRUE;
				NextFlag = TRUE;
			}
...
// Atributos padrões dos personagens
...
// Status item perfeito
sITEMINFO* GetPerfItem(sITEMINFO* lpItem, sDEF_ITEMINFO* lpDefItem, int SpJobCode, int ItemNormam)
{
	int is, ie;
	int cnt;
	...
	if (lpDefItem->sDurability[0]) {
		...
		if (lpDefItem->sDurability[1])
			lpItem->Durability[1] = GetRandomPos_Normal(lpDefItem->sDurability[1], lpDefItem->sDurability[1]);
		else
			lpItem->Durability[1] = lpDefItem->sDurability[0];

		ie = lpItem->Durability[1];
		is = ie / 2;
		lpItem->Durability[0] = GetRandomPos_Normal(ie, ie);
	}

	for (cnt = 0; cnt < 8; cnt++) {
		if (lpDefItem->sResistance[cnt][1]) {
			lpItem->Resistance[cnt] =
				GetRandomPos_Normal(lpDefItem->sResistance[cnt][1], lpDefItem->sResistance[cnt][1]);
		}
		else
	...
	if (lpDefItem->sDamage[1] || lpDefItem->sDamage[3]) {
		lpItem->Damage[0] = GetRandomPos_Normal(lpDefItem->sDamage[1], lpDefItem->sDamage[1]);
		lpItem->Damage[1] = GetRandomPos_Normal(lpDefItem->sDamage[3], lpDefItem->sDamage[3]);
	}

	if (lpDefItem->sAttack_Rating[1])
		lpItem->Attack_Rating = GetRandomPos_Normal(lpDefItem->sAttack_Rating[1], lpDefItem->sAttack_Rating[1]);
	else
		lpItem->Attack_Rating = lpDefItem->sAttack_Rating[0];

	if (lpDefItem->fAbsorb[1] != 0)
		lpItem->fAbsorb = GetRandomFloatPos2(lpDefItem->fAbsorb[1], lpDefItem->fAbsorb[1]);
	else
		lpItem->fAbsorb = lpDefItem->fAbsorb[0];

	if (lpDefItem->sDefence[1])
		lpItem->Defence = GetRandomPos_Normal(lpDefItem->sDefence[1], lpDefItem->sDefence[1]);
	else
		lpItem->Defence = lpDefItem->sDefence[0];

	if (lpDefItem->fBlock_Rating[1] != 0)
		lpItem->fBlock_Rating = GetRandomFloatPos(lpDefItem->fBlock_Rating[1], lpDefItem->fBlock_Rating[1]);
	else
		lpItem->fBlock_Rating = lpDefItem->fBlock_Rating[0];

	if (lpDefItem->fSpeed[1] != 0)
		lpItem->fSpeed = GetRandomFloatPos2(lpDefItem->fSpeed[1], lpDefItem->fSpeed[1]);
	else
		lpItem->fSpeed = lpDefItem->fSpeed[0];

	if (lpDefItem->fMana_Regen[1] != 0)
		lpItem->fMana_Regen = GetRandomFloatPos2(lpDefItem->fMana_Regen[1], lpDefItem->fMana_Regen[1]);
	else
		lpItem->fMana_Regen = lpDefItem->fMana_Regen[0];

	if (lpDefItem->fLife_Regen[1] != 0)
		lpItem->fLife_Regen = GetRandomFloatPos2(lpDefItem->fLife_Regen[1], lpDefItem->fLife_Regen[1]);
	else
		lpItem->fLife_Regen = lpDefItem->fLife_Regen[0];

	if (lpDefItem->fStamina_Regen[1] != 0)
		lpItem->fStamina_Regen = GetRandomFloatPos2(lpDefItem->fStamina_Regen[1], lpDefItem->fStamina_Regen[1]);
	else
		lpItem->fStamina_Regen = lpDefItem->fStamina_Regen[0];

	if (lpDefItem->Increase_Life[1])
		lpItem->fIncrease_Life = (float)GetRandomPos_Normal(lpDefItem->Increase_Life[1], lpDefItem->Increase_Life[1]);
	else
		lpItem->fIncrease_Life = (float)lpDefItem->Increase_Life[0];

	if (lpDefItem->Increase_Mana[1])
		lpItem->fIncrease_Mana = (float)GetRandomPos_Normal(lpDefItem->Increase_Mana[1], lpDefItem->Increase_Mana[1]);
	else
		lpItem->fIncrease_Mana = (float)lpDefItem->Increase_Mana[0];

	if (lpDefItem->Increase_Stamina[1])
		lpItem->fIncrease_Stamina = (float)GetRandomPos_Normal(lpDefItem->Increase_Stamina[1], lpDefItem->Increase_Stamina[1]);
	else
			...
			for (cnt = 0; cnt < sSPECIAL_REVISION_MAX; cnt++) {
				if (sSpecialRevision[cnt].dwJobCode == dwSpecialJob) {
					if (lpItem->Level && sSpecialRevision[cnt].sLevel[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sLevel[1], sSpecialRevision[cnt].sLevel[1]);
						Revision = (lpItem->Level * RndPos) / 100;
						lpItem->Level += Revision;
					}
					if (lpItem->Strength && sSpecialRevision[cnt].sStrength[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sStrength[1], sSpecialRevision[cnt].sStrength[1]);
						Revision = (lpItem->Strength * RndPos) / 100;
						lpItem->Strength += Revision;
					}
					if (lpItem->Spirit && sSpecialRevision[cnt].sSpirit[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sSpirit[1], sSpecialRevision[cnt].sSpirit[1]);
						Revision = (lpItem->Spirit * RndPos) / 100;
						lpItem->Spirit += Revision;
					}
					if (lpItem->Talent && sSpecialRevision[cnt].sTalent[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sTalent[1], sSpecialRevision[cnt].sTalent[1]);
						Revision = (lpItem->Talent * RndPos) / 100;
						lpItem->Talent += Revision;
					}
					if (lpItem->Dexterity && sSpecialRevision[cnt].sDexterity[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sDexterity[1], sSpecialRevision[cnt].sDexterity[1]);
						Revision = (lpItem->Dexterity * RndPos) / 100;
						lpItem->Dexterity += Revision;
					}
					if (lpItem->Health && sSpecialRevision[cnt].sHealth[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sHealth[1], sSpecialRevision[cnt].sHealth[1]);
...
// Cria item perfeito
int CreatePerfItem(sITEMINFO* lpItem, sDEF_ITEMINFO* lpDefItem, int SpJobCode, int ItemNormam)
{
	int is, ie;
	int cnt;
	JOB_DATA_CODE* lpJobDataCode = 0;
	...
	if (SpJobCode > 0) {
		...
		if (lpJobDataCode)
		{
			...
			for (cnt = 0; cnt < lpDefItem->JobBitCodeRandomCount; cnt++) {
				...
				if (lpDefItem->dwJobBitCode_Random[cnt] == dwSpecialJob) {
					break;
				}
	...
	if (lpItem->JobCodeMask || lpDefItem->JobBitCodeRandomCount || SpJobCode) {
		...
		if (cnt <= 3 || SpJobCode) {
			...
			for (cnt = 0; cnt < sSPECIAL_REVISION_MAX; cnt++) {
				if (sSpecialRevision[cnt].dwJobCode == dwSpecialJob) {
					if (lpItem->Level && sSpecialRevision[cnt].sLevel[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sLevel[0], sSpecialRevision[cnt].sLevel[0]);
						Revision = (lpItem->Level * RndPos) / 100;
						lpItem->Level += Revision;
					}
					if (lpItem->Strength && sSpecialRevision[cnt].sStrength[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sStrength[0], sSpecialRevision[cnt].sStrength[0]);
						Revision = (lpItem->Strength * RndPos) / 100;
						lpItem->Strength += Revision;
					}
					if (lpItem->Spirit && sSpecialRevision[cnt].sSpirit[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sSpirit[0], sSpecialRevision[cnt].sSpirit[0]);
						Revision = (lpItem->Spirit * RndPos) / 100;
						lpItem->Spirit += Revision;
					}
					if (lpItem->Talent && sSpecialRevision[cnt].sTalent[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sTalent[0], sSpecialRevision[cnt].sTalent[0]);
						Revision = (lpItem->Talent * RndPos) / 100;
						lpItem->Talent += Revision;
					}
					if (lpItem->Dexterity && sSpecialRevision[cnt].sDexterity[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sDexterity[0], sSpecialRevision[cnt].sDexterity[0]);
						Revision = (lpItem->Dexterity * RndPos) / 100;
						lpItem->Dexterity += Revision;
					}
					if (lpItem->Health && sSpecialRevision[cnt].sHealth[0]) {
						RndPos = GetRandomPos_Normal(sSpecialRevision[cnt].sHealth[0], sSpecialRevision[cnt].sHealth[0]);
						Revision = (lpItem->Health * RndPos) / 100;
						lpItem->Health += Revision;
					}
				}
			...
			if (lpDefItem->Lev_Attack_Rating[1])
			{
				lpItem->JobItem.Lev_Attack_Rating = 1;
			}
			else
			{
				lpItem->JobItem.Lev_Attack_Rating = lpDefItem->Lev_Attack_Rating[0];
...
int CreatePerfItemShop(sITEMINFO* lpItem, sDEF_ITEMINFO* lpDefItem, int SpJobCode, int ItemNormam)
{
	int is, ie;
	int cnt;
	JOB_DATA_CODE* lpJobDataCode = 0;
	...
	if (SpJobCode > 0) {
		...
		if (lpJobDataCode)
		{
			...
			for (cnt = 0; cnt < lpDefItem->JobBitCodeRandomCount; cnt++) {
				...
				if ((lpDefItem->dwJobBitCode_Random[cnt] == dwSpecialJob) && dwSpecialJob != 0) {
					flag = true;
					lpItem->JobCodeMask = dwSpecialJob;
					break;
				}
...
int CreateDefItem(sITEMINFO* lpItem, sDEF_ITEMINFO* lpDefItem, int SpJobCode, int ItemNormam)
{
	...
	if (lpItem->JobCodeMask || lpDefItem->JobBitCodeRandomCount || SpJobCode)
	{
		...
		// Se o numero sorteado for menor ou igual a 3 entra aqui
		if (cnt <= 5 || SpJobCode)
		{
			// sorteia outro numero de 0 a 9
			cnt = rand() % 10;

			dwSpecialJob = lpItem->JobCodeMask;

			// se o numero for menor que 3, seta o jobcode como 0 < se aumentaro 3 aqui e ali em cima, mais chances de cair 1 spec
			if (cnt < 5 && lpDefItem->JobBitCodeRandomCount) {
				lpItem->JobCodeMask = 0;
			}

			// se jobcode = 0, entra aqui e pega um spec aleatorio que o item tenha
			if (!lpItem->JobCodeMask && lpDefItem->JobBitCodeRandomCount)
			{
				if (tempJobCode > 0)
				{
				...
				{
					// como a jera é passada com jobcode 0, ele vai parar aqui se as condicoes acima forem atendidas
					// aqui ele sorteia 1 spec aleatorio
					cnt = rand() % lpDefItem->JobBitCodeRandomCount;
					dwSpecialJob = lpDefItem->dwJobBitCode_Random[cnt];
					lpItem->JobCodeMask = dwSpecialJob;
			...
			// Item com spec = 10% mais caro para vender no NPC
			if (lpItem->JobCodeMask) lpItem->Price += (lpItem->Price * 2) / 10;
...
extern std::string ConvertUnicode(const std::wstring& strUnicodeText);
...
// Lê os monstros de Bellatra aqui
	...
	while (!feof(fp))
	{
		...
		if (!NextFlag && lstrcmpi(decode, "*BELATRA_MONSTER") == 0) {
			if (hcRoundCount < rsHARDCORE_COUNT_END) {
				...
				std::wstring Monster = s2ws(strBuff);
				sprintf_s(hcMonster[hcRoundCount].hCoreMonster1.szName, sizeof(hcMonster[hcRoundCount].hCoreMonster1.szName), "%ls", Monster.c_str());

				p = GetWord(strBuff, p);
				hcMonster[hcRoundCount].hCoreMonster1.perCount = atoi(strBuff);
				pb = p; p = GetWord(strBuff, p); if (strBuff[0] == 34)p = GetString(strBuff, pb);

				Monster = s2ws(strBuff);
				sprintf_s(hcMonster[hcRoundCount].hCoreMonster2.szName, sizeof(hcMonster[hcRoundCount].hCoreMonster2.szName), "%ls", Monster.c_str());
				...
				Monster = s2ws(strBuff);
				sprintf_s(hcMonster[hcRoundCount].hCoreMonster3.szName, sizeof(hcMonster[hcRoundCount].hCoreMonster3.szName), "%ls", Monster.c_str());
				...
				Monster = s2ws(strBuff);
				sprintf_s(hcMonster[hcRoundCount].hCoreMonster4.szName, sizeof(hcMonster[hcRoundCount].hCoreMonster4.szName), "%ls", Monster.c_str());
				...
				Monster = s2ws(strBuff);
				sprintf_s(hcMonster[hcRoundCount].hCoreMonsterBoss.szName, sizeof(hcMonster[hcRoundCount].hCoreMonsterBoss.szName), "%ls", Monster.c_str());

...
// Dib를 서페이스에 그린다
int DrawSurfaceFromDib( void* lpDDSurface , BYTE *lpDib );

//////////////// [ BMP파일을 텍스쳐 서페이스로 만든다 ] ///////////////
void* LoadDibSurface( char *Filename , int SysMem=FALSE );
////////// [ BMP파일을 텍스쳐 서페이스로 만든다 (알파텍스쳐랑 합성 ) ] ///////
void* LoadDibSurfaceAlpha( char *Filename , char *FileAlpha , int SysMem=FALSE );
...
int DrawAnimBG( void* lpDDSSrc , int frame );
int LoadAnimBG();
int CloseAnimBG();
int ClearBG( void* lpDDSSrc, DWORD color );

...
int dm_SendTransDamage(smCHAR* lpChar, int PowParam1, int PowParam2, int AttackState, int Resistance, DWORD dwSkillCode, int UseAccuracy, smWINSOCK* lpsmSock)
{
	...
	if (UseAccuracy)
	{
		...
		case smCHAR_STATE_ENEMY:
			...
			// Ajuste para os personagens errarem menos os hits
			if (lpCurPlayer->smCharInfo.JOB_CODE == JOBCODE_ARCHER) {
				accuracy = sinGetAccuracy(lpChar->smCharInfo.Level, lpChar->smCharInfo.Defence / 1.05);
			}
			else {
				accuracy = sinGetAccuracy(lpChar->smCharInfo.Level, lpChar->smCharInfo.Defence / 1.14);
			}
			...
			{
				char szBuff[256];
				wsprintf(szBuff, "Probabilidade: %d/%d", rnd, accuracy);
	...
	if (lpCurPlayer->PlayEvasionMastery && lpChar->smCharInfo.State == smCHAR_STATE_ENEMY)
	{
		if (rand() % 100 < 50)
		{
			return FALSE;
	...
	/*
	// Ataque dos aneis
	if (cInvenTory.InvenItem[sInven[5].ItemIndex - 1].sItemInfo.Damage[0])
	{
		TransAttackData.Power[0] += cInvenTory.InvenItem[sInven[5].ItemIndex - 1].sItemInfo.Damage[0];
		TransAttackData.Power[1] += cInvenTory.InvenItem[sInven[5].ItemIndex - 1].sItemInfo.Damage[1];
	}


	// Ataque dos aneis
	if (cInvenTory.InvenItem[sInven[6].ItemIndex - 1].sItemInfo.Damage[0])
	{
		TransAttackData.Power[0] += cInvenTory.InvenItem[sInven[6].ItemIndex - 1].sItemInfo.Damage[0];
		TransAttackData.Power[1] += cInvenTory.InvenItem[sInven[6].ItemIndex - 1].sItemInfo.Damage[1];
	}


	// Ataque dos amuletos
	if (cInvenTory.InvenItem[sInven[4].ItemIndex - 1].sItemInfo.Damage[0])
	{
		TransAttackData.Power[0] += cInvenTory.InvenItem[sInven[4].ItemIndex - 1].sItemInfo.Damage[0];
		TransAttackData.Power[1] += cInvenTory.InvenItem[sInven[4].ItemIndex - 1].sItemInfo.Damage[1];
	}


	// Ataque dos Sheltons
	if (cInvenTory.InvenItem[sInven[7].ItemIndex - 1].sItemInfo.Damage[0])
	{
		TransAttackData.Power[0] += cInvenTory.InvenItem[sInven[7].ItemIndex - 1].sItemInfo.Damage[0];
		TransAttackData.Power[1] += cInvenTory.InvenItem[sInven[7].ItemIndex - 1].sItemInfo.Damage[1];
	}


	if (cInvenTory.InvenItem[sInven[5].ItemIndex - 1].sItemInfo.Critical_Hit)
	{
		TransAttackData.Critical[0] += cInvenTory.InvenItem[sInven[5].ItemIndex - 1].sItemInfo.Critical_Hit;
	}

	if (cInvenTory.InvenItem[sInven[6].ItemIndex - 1].sItemInfo.Critical_Hit)
	{
		TransAttackData.Critical[0] += cInvenTory.InvenItem[sInven[6].ItemIndex - 1].sItemInfo.Critical_Hit;
	}

	if (cInvenTory.InvenItem[sInven[4].ItemIndex - 1].sItemInfo.Critical_Hit)
	{
		TransAttackData.Critical[0] += cInvenTory.InvenItem[sInven[4].ItemIndex - 1].sItemInfo.Critical_Hit;
	}


	if (cInvenTory.InvenItem[sInven[7].ItemIndex - 1].sItemInfo.Critical_Hit)
	{
		TransAttackData.Critical[0] += cInvenTory.InvenItem[sInven[7].ItemIndex - 1].sItemInfo.Critical_Hit;
	}
	*/

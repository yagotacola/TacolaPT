...
extern int SetBoss1Bonus1;
extern int SetBoss1Bonus2;
extern int SetBoss2Bonus1;
extern int SetBoss2Bonus2;
...
//채팅 문자를 서버로 전송
int SendChatMessageToServer( std::string szMessage );
//다른 유저와 연결 시킨다
...
int	SaveCaravan(sCARAVAN* lpWareHouse, TRANS_CARAVAN* lpTransWareHouse);
...
extern int	ShowIncoScrol;    //show iconscrol chat xxstr
...
int SendWingItemEVENTPRISTON_ToServer(void *lpsCraftItem_Send); //event priston xxstr
//판매할 아이템을 서버로 보냄
...
DWORD	GetPlayTime_T();
//머리가 커지는 모드 설정
int	ChangeBigHeadMode( int Mode , int Time, bool bNew = false );

...
//							sAgingItem.AgingItem[j].Flag = 0;  //아이템을 없애준다 
	...
	if (cElement.OpenFlag)
	{
		for (j = sinMixtureResetDelayCount; j < sinMixtureResetDelayCount + 1; j++)
		{
			if (cElement.ElementItem[j].Flag)
			{
				sinMixtureResetDelayCount++;
				...
				for (i = 0; i < SIN_MAX_INTER_EFFECT; i++)
				{
					if (!sInter_Effect[i].Time)
					{
						sInter_Effect[i].Kind = SIN_INTER_EFFECT_AGING2;
						sInter_Effect[i].Time = 1;
						sInter_Effect[i].Face.x = cElement.ElementItem[j].x + 5;
						sInter_Effect[i].Face.y = cElement.ElementItem[j].y + 5;
						sInter_Effect[i].Face.width = rand() % 5 + 5;
						sInter_Effect[i].Face.height = sInter_Effect[i].Face.width;
						sInter_Effect[i].Max_Time = rand() % 10 + 5;
						sInter_Effect[i].Face.Transparency = 255;
						...
						if (CountTemp >= 24)
						{
							...
							cElement.ElementItem[j].x = 10000;
							cElement.ElementItem[j].y = 10000;
			...
			if (cSocket.SocketItem[j].Flag)
			{
				...
				for (i = 0; i < SIN_MAX_INTER_EFFECT; i++)
				{
					if (!sInter_Effect[i].Time)
					{
						sInter_Effect[i].Kind = SIN_INTER_EFFECT_AGING2;
						sInter_Effect[i].Time = 1;
						sInter_Effect[i].Face.x = cSocket.SocketItem[j].x + 5;
						sInter_Effect[i].Face.y = cSocket.SocketItem[j].y + 5;
						sInter_Effect[i].Face.width = rand() % 5 + 5;
						sInter_Effect[i].Face.height = sInter_Effect[i].Face.width;
						sInter_Effect[i].Max_Time = rand() % 10 + 5;
						sInter_Effect[i].Face.Transparency = 255;
						...
						if (CountTemp >= 24)
						{
							CountTemp = 0;
							sinPlaySound(SIN_SOUND_FAILD_MIXITEM);
							cSocket.SocketItem[j].x = 10000;
							cSocket.SocketItem[j].y = 10000;

...
void cSINHELP::Init()
{
	...
	dwCheckBox[0] = CreateTextureMaterial( "game\\images\\npc\\checkbox.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA );

	dwCheckBox[1] = CreateTextureMaterial( "game\\images\\npc\\checkbox_.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA );

	dwWindowImage = CreateTextureMaterial("game\\images\\npc\\BattleRoyale\\Window.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);

	dwQuestWindowImage = CreateTextureMaterial("game\\images\\npc\\windowQuest.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);

	dwButtonOK[0] = CreateTextureMaterial("game\\images\\npc\\BattleRoyale\\btnenter.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	dwButtonOK[1] = CreateTextureMaterial("game\\images\\npc\\BattleRoyale\\btnenter_.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);

	dwButtonClose[0] = CreateTextureMaterial("game\\images\\npc\\BattleRoyale\\btnexit.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	dwButtonClose[1] = CreateTextureMaterial("game\\images\\npc\\BattleRoyale\\btnexit_.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
...
BOOL bNewTeleportWindow = FALSE;
BOOL bQuestWindow = FALSE;

void cSINHELP::CheckBox( HDC h, int x, int y, char * text, bool b )
{
	if( b )
		dsDrawTexImage( dwCheckBox[1], x, y, 13, 13, 255 );
	else
		dsDrawTexImage( dwCheckBox[0], x, y, 13, 13, 255 );
	...
	{
		HFONT oldFont;
		oldFont = (HFONT)SelectObject( h, hFont );
		dsTextLineOut( h, x + 20, y, text, strlen( text ) );
...
BOOL bCheckBoxes[8] = { 0,0,0,0,0,0,0,0 };

BOOL bQuestCheckBoxes[3] = { 0,0,0 };
extern int iNewTeleportIndex;

void cSINHELP::Draw()
{
	//Render New Telport Window
	if (bNewTeleportWindow)
	{
		int iBaseX = 55;
		int iBaseY = 50;

		//Base Window
		dsDrawTexImage(dwWindowImage, iBaseX, iBaseY, 392, 354, 255);

		HDC hdc = NULL;

		//wsprintf(szTime, "%02d:%02d:%02d", TimeHourArena, TimeMinuteArena, TimeSegundoArena);
		//DrawFontText(iBaseX + 151, iBaseY + 220, szTime, RGB(255, 255, 255), false);

		//Button OK
		if (GetClick(iBaseX + 135, iBaseY + 245, 66, 21))
		{
			dsDrawTexImage(dwButtonOK[1], iBaseX + 135, iBaseY + 240, 122, 48, 255);
			DrawFontText(iBaseX + 151, iBaseY + 258, "Efetuar inscri豫o", RGB(255, 255, 255), false);
			//Funcao Botao OK
			if (MouseButton[0])
			{
				int i = 0;
				for (i = 0; i < 8; i++)
				{
					if (bCheckBoxes[i] == TRUE)
						break;
				}

				iNewTeleportIndex = i;
				cMessageBox.ShowMessage3(MESSAGE_ARENA, "Entrar na arena");

				bNewTeleportWindow = FALSE;
				return;
			}
		}
		else
			dsDrawTexImage(dwButtonOK[0], iBaseX + 135, iBaseY + 240, 122, 48, 255);
		DrawFontText(iBaseX + 151, iBaseY + 258, "Efetuar inscri豫o", RGB(255, 255, 255), false);

		//Button Close
		if (GetClick(iBaseX + 310, iBaseY + 285, 66, 21))
		{
			dsDrawTexImage(dwButtonClose[1], iBaseX + 310, iBaseY + 280, 50, 50, 255);
			...
			dsDrawTexImage(dwButtonClose[0], iBaseX + 310, iBaseY + 280, 50, 50, 255);
	...
	/*if( bQuestWindow )
	{
		...
		//Base Window
		dsDrawTexImage( dwQuestWindowImage, iBaseX, iBaseY, 352, 362, 255 );
	}*/
...
void cSINHELP::Main()
{
	...
	for(int i = 0 ; i < SIN_MAX_HELP_NUMBER; i++){         
		if(sSinHelp[i].KindFlag){ 
			...
			if( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TELEPORT_MAR )
			{
				for( int j = 0; j < 1; j++ )
				{
					if( sSinHelp[ i ].PosiX + 20 < pCursorPos.x  && sSinHelp[ i ].PosiX + sSinHelp[ i ].SizeW * 32 > pCursorPos.x &&
			...
			if( sSinHelp[i].KindFlag == SIN_HELP_KIND_TELEPORT_BA )
			{
				for( int j = 0; j < 1; j++ )
				{
					if( sSinHelp[i].PosiX + 20 < pCursorPos.x  && sSinHelp[i].PosiX + sSinHelp[i].SizeW * 32 > pCursorPos.x &&
			...
			if( sSinHelp[ i ].KindFlag == SIN_HELP_KING_TELEPORT_ARENA )
			{
				for( int j = 0; j < 1; j++ )
				{
					if( sSinHelp[ i ].PosiX + 20 < pCursorPos.x  && sSinHelp[ i ].PosiX + sSinHelp[ i ].SizeW * 32 > pCursorPos.x &&
			...
			if( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TRADEBPEXP )
			{
				for( int j = 0; j < 1; j++ )
				{
					if( sSinHelp[ i ].PosiX + 20 < pCursorPos.x  && sSinHelp[ i ].PosiX + sSinHelp[ i ].SizeW * 32 > pCursorPos.x &&
			...
			if(sSinHelp[i].KindFlag == SIN_HELP_KIND_TELEPORT_SCROLL){
				if(chaPremiumitem.TelePortCore.PageIndex == 3)
					Temp = 8;	//kyle //로스트아일랜드, 로스트템플  // 장별 - 텔레포트 코어 필드 추가
				else
					Temp = 10;
	...
	for(int i = 0 ; i < SIN_MAX_HELP_NUMBER; i++){        
		if(sSinHelp[i].KindFlag){
			...
			if( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TELEPORT_MAR )
			{
				...
				{
					sinTeleportMoneyIndex = sinTeleportIndex;
					cMessageBox.ShowMessage3( MESSAGE_TELEPORT_MAR, "New Map" );
					cSinHelp.NPCTeleportFlag = 0;
					HelpBoxKindIndex = 0;
					memset( &sSinHelp[ i ], 0, sizeof( sSINHELP ) );
					break; //전업퀘스트가 실행될 가능성이 있으므로 break한다
			...
			if( sSinHelp[ i ].KindFlag == SIN_HELP_KING_TELEPORT_ARENA )
			{
				...
				{
					sinTeleportMoneyIndex = sinTeleportIndex;
					cMessageBox.ShowMessage3( MESSAGE_TELEPORT_ARENA, "Teleport" );
					cSinHelp.NPCTeleportFlag = 0;
					HelpBoxKindIndex = 0;
					memset( &sSinHelp[ i ], 0, sizeof( sSINHELP ) );
					break; //전업퀘스트가 실행될 가능성이 있으므로 break한다
			...
			if( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TRADEBPEXP )
			{
				...
				{
					sinTeleportMoneyIndex = sinTeleportIndex;
					cMessageBox.ShowMessage3( MESSAGE_TRADE_BPEXP, "BP por EXP" );
					cSinHelp.NPCTeleportFlag = 0;
					HelpBoxKindIndex = 0;
					memset( &sSinHelp[ i ], 0, sizeof( sSINHELP ) );
					break; //전업퀘스트가 실행될 가능성이 있으므로 break한다
			...
			if( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TELEPORT_BA )
			{
				...
				{
					sinTeleportMoneyIndex = sinTeleportIndex;
					cMessageBox.ShowMessage3( MESSAGE_TELEPORT_BA, "Teleport" );
					cSinHelp.NPCTeleportFlag = 0;
					HelpBoxKindIndex = 0;
					memset( &sSinHelp[ i ], 0, sizeof( sSINHELP ) );
					break; //전업퀘스트가 실행될 가능성이 있으므로 break한다
...
void cSINHELP::KeyDown()
{
	...
	if(smConfig.DebugMode){ 
		...
		//	sinShowHelp(SIN_HELP_KIND_NORMAL,150,150,6,6, D3DCOLOR_RGBA(128,15,15,125)); 
		...
		//	}
		//	sinShowHelp(SIN_HELP_KIND_NORMAL,150,150,6,5, D3DCOLOR_RGBA(0,128,15,125)); 

		//}
		/*
		if(sinGetKeyClick('5')){          
			//sinShowHelp(SIN_HELP_KIND_NORMAL,175,93,7,5, D3DCOLOR_RGBA(0,15,128,125)); 
		}
		*/
	}
...
void cSINHELP::DrawText()
{
	...
	for(i = 0 ; i < SIN_MAX_HELP_NUMBER; i++){         
		if(sSinHelp[i].KindFlag){      
			...
			SelectObject( hdc, sinFont);
			//SetBkMode( hdc, TRANSPARENT ); 
			SetFontTextColor( hdc, RGB(255,255,180) ); 
			*/
			//haGoon
			haResearch.Number = 0;
			//텔레포트 코어
						...
						for(int p = 0; p < cnt; p++){
							...
							if(strBuff[p] == '.'){
								if(atoi(&strBuff[p-1])==chaPremiumitem.TelePortCore.FieldIndex[0]-1 && chaPremiumitem.TelePortCore.FieldIndex[0]!=0){
									SetFontTextColor( RGB(255,100,45) );
									strcpy(chaPremiumitem.TelePortCore.FieldName,&strBuff[p+1]);							
								}
								else
									SetFontTextColor( RGB(240,240,240) );
							}
				...
				for(j=0 ; j < tHeight-1 ; j++){
					if(sSinHelp[i].szDocBuff[j+TempLine4]){
						...
						if( sinTeleportIndex+10 == j){
							SelectObject( hdc, sinFont);
							//SetBkMode( hdc, TRANSPARENT ); 
							SetFontTextColor( RGB(255,171,45) ); 
						}
						else{
							SelectObject( hdc, sinFont);
							//SetBkMode( hdc, TRANSPARENT ); 
						...
						if(sinChar->Level < FieldLimitLevel_Table[12] && j == 14){  
							SelectObject( hdc, sinFont);
							//SetBkMode( hdc, TRANSPARENT ); 
							SetFontTextColor( RGB(128,128,128) ); 
			...
			// pluto 밀터 던전 워프
			else if( ( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TELEPORT_MILTER ) || ( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TELEPORT_MAR ) || ( sSinHelp[i].KindFlag == SIN_HELP_KIND_TELEPORT_BA ) || ( sSinHelp[ i ].KindFlag == SIN_HELP_KING_TELEPORT_ARENA ) || ( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TRADEBPEXP )  )
			{ //텔레포트일경우 글자색을 달리해준다 
				for(j=0 ; j < tHeight-1 ; j++){
					if(sSinHelp[i].szDocBuff[j+TempLine4]){
						wsprintf(strBuff,"%s",sSinHelp[i].szDocBuff[j+TempLine4]);
			...
			if(sSinHelp[i].KindFlag == SIN_HELP_KIND_NORMAL){ //도움말            
				...
				*/
				SelectObject( hdc, sinBoldFont);
				SetFontTextColor( RGB(255,255,240) ); 
			...
			else if(sSinHelp[i].KindFlag == SIN_HELP_KIND_BABEL || ( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TRADEBPEXP )){
				SelectObject( hdc, sinBoldFont);
				SetFontTextColor( RGB(255,255,240) ); 
			...
			else if( ( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TELEPORT_MILTER ) || ( sSinHelp[i].KindFlag == SIN_HELP_KIND_TELEPORT_BA ) || ( sSinHelp[ i ].KindFlag == SIN_HELP_KIND_TELEPORT_MAR ) || ( sSinHelp[ i ].KindFlag == SIN_HELP_KING_TELEPORT_ARENA )  )	// pluto 밀터 던전 워프
			{
				SelectObject( hdc, sinBoldFont);
				SetFontTextColor( RGB(255,255,240) ); 
#ifdef _LANGUAGE_ARGENTINA
			...
			if (bQuestWindow) {
				...
				if (sSinHelp[i].KindFlag == SIN_HELP_DAILYQUEST)
				{
					if (sinQuest_Daily.State == 0)
					{
						...
						//Checkboxes
						CheckBox(hdc, iBaseX + 30, iBaseY + 245, "Daily Quest # 1 - Easy Difficulty - Level 80", bQuestCheckBoxes[0]);
						CheckBox(hdc, iBaseX + 30, iBaseY + 265, "Daily Quest # 1 - Medium Difficulty - Level 90", bQuestCheckBoxes[1]);
						CheckBox(hdc, iBaseX + 30, iBaseY + 285, "Daily Quest # 1 - Hard Difficulty - Level 100", bQuestCheckBoxes[2]);
						...
						if (MouseButton[0])
						{
							for (int i = 0; i < 3; i++)
							{
								if (GetClick(iBaseX + 30, iBaseY + 245 + (i * 20), 15, 15))
								{
									for (int k = 0; k < 3; k++)
										bQuestCheckBoxes[k] = FALSE;
						...
						//Button OK
						if (GetClick(iBaseX + 120, iBaseY + 322, 66, 21))
						{

							for (int i = 0; i < 3; i++)
							{
								// verifica se algum checkbox foi escolhido
								if (bQuestCheckBoxes[i])
								{
									//Funcao Botao OK
									if (MouseButton[0])
									{
										...
										for (i = 0; i < 8; i++)
										{
											if (bQuestCheckBoxes[i] == TRUE)
												break;
										}
										// seleciona o level da quest e pega o ID
										if (i == 0)
											iDailyQuestCode = 435501;
										else if (i == 1)
											iDailyQuestCode = 435502;
										else if (i == 2)
											iDailyQuestCode = 435503;

										cMessageBox.ShowMessage3(MESSAGE_DAILYQUEST, "Daily Quest");
								...
								{
									//dsDrawTexImage(dwButtonOK[0], iBaseX + 97, iBaseY + 322, 66, 21, 255);
...
int cSINHELP::ShowSodRanking(TRANS_SOD_RESULT *SodResult ,int MyGame)
{
	...
	*/
	...
	else cSinHelp.sinShowHelp(SIN_HELP_KIND_SOD,QuestMessageBoxPosi2.x,QuestMessageBoxPosi2.y,QuestMessageBoxSize2.x,QuestMessageBoxSize2.y, D3DCOLOR_RGBA(0,15,128,125),"");  
...
int TeleportMars()
{
	char *TeleportText = "image\\Sinimage\\Help\\NewTeleport.sin";
	cSinHelp.NPCTeleportFlag = 1;

	cSinHelp.sinShowHelp( SIN_HELP_KIND_TELEPORT_MAR, QuestMessageBoxPosi2.x, QuestMessageBoxPosi2.y, QuestMessageBoxSize2.x, QuestMessageBoxSize2.y, D3DCOLOR_RGBA( 0, 15, 128, 125 ), TeleportText );
...
int TeleportBA()
{
	char *TeleportText = "image\\Sinimage\\Help\\BATeleport.sin";
	cSinHelp.NPCTeleportFlag = 1;

	cSinHelp.sinShowHelp( SIN_HELP_KIND_TELEPORT_BA, QuestMessageBoxPosi2.x, QuestMessageBoxPosi2.y, QuestMessageBoxSize2.x, QuestMessageBoxSize2.y, D3DCOLOR_RGBA( 0, 15, 128, 125 ), TeleportText );
...
int TeleportArena()
{
	char *TeleportText = "image\\Sinimage\\Help\\EragonTeleport.sin";
	cSinHelp.NPCTeleportFlag = 1;

	cSinHelp.sinShowHelp( SIN_HELP_KING_TELEPORT_ARENA, QuestMessageBoxPosi2.x, QuestMessageBoxPosi2.y, QuestMessageBoxSize2.x, QuestMessageBoxSize2.y, D3DCOLOR_RGBA( 0, 15, 128, 125 ), TeleportText );
...
int TradeBPEXP()
{
	char *TeleportText = "image\\Sinimage\\Help\\BattlePointsEXP.sin";
	cSinHelp.NPCTeleportFlag = 1;

	cSinHelp.sinShowHelp( SIN_HELP_KIND_TRADEBPEXP, QuestMessageBoxPosi2.x, QuestMessageBoxPosi2.y, QuestMessageBoxSize2.x, QuestMessageBoxSize2.y, D3DCOLOR_RGBA( 0, 15, 128, 125 ), TeleportText );
...
int haShowCastleTeleportMap()
{
	...
	if ( sBiInfo && (sBiInfo->EventPlay[0]&BIMASK_ADULT_USER)!=0 ){
		...
		cSinHelp.sinShowHelp(SIN_HELP_KIND_C_TELEPORT,QuestMessageBoxPosi2.x,QuestMessageBoxPosi2.y,QuestMessageBoxSize2.x,QuestMessageBoxSize2.y, D3DCOLOR_RGBA(0,15,128,125),												haC_Not_TeleportFilePath);  
	}
	*/
	return TRUE;
}

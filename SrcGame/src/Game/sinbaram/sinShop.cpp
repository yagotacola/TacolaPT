...
void cSHOP::Load()
{
	...
	lpAging_Info2 = LoadDibSurfaceOffscreen("Image\\SinImage\\Shopall\\Aging\\infoage.bmp");
	lpAging_btnInfo = LoadDibSurfaceOffscreen("Image\\SinImage\\Shopall\\Aging\\botaoinfo.bmp");
	lpAging_btnInfo_ = LoadDibSurfaceOffscreen("Image\\SinImage\\Shopall\\Aging\\botaoinfo_.bmp");

	lpModifyButtonInfo = LoadDibSurfaceOffscreen("Image\\SinImage\\Shopall\\modifyitem\\buttoninfo.bmp");
...
void cSHOP::Release()
{
	...
	if (lpSocketTitle)
	{
		lpSocketTitle->Release();
		lpSocketTitle = 0;
	}
	if (lpSocketMain)
	{
		lpSocketMain->Release();
		lpSocketMain = 0;
	}
	if (lpSocketButton)
	{
		lpSocketButton->Release();
		lpSocketButton = 0;
	}
	if (lpSocketButtonInfo)
	{
		lpSocketButtonInfo->Release();
		lpSocketButtonInfo = 0;
	}


	if (lpElementTitle)
	{
		lpElementTitle->Release();
		lpElementTitle = 0;
	}
	if (lpElementMain)
	{
		lpElementMain->Release();
		lpElementMain = 0;
	}
	if (lpElementButton)
	{
		lpElementButton->Release();
		lpElementButton = 0;
	}
	if (lpElementButtonInfo)
	{
		lpElementButtonInfo->Release();
		lpElementButtonInfo = 0;
	}
}

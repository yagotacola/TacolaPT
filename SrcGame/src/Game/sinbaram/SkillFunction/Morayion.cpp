...
{
	int i = 0;
	if (lpSkill->CODE == SKILL_ENERGY_SHIELD || lpSkill->CODE == SKILL_COMPULSION || lpSkill->CODE == SKILL_DRASTIC_SPIRIT)
	{
		for (i = 0; i < MAX_CONTINUE_SKILL; i++)
		{
			if (ContinueSkill[i].Flag)
			{
				if (ContinueSkill[i].CODE == SKILL_ENERGY_SHIELD || ContinueSkill[i].CODE == SKILL_COMPULSION || ContinueSkill[i].CODE == SKILL_DRASTIC_SPIRIT)
				{
	...
	if (lpSkill->CODE == SKILL_HOLY_VALOR)
	{
		for (i = 0; i < MAX_CONTINUE_SKILL; i++)
		{
			if (ContinueSkill[i].Flag)
			{
				if (ContinueSkill[i].CODE == SKILL_HOLY_VALOR)
				{
					memset(&ContinueSkill[i], 0, sizeof(sSKILL));
					break;
				}
	...
	if (lpSkill->CODE == SKILL_MAXIMIZE)
	{
		for (i = 0; i < MAX_CONTINUE_SKILL; i++)
		{
			if (ContinueSkill[i].Flag)
			{
				if (ContinueSkill[i].CODE == SKILL_METAL_ARMOR)
				{
					memset(&ContinueSkill[i], 0, sizeof(sSKILL));
					break;
				}
	...
	if (lpSkill->CODE == SKILL_METAL_ARMOR)
	{
		for (i = 0; i < MAX_CONTINUE_SKILL; i++)
		{
			if (ContinueSkill[i].Flag)
			{
				if (ContinueSkill[i].CODE == SKILL_MAXIMIZE)
				{
					memset(&ContinueSkill[i], 0, sizeof(sSKILL));
					break;
				}

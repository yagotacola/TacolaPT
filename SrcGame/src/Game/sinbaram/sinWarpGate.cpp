			...
			if(sSinWarpGate.MapPosi.x + sHaWarpGate.GatePosi[i].x-5 < pCursorPos.x && sSinWarpGate.MapPosi.x + sHaWarpGate.GatePosi[i].x +21 > pCursorPos.x &&
				sSinWarpGate.MapPosi.y + sHaWarpGate.GatePosi[i].y-5 < pCursorPos.y && sSinWarpGate.MapPosi.y + sHaWarpGate.GatePosi[i].y +21 > pCursorPos.y ){
					...
					if ( sBiInfo && (sBiInfo->EventPlay[0]&BIMASK_ADULT_USER)!=0 ) {
						cSinHelp.sinShowHelp(SIN_HELP_KIND_WARPGATE,QuestMessageBoxPosi2.x,QuestMessageBoxPosi2.y,QuestMessageBoxSize2.x,QuestMessageBoxSize2.y, D3DCOLOR_RGBA(0,15,128,125),HaWarpGateDoc[i]);  
					}
					else{
						cSinHelp.sinShowHelp(SIN_HELP_KIND_WARPGATE,QuestMessageBoxPosi2.x,QuestMessageBoxPosi2.y,QuestMessageBoxSize2.x,QuestMessageBoxSize2.y, D3DCOLOR_RGBA(0,15,128,125),haC_Not_TeleportFilePath);  
					}
					*/
				}
		}

...
#define INVENTORY_MAX_POS		19    //창고를 제외한 10곳의 아이템셋팅 포지션 뒤의 3은 임시이다 RECT만 사용한다 
...
public:
	...
	LPDIRECT3DTEXTURE9 lpItemElementClass[3];

	LPDIRECT3DTEXTURE9	lpIndiShopIcon;


	LPDIRECT3DTEXTURE9   lpPuzzleInven;     //퍼즐 칸
	LPDIRECT3DTEXTURE9   lpPuzzlewidth;     //가로퍼즐 잡을때 마우스이미지 
	LPDIRECT3DTEXTURE9   lpPuzzleHeight;    //세로퍼즐 잡을때 마우스이미지 
...
public:
	...
	/////////////////////////
	int ThrowInvenItemToField(sITEM *pItem ,int SellRight=0); //아이템을 버린다 
	...
	POINT CheckImageSize(LPDIRECT3DTEXTURE9 lpdd); //그림의 사이즈를 구해서 리턴한다 
	...
	int SearchItemByName(char name[64], int numberOfItems);
	...
	*-----------------------------------------------------------------------------*/
	...
	int SheltomUpToAgingTrade(int InvenIndex);
...
//일곱개의 보은
extern DWORD CristalEventCODE[13];

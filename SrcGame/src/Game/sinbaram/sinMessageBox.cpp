/*----------------------------------------------------------------------------*
*	∆ƒ¿œ∏Ì :  sinMessageBox.cpp
*	«œ¥¬¿œ :  ∞¢¡æ ∏ﬁºº¡ˆ π⁄Ω∫µÈ¿ª ∞¸∏Æ«—¥Ÿ
*	¿€º∫¿œ :  √÷¡ææ˜µ•¿Ã∆Æ 12ø˘
*	¿˚º∫¿⁄ :  π⁄ªÛø≠
*-----------------------------------------------------------------------------*/
...
sITEM MessageBoxItem; //∏ﬁºº¡ˆ π⁄Ω∫ø°º≠ æ≤¿œ ±∏¡∂√º 
sITEM* pMessageItem;  //∏ﬁºº¡ˆ π⁄Ω∫ø°º≠ æ≤¿œ æ∆¿Ã≈€ ∆˜¿Œ≈Õ 
sITEM* pMessageItem2;  //∏ﬁºº¡ˆ π⁄Ω∫ø°º≠ æ≤¿œ æ∆¿Ã≈€ ∆˜¿Œ≈Õ (∞«µÈ±‚ Ω√∑Øº≠ «—∞≥ ¥ı∏Õ±›)

cMESSAGEBOX cMessageBox;
int MessageTimeCount = 0;

int sinMessageBoxShowFlag = 0; //«ˆ¿Á ∏ﬁºº¡ˆπ⁄Ω∫∞° ¿÷≥™ æ¯≥™∏¶ «√∑¢¿∏∑Œ æÀºˆ¿÷¥Ÿ 

int MessageArrowPosi = 0;      //π⁄Ω∫¿« »≠ªÏ«• ∆˜¡ˆº« 
int MessageButtonPosi = 0;     //πˆ∆∞ ∆˜¡ˆº« 
...
int sinSkillPontInitButtonFlag = 0; //Ω∫≈≥√ ±‚»≠ ∏ﬁºº¡ˆ π⁄Ω∫ 
int sinGiveMoneyCheckFlag = 0; //±‚∫Œ±› «—π¯¥ı √º≈©
int sinGiveMoneyAmount = 0;
char szGiveMoneyBuff[32];

int sinInitPointFlag7 = 0;
int sinInitPointPassFlag = 0; //√ππ¯ «—π¯∏∏ ∆˜¿Œ∆Æ √º≈©∏¶ ∆–Ω∫«—¥Ÿ
...
{
	/////////////////////// ªÛ≈¬∏∏ «•Ω√«œ¥¬ ∏ﬁºº¡ˆ 
	MatBoxLeft = CreateTextureMaterial("game\\images\\UI\\title\\Left.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);   //¡¬√¯ π⁄Ω∫ ±◊∏≤ 
	MatBoxRight = CreateTextureMaterial("game\\images\\UI\\title\\Right.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);   //øÏ√¯ π⁄Ω∫ ±◊∏≤ 
	MatBoxCenter = CreateTextureMaterial("game\\images\\UI\\title\\Center.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);  //«œ¥‹ π⁄Ω∫ ±◊∏≤ 

	MatBoxMain = CreateTextureMaterial("Image\\SinImage\\MessageBox\\BoxMain.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);  //π⁄Ω∫ ∏ﬁ¿Œ 

	//»´∫∏π⁄Ω∫ ∂Û¿Œ
	MatMyShopPRBoxLine = CreateTextureMaterial("Image\\SinImage\\Inven\\PR_Box.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);  //π⁄Ω∫ ∏ﬁ¿Œ 
...
void cMESSAGEBOX::Draw()
{
	...
	if (sLost_Item.Flag) {
		...
		if (sLost_Item.LostItem[0].CODE) { //ªÁ∂Û¡˙ æ∆¿Ã≈€¿« ¿ÃπÃ¡ˆ∏¶ ∫∏ø©¡ÿ¥Ÿ
		...
		if (sLost_Item.LostItem[1].CODE) { //ªÁ∂Û¡˙ æ∆¿Ã≈€¿« ¿ÃπÃ¡ˆ∏¶ ∫∏ø©¡ÿ¥Ÿ
			x = (60 - sLost_Item.LostItem[0].w) / 2;
			y = (80 - sLost_Item.LostItem[0].h) / 2;
	...
	if (sMessageBox_RecvItem.Flag) {
		...
		if (sMessageBox_RecvItem.RecvItem.CODE) { //πﬁ¿ª æ∆¿Ã≈€¿« ¿ÃπÃ¡ˆ∏¶ ±◊∑¡¡ÿ¥Ÿ 
	...
	for (i = 0; i < MESSAGE3_MAX; i++) {
		if (sMessageBox3[i].Flag) {
			dsDrawTexImage(MatBoxMain, sMessageBox3[i].x, sMessageBox3[i].y, 256, 128, 255);  //ªÛ¥‹
	...
	for (i = 0; i < 20; i++) {
		if (sMessageBox2[i].Flag) {
			dsDrawTexImage(MatBoxMain, sMessageBox2[i].x, sMessageBox2[i].y, 256, 128, 255);  //ªÛ¥‹
			DrawSprite(sMessageBox2[i].x + 34, sMessageBox2[i].y + 62, lpCount, 0, 0, 109, 25);
			if (i != MESSAGE_BUY_STAR) { //øπ æ∆¥œø¿∑Œ «œ±‚¿ß«ÿ ªÏ¬¶ ∂´ªß
	...
	for (i = 0; i < 100; i++) {
		if (sMessageBox[i].Flag) {
			dsDrawTexImage(MatBoxLeft, sMessageBox[i].x, sMessageBox[i].y, 32, 43, 255);  //¡¬√¯ 
			int j = 0;
			for (j = 0; j < sMessageBox[i].Line; j++) {
				dsDrawTexImage(MatBoxCenter, sMessageBox[i].x + 32 + (j * 16), sMessageBox[i].y, 16, 43, 255);//∞°øÓµ•
			}
			dsDrawTexImage(MatBoxRight, sMessageBox[i].x + 32 + (j * 16), sMessageBox[i].y, 32, 43, 255);  //ªÛ¥‹
...
void cMESSAGEBOX::Main()
{
	...
	MessageArrowPosi = 0;      //π⁄Ω∫¿« »≠ªÏ«• ∆˜¡ˆº« 
	MessageButtonPosi = 0;     //πˆ∆∞ ∆˜¡ˆº« 

	int i = 0;
	for (i = 0; i < 100; i++) {
		if (sMessageBox[i].Flag) {
			MessageTimeCount++;
			if (i == MESSAGE_MOVE_TRADE_ITEM) {	//ªÁ±‚πÊ¡ˆ ∏ﬁºº¡ˆ¥¬ ∞°øÓµ• ∫∏ø©¡ÿ¥Ÿ 
				if (MessageTimeCount > 70 * 5) { //3√ »ƒ ∏ﬁºº¡ˆ∏¶ ¡ˆøˆ¡ÿ¥Ÿ 
					sMessageBox[i].Flag = 0;
					MessageTimeCount = 0;    //√ ±‚»≠ 
				}
			}
			else {
				/////// ¿Ã∫•∆Æ ¡°ºˆ Ω∫ƒ⁄æÓ∏∏ Ω√∞£¿ª ±Ê∞‘«ÿ¡ÿ¥Ÿ (Ω∫∆‰º» ≥Øƒ°±‚ ∂´ªß)
				if (i == MESSAGE_SCORE) {
					if (MessageTimeCount > 70 * 6) { //3√ »ƒ ∏ﬁºº¡ˆ∏¶ ¡ˆøˆ¡ÿ¥Ÿ 
						sMessageBox[i].Flag = 0;
						MessageTimeCount = 0;    //√ ±‚»≠ 
					}
					...
					if (MessageTimeCount > 70 * 3) { //3√ »ƒ ∏ﬁºº¡ˆ∏¶ ¡ˆøˆ¡ÿ¥Ÿ 
						sMessageBox[i].Flag = 0;
						MessageTimeCount = 0;    //√ ±‚»≠ 
					}
				}
	...
	if (sStarShop.Flag) {
		...
		if (!sinMessageBoxShowFlag) { //∏ﬁºº¡ˆ π⁄Ω∫∞° ¡ﬂ∫π¿∏∑Œ ∂ﬂ¥¬∞… ∏∑¥¬¥Ÿ
			if (pCursorPos.x > BackStartPos.x + BUTTON2_X + 35 && pCursorPos.x < BackStartPos.x + BUTTON2_X + 35 + 32 &&
...
void cMESSAGEBOX::LButtonDown(int x, int y)
{
	...
	//æ∆¿Ã≈€ ≈√πËº≠∫ÒΩ∫
	if (sMessageBox_RecvItem.Flag) {
		if (sMessageBox_RecvItem.ButtonYes) { //æ∆¿Ã≈€ ¿Œ¡ı º≠πˆ∑Œ ƒ⁄µÂ∫∏≥ø 
			if (cInvenTory.CheckInvenEmpty(&sMessageBox_RecvItem.RecvItem)) {
				if (sMessageBox_RecvItem.Param[0]) {
					...
					if (sMessageBox_RecvItem.RecvItem.CODE == (sinGG1 | sin01)) { //µ∑¿œ ∞ÊøÏ¥¬ µ˚∑Œ √≥∏Æ«ÿ¡ÿ¥Ÿ
			...
			if (MessageButtonPosi == 2) { //√Îº“
				...
				if (i == MESSAGE_CHANGE_JOB4_2) {
					if (sinChar->JOB_CODE <= 4) {
						WarpField2(START_FIELD_NUM); //µπæ∆∞£¥Ÿ
					}
					else
					{
						WarpField2(START_FIELD_MORYON); //µπæ∆∞£¥Ÿ
					}
				}
				...
				if (i == MESSAGE_SECRET_NUM) {
					...
					sinChatEnter = 0; //¿”¿«∑Œ ±€ææ∞° æ»∫∏¿Ã∞‘«—¥Ÿ
				}
				//100LVƒ˘Ω∫∆Æ
				if (i == MESSAGE_QUEST_100LVITEMOK) {
					chaQuest.iFuryQuestIndex[1] = 0;
				}
				// dell item premium xxstr
	...
	for (i = 0; i < 20; i++) {
		if (sMessageBox2[i].Flag) {
			if (MessageArrowPosi == 1) {
				...
				if (i == 3 || i == MESSAGE_MOVE_MONEY_TRADE || i == MESSAGE_MOVE_MONEY_MOVE || i == MESSAGE_GIVE_MONEY) {//µ∑¿Ã∏È 
				...
				{
					if (NewShop::GetInstance()->GetCoin() < sMessageBox2[i].Count)
					{
						sMessageBox2[i].Count = NewShop::GetInstance()->GetCoin();
					}
				}
				...
				if (i == MESSAGE_MOVE_MONEY_MOVE) { //√¢∞Ìø° µ∑¿ª ø≈±Ê∂ß 
					if (sinChar->Money < sMessageBox2[i].Count) {
						sMessageBox2[i].Count = sinChar->Money;
					}

					if ((sWareHouse.Money - 2023) + sMessageBox2[i].Count > MAX_GOLD_TIER4) {
						sMessageBox2[i].Count -= ((sWareHouse.Money - 2023) + sMessageBox2[i].Count) - MAX_GOLD_TIER4;
					}
				...
				if (i == MESSAGE_MOVE_MONEY_RECV) {
					...
					if (sinChar->Level <= 10) {
						if (sinChar->Money + sMessageBox2[i].Count > MAX_GOLD_LOW_LEVEL) {
							sMessageBox2[i].Count = MAX_GOLD_LOW_LEVEL - sinChar->Money;
						...
						case 0:
							if ((sinChar->Money + sMessageBox2[i].Count) > (sinChar->Level * MAX_GOLD_LOW_LEVEL) - 1800000) {
								sMessageBox2[i].Count = (sinChar->Level * MAX_GOLD_LOW_LEVEL) - 1800000 - sinChar->Money;
								if (sMessageBox2[i].Count < 0) sMessageBox2[i].Count = 0;
						...
						case 1:
							if ((sinChar->Money + sMessageBox2[i].Count) > MAX_GOLD_TIER2) {
								sMessageBox2[i].Count = MAX_GOLD_TIER2 - sinChar->Money;
								if (sMessageBox2[i].Count < 0) sMessageBox2[i].Count = 0;
							}
							break;
						case 2:
							if ((sinChar->Money + sMessageBox2[i].Count) > MAX_GOLD_TIER3) {
								sMessageBox2[i].Count = MAX_GOLD_TIER3 - sinChar->Money;
								if (sMessageBox2[i].Count < 0) sMessageBox2[i].Count = 0;
							}
							break;
						case 3:
							if ((sinChar->Money + sMessageBox2[i].Count) > MAX_GOLD_TIER4) {
								sMessageBox2[i].Count = MAX_GOLD_TIER4 - sinChar->Money;
								if (sMessageBox2[i].Count < 0) sMessageBox2[i].Count = 0;
							}
				...
				if (i == 0) {//æ∆¿Ã≈€ ªÁ±‚¿Ã∏È 
					...
					if (CountTemp > sinChar->Money) {
						ShowMessage(MESSAGE_NOT_ENOUGH_MONEY); //∏ﬁºº¡ˆ∏¶ ∫∏ø©¡ÿ¥Ÿ 
					...
					if (sinChar->Weight[1] < CountTemp) { //π´∞‘∏¶ ≥—æÓº≠∏È
						ShowMessage(MESSAGE_OVER_WEIGHT); //∏ﬁºº¡ˆ∏¶ ∫∏ø©¡ÿ¥Ÿ 
			...
			for (int j = 0; j < 10; j++) {
				if (sinGetKeyClick(szKeyBuff[j]) || sinGetKeyClick(szKeyBuffPad[j])) {
					...
					if (i == MESSAGE_MOVE_MONEY_MOVE)
					{
						...
						if ((sWareHouse.Money - 2023) + sMessageBox2[i].Count > MAX_GOLD_WHAREHOUSE) {
							sMessageBox2[i].Count -= ((sWareHouse.Money - 2023) + sMessageBox2[i].Count) - MAX_GOLD_WHAREHOUSE;
						}
...
int cMESSAGEBOX::ShowMessageEvent(char* Msg)
{
	...
	sMessageBox[49].x = (smConfig.ScreenSize.x >> 1) - ((sMessageBox[49].len * 10) / 2) + 40;
	sMessageBox[49].y = 5;
...
{
	TitleBox::GetInstance()->SetText(sMessageBox[Kind].MessageDoc, 3);
	/*for (int i = 0; i < 100; i++) {
		if (sMessageBox[i].Flag)
			sMessageBox[i].Flag = 0;
	}
...
int cMESSAGEBOX::ShowMessagexD(int value)
{
	char message[32]; // Define um buffer para armazenar o valor convertido para string
	sprintf_s(message, sizeof(message), "%d", value); // Converte o valor inteiro para string
...
int cMESSAGEBOX::ShowMessage2(int Kind)
{
	...
	if (Kind == MESSAGE_MOVE_COIN_TRADE)		//Se clicar no Botao AddCoin do Trade
	{
		if (sTrade.Coin)						//Se ja tiver Coin na janela do Trade
		{
			if (!CheckTrade((void*)&sTrade))	//Check Info do Trade
				cTrade.CancelTradeItem();		//Cancela o Trade
			CheckCharForm();					//Check Info do Current Player
			TempShowCoin = 0;					//Zera o valor do Coin que mostra na janela
			NewShop::GetInstance()->PlusCoin(sTrade.Coin);//Devolve o Coin que tava na Janela, pro Current Player
			sTrade.Coin = 0;					//Zera o valor da struct que ?enviada pro outro Player (Atualizar Info da Janela)
			ReformCharForm();					//Atualiza Info pra checagem do Current Player
			ReformTrade((void*)&sTrade);		//Atualiza Info pra checagem do Trade
			SendTradeItem(cTrade.TradeCharCode);//Envia as Info pro outro Player (Atualizar Info da Janela)
		}
	}
...
int cMESSAGEBOX::ShowMessage3(int Kind, char* Doc)
{
	...
	if ((Kind == MESSAGE_TELEPORT_MAR) || (Kind == MESSAGE_TELEPORT_ARENA) || (Kind == MESSAGE_TELEPORT_BA) || (Kind == MESSAGE_NEWTELEPORT))
		lstrcpy(sMessageBox3[Kind].MessageDoc1, "Deseja ser teleportado?");

	if (Kind == MESSAGE_TRADE_BPEXP)
		lstrcpy(sMessageBox3[Kind].MessageDoc1, "Deseja confirmar a troca?");

	if (Kind == MESSAGE_DAILYQUEST)
		lstrcpy(sMessageBox3[Kind].MessageDoc1, "Deseja aceitar o desafio?");
	...
	if (Kind == MESSAGE_TELEPORT_MAR)
	{
		sMessageBox3[Kind].x += 120;
		sMessageBox3[Kind].y += 100;
	}
	if (Kind == MESSAGE_TELEPORT_BA)
	{
		sMessageBox3[Kind].x += 120;
		sMessageBox3[Kind].y += 100;
	}

	// msg confirmation box delete item premium xxstr
...
int StageEragonLair = 0;
int TimeEragonLair = 0;
extern BOOL bQuestWindow;

int iDailyQuestCode = 0;
...
void SaveDailyQuest()
{
	...
	Pck p;
	p.code = 0x43550004;
	p.questid = sinQuest_Daily.CODE;
	p.state = sinQuest_Daily.State;
	p.value1 = sinQuest_Daily.Monster[0];
	p.size = sizeof(Pck);

	if (smWsockDataServer)
		smWsockDataServer->Send((char*)&p, p.size, TRUE);
}

//∏ﬁºº¡ˆ π⁄Ω∫¿« ≥ªøÎ¿ª ∫∏ø©¡ÿ¥Ÿ 
void cMESSAGEBOX::DrawMessageText()
{
	...
	//lstrcpy(sMessageBox_RecvItem.szDoc,"π⁄ªÛø≠∏∏ºº ∏∏ºº ∏∏∏∏∏∏∏∏1∏∏∏∏1∏∏∏∏∏∏∏∏∏∏∏∏∏∏∏∏∏∏ºº  ioi ");
	//æ∆¿Ã≈€ ≈√πËº≠∫ÒΩ∫  ≥–¿Ã 256 
	//sinBoldFont
	...
	if (sinMessageBoxShowFlag || TempFlag) {
		...
		for (i = 0; i < 100; i++) {
			if (sMessageBox[i].Flag) { //≥Øƒ°±‚∑Œ ∏¬√Ë¥Ÿ 
				if (i == MESSAGE_100LVITEM_OK) {
					//¿Ã∫•∆Æ ∏ﬁºº¡ˆø° æ∆¿Ã≈€ ¿Ã∏ß¿ª ≥÷±‚¿ß«— ªÁøÎ
		...
		for (i = 0; i < 20; i++) {
			if (sMessageBox2[i].Flag) { //≥Øƒ°±‚∑Œ ∏¬√Ë¥Ÿ 
				...
				if (i == MESSAGE_MYSHOP_POTION) { //∆˜º«ªÁ±‚¿œ∞ÊøÏø°¥¬ ∞°∞›¿ª «•Ω√«ÿ¡ÿ¥Ÿ            
	...
	//				dsTextLineOut(hdc,sMessageBox2[i].TextSetX,sMessageBox2[i].y+72,szBuff15,lstrlen(szBuff15) );	
		...
		for (i = 0; i < MESSAGE3_MAX; i++) {
			if (sMessageBox3[i].Flag) {
				...
				if (i == MESSAGE_EVENT_SMASHTV) { //ø’µ˚Ω√ ≥Øƒ°±‚ §—,.§—
...
int iNewTeleportIndex = 0;

///∏ﬁºº¡ˆ∞° ª¿œµ… ∏ﬁºº¡ˆ π⁄Ω∫ 
int cMESSAGEBOX::CheckOkMessageBox2(int Kind)
{
	...
	case MESSAGE_MASTER_SKILL:
		if (pMasterSkill)
		{
			...
			// Atualiza a skill quando sobe o level
			GAMECOREHANDLE->pcSkillInfoBox->SetSkillInfoLevelOffset(0);
			...
			case SKILL_POISON_ATTRIBUTE:	//µ∂º”º∫ ¡ı∞° 
				pUseSkill = pMasterSkill;
				pUseSkill->Skill_Info.FuncPointer();
				break;
			case SKILL_MELEE_MASTERY:       //µ•πÃ¡ˆ ¡ı∞° 
				pUseSkill = pMasterSkill;
				pUseSkill->Skill_Info.FuncPointer();
				break;
			case SKILL_FIRE_ATTRIBUTE:      //∫“º”º∫ ¡ı∞° 
				pUseSkill = pMasterSkill;
				pUseSkill->Skill_Info.FuncPointer();
				break;
			case SKILL_ICE_ATTRIBUTE:       //æÛ¿Ω º”º∫ ¡ı∞° 
				pUseSkill = pMasterSkill;
				pUseSkill->Skill_Info.FuncPointer();
				break;
			case SKILL_SHOOTING_MASTERY:    //»∞ µ•πÃ¡ˆ ¡ı∞°  
				pUseSkill = pMasterSkill;
				pUseSkill->Skill_Info.FuncPointer();
				break;
			case SKILL_MAXIMIZE:			//√÷¥Î µ•πÃ¡ˆ ¡ı∞° 
				pUseSkill = pMasterSkill;
				pUseSkill->Skill_Info.FuncPointer();
				break;
			case SKILL_WEAPONE_DEFENCE_MASTERY:	//∫Ì∑∞¿≤ ¡ı∞° 
				pUseSkill = pMasterSkill;
				pUseSkill->Skill_Info.FuncPointer();
				break;
			case SKILL_DIONS_EYE:	//∏Ì¡ﬂ∑¸ ¡ı∞°  
				pUseSkill = pMasterSkill;
				pUseSkill->Skill_Info.FuncPointer();
				break;
			case SKILL_PHYSICAL_TRANING: //±Ÿ∑¬∑Æ √ﬂ∞° 
			...
			cInvenTory.CheckDamage(); //µ•πÃ¡ˆ∏¶ √º≈©«ÿº≠ º≠πˆ∑Œ∫∏≥Ω¥Ÿ
			LearnSkillEffect(0);
			TempTalent = (int)(sinChar->Talent / 3);   ////////º˜∑√µµ∏¶ ∫∏¡§«—¥Ÿ 
	...
	case MESSAGE_CHANGE_JOB:
		...
		///////////////// ¿Ã∞˜ø° ƒ˘Ω∫∆Æ ºˆ«‡ ∏ﬁºº¡ˆ∞° ≥™ø¬¥Ÿ 
		sinChar->ChangeJob++; //¿¸æ˜¿ª «—¥Ÿ 
		CSKILL->OpenFlag = 1;
		cInterFace.CheckAllBox(SIN_SKILL); //√¢¿ª «œ≥™∏∏ ∂ÁøÓ¥Ÿ 
		SkillMasterFlag = 1;  //Ω∫≈≥¿ª «“¥Á«“ºˆ¿÷∞‘ «ÿ¡ÿ¥Ÿ

		if (sinQuest_ChangeJob.CODE) { //≈€Ω∫≈©∑–
			DeleteQuestItem((sinQT1 | sin01)); //ƒ˘Ω∫∆Æ æ∆¿Ã≈€¿ª ¡ˆøˆ¡ÿ¥Ÿ
			...
			sinQuest_ChangeJob.CODE = 0; //ƒ˘Ω∫∆Æ∞° øœ∑·µ«∏È ƒ⁄µÂ∞° ªÁ∂Û¡¯¥Ÿ 
			sinChar->wVersion[1] = 0;
			EndQuest_Code(sinQuest_ChangeJob.BackUpCode);

			cInvenTory.ReFormInvenItem();
			sinSkillPontInitButtonFlag = 1; //Ω∫≈≥ ∆˜¿Ã∆Æ √ ±‚»≠ π⁄Ω∫∏¶ ∂ÁøÓ¥Ÿ 
		}
		if (FindLastQuestCode(SIN_QUEST_CODE_CHANGEJOB2_NPC_M) || FindLastQuestCode(SIN_QUEST_CODE_CHANGEJOB2_NPC_D)) { //∏∂Û¿Ãø¬
			sinJobList = GetJobDataCode(sinChar->JOB_CODE, sinChar->ChangeJob);
			lstrcpy(cInvenTory.JobName, sinJobList->szName2);
			sinSkillPontInitButtonFlag = 1; //Ω∫≈≥ ∆˜¿Ã∆Æ √ ±‚»≠ π⁄Ω∫∏¶ ∂ÁøÓ¥Ÿ 
			//¿Ã∫Œ∫–ø°º≠ ƒ˘Ω∫∆Æƒ⁄µÂ∞¸∑√ √ ±‚»≠∏¶ «ÿ¡ÿ
		...
		if (sinQuest_ChangeJob3.CODE) {
			...
			//sinQuest_ChangeJob3.CODE = 0; //ƒ˘Ω∫∆Æ∞° øœ∑·µ«∏È ƒ⁄µÂ∞° ªÁ∂Û¡¯¥Ÿ 
		...
		SaveGameData(); //¿¸æ˜»ƒ ¿˙¿Â«—¥Ÿ
		ChangeJobFace();
		break;
	case MESSAGE_QUEST:	 //ƒ˘Ω∫∆Æ ºˆ«‡∏ﬁºº¡ˆ «•Ω√ 
	...
	case MESSAGE_QUEST_CHANGEJOB_MORAYOIN:
		...
		// sinQuest_ChangeJob2.Kind = GetRandomPos(1,2); //¿¸æ˜ƒ˘Ω∫∆Æ∏¶ A,B∑Œ º¬∆√«—¥Ÿ
		//sinQuest_ChangeJob2.Kind = 2;
		SaveGameData();
		StartQuest_Code(sinQuest_ChangeJob2.CODE); //º¬∆√µ«¥¬ ∫Œ∫–¿∫ ≥™¡ﬂø° ¥ŸΩ√∫¡æﬂ«—¥Ÿ 
		switch (sinQuest_ChangeJob2.Kind) {
		case 1:
			sinQuest_ChangeJob2.CODE = SIN_QUEST_CODE_CHANGEJOB2_NPC_M; //∏Ù∏Æ «“∏”¥œ π˙≤‹ƒ˘Ω∫∆Æ
			break;
		case 2:
			sinQuest_ChangeJob2.CODE = SIN_QUEST_CODE_CHANGEJOB2_NPC_D; //∏Ù∏Æ «“∏”¥œ π˙≤‹ƒ˘Ω∫∆Æ
			break;
		}
		SaveGameData();
		StartQuest_Code(sinQuest_ChangeJob2.CODE); //º¬∆√µ«¥¬ ∫Œ∫–¿∫ ≥™¡ﬂø° ¥ŸΩ√∫¡æﬂ«—¥Ÿ 
		//ƒ˘Ω∫∆Ææ∆¿Ã≈€¿Ã ¿÷¥¬¡ˆ∏¶ √º≈©«ÿº≠ ≥Ø∑¡πˆ∏∞¥Ÿ 
		cInvenTory.ClearQuestItem();
		break;
	case MESSAGE_QUEST_CHANGEJOB3: //3¬˜¿¸¡˜ ƒ˘Ω∫∆Æ 
		sinQuest_ChangeJob3.CODE = SIN_QUEST_CODE_CHANGEJOB3;
		sinQuest_ChangeJob3.State = 1;
		SaveGameData();
		StartQuest_Code(sinQuest_ChangeJob3.CODE); //º¬∆√µ«¥¬ ∫Œ∫–¿∫ ≥™¡ﬂø° ¥ŸΩ√∫¡æﬂ«—¥Ÿ 
		break;

	case MESSAGE_INIT_SKILL_POINT: //Ω∫≈≥ ∆˜¿Œ∆Æ √ ±‚»≠ 
		CSKILL->InitSkillPoint();
		break;
	case MESSAGE_EVENT_GYUNGFUM: //¥…∑¬ƒ° √ ±‚»≠∑Œ ªÁøÎ«—¥Ÿ 
		...
		//sinSendEventItem(1); //¿Ã∫•∆Æ æ∆¿Ã≈€¿ª º≠πˆ∑Œ∫∏≥ª∞Ì ªË¡¶«—¥Ÿ 
		break;

	case MESSAGE_EVENT_SMASHTV: //¿Ã∫•∆Æ Ω∫∏ﬁΩ¨ TV 
	...
	case MESSAGE_POSTBOX:
		...
		else PostBoxIndexCnt =0; //0¿∏∑Œ «√∑¢¿ª √ ±‚»≠«ÿº≠ √º≈©«—¥Ÿ
		*/


		break;
	case MESSAGE_LINKCORE:
		//cInvenTory.SetLinkCore(); //∏µ≈©ƒ⁄æÓ∏¶ º¬∆√«—¥Ÿ
		cInvenTory.UnionCoreParticle2(UnionCorePosi.x, UnionCorePosi.y); //¿Ã∆Â∆Æ 
		sinPlaySound(SIN_SOUND_EAT_POTION2); //¿Ø¥œø¬ƒ⁄æÓ ªÁøÓµÂ
		SendLinkCoreToServer(&TempLinkCoreItem);

		break;
	case MESSAGE_RECVITEM:			//æ∆¿Ã≈€¿ª πﬁ¿ª∂ß »Æ¿Œ√¢
		SendWingQuestItem();
		sinbaram_Stop = 1;
		//WingItemQuestIndex
		//º≠πˆø° æ∆¿Ã≈€¿ª ∫∏≥Ω¥Ÿ 
	...
	case MESSAGE_WARP:
		if (sSinWarpGate.GateUseIndex > 2 && sSinWarpGate.MoveFieldNum > 1 && !sSinWarpGate.SameAreaFlag) {
			CheckCharForm(); //¿Œ¡ı 
			//<ha>∞¯º∫¿¸ ºº¿≤¿˚øÎ«— øˆ«¡∞‘¿Ã∆Æ ∏”¥œ∏¶ º≠πˆ∑Œ ∫∏≥Ω¥Ÿ.
			...
			sinMinusMoney(WarpGateUseCost[sSinWarpGate.GateUseIndex - 4], 1); //µ∑¿ªª´¥Ÿ
			ReformCharForm(); //¿Á¿Œ¡ı 
		}
	...
	case MESSAGE_SELL_HIGHRANK_ITEM:
		MouseItem.Flag = 1; //ªÏ¬•ƒÙ ∂´ªß øÚ»≠»≠»±
		ResetInvenItemCode(); //∏∂øÏΩ∫ø° «√∑∫¿ª ≥÷¿ª∂ß √º≈©«ÿ¡ÿ¥Ÿ
	...
	case MESSAGE_MYSHOP_ITEM_BUY:
		if (cCharShop.OpenFlag) {
			Send_PersonalShopItem(sRecvMyShop.CHAR_CODE, &sMyShop_Server); //º≠πˆ∑Œ ±∏∏≈«“ æ∆¿Ã≈€¿« ¡§∫∏∏¶ ∫∏≥Ω¥Ÿ
		}
		break;
	...
	case MESSAGE_QUEST_LEVEL30:
		...
		sinQuest_Level30.Monster[1] = 20; //πŸ∞Ô 20∏∂∏Æ
		SaveGameData();
	...
	case MESSAGE_QUEST_LEVEL55:
		...
		sinQuest_Level55.Monster[1] = 20; //∏”«… 20∏∂∏Æ
		SaveGameData();
	...
	case MESSAGE_QUEST_LEVEL55_2:
		...
		sinQuest_Level55_2.Monster[1] = 20; //¥Ÿ¿ÃæÓ∫Ò 20∏∂∏Æ
		SaveGameData();
	...
	case MESSAGE_QUEST_LEVEL70:
		...
		sinQuest_Level70.Monster[1] = 10; //≈∏¿Ã«¨ 10∏∂∏Æ
		SaveGameData();
	...
	case MESSAGE_QUEST_LEVEL80:
		...
		sinQuest_Level80.Monster[1] = 5; //∂Û≈ı 5∏∂∏Æ
		SaveGameData();
	...
	case MESSAGE_QUEST_LEVEL85:
		...
		sinQuest_Level85.State = 2; //ªÛ≈¬∞° 1¿Ã∏È ªÁ∏¡ 
		sinQuest_Level85.Monster[1] = 10; //±◊∑Œ≈◊Ω∫≈© 10∏∂∏Æ
		SaveGameData();
	...
	case MESSAGE_QUEST_LEVEL90:
		...
		sinQuest_Level90.Monster[1] = 10; //±◊∑Œ≈◊Ω∫≈© 10∏∂∏Æ
		SaveGameData();
	...
	case MESSAGE_TELEPORT:
		...
		sinMinusMoney(TeleportUseCose[sinTeleportMoneyIndex - 1], 1); //µ∑¿ªª´¥Ÿ
		WarpField2(sinTeleportIndexArray[sinTeleportMoneyIndex - 1]);
		sinPlaySound(SIN_SOUND_COIN);
		ReformCharForm(); //¿Á¿Œ¡ı 
		break;
	case MESSAGE_CHANGE_JOB4: //ƒ˘Ω∫∆Æ Ω√¿€
		sinQuest_ChangeJob4.CODE = SIN_QUEST_CODE_CHANGEJOB4;
		sinQuest_ChangeJob4.State = 1;
		StartQuest_Code(sinQuest_ChangeJob4.CODE);
		sinChangeJob4MonSet(&sinQuest_ChangeJob4); //∏ÛΩ∫≈Õ∏¶ º¬∆√«—¥Ÿ
		SaveGameData();
		break;
	case MESSAGE_CHANGE_JOB4_2: //ƒ˘Ω∫∆Æ øœ∑·
		///////////////// ¿Ã∞˜ø° ƒ˘Ω∫∆Æ ºˆ«‡ ∏ﬁºº¡ˆ∞° ≥™ø¬¥Ÿ 
		sinChar->ChangeJob++; //¿¸æ˜¿ª «—¥Ÿ 
		CSKILL->OpenFlag = 1;
		cInterFace.CheckAllBox(SIN_SKILL); //√¢¿ª «œ≥™∏∏ ∂ÁøÓ¥Ÿ 
		...
		sinQuest_ChangeJob4.CODE = 0; //ƒ˘Ω∫∆Æ∞° øœ∑·µ«∏È ƒ⁄µÂ∞° ªÁ∂Û¡¯¥Ÿ 
		sinChar->wVersion[1] = 0;
		EndQuest_Code(sinQuest_ChangeJob4.BackUpCode);

		cInvenTory.ReFormInvenItem();
		sinSkillPontInitButtonFlag = 1; //Ω∫≈≥ ∆˜¿Œ∆Æ √ ±‚»≠ π⁄Ω∫∏¶ ∂ÁøÓ¥Ÿ 
		SaveGameData(); //¿¸æ˜»ƒ ¿˙¿Â«—¥Ÿ
		...
		break;
	case MESSAGE_CRISTALEVENTITEM_SEND: //¿œ∞ˆ∞°¡ˆ ≈©∏ÆΩ∫≈ª¿« ∫∏¿∫
		if (cInvenTory.SendCristalEventItem()) {
			SendWingQuestItem();
		}
		break;
	case MESSAGE_WATERMELON_EVENTITEM: // π⁄¿Áø¯ - ºˆπ⁄ ∏æ∆ø¿±‚
		if (cInvenTory.SendWatermelonEventItem()) {
			SendWingQuestItem();
		}
		break;
	case MESSAGE_VALENTINE_EVENTITEM: // ¿Â∫∞ - πﬂ∑ª≈∏¿Œ √ ƒ›∏¥ ∏æ∆ø¿±‚
		if (cInvenTory.SendValentineEventItem())
		{
			SendWingQuestItem();
		}
		break;
	case MESSAGE_PRISTON_ALPHABET_EVENTITEM: // π⁄¿Áø¯ - æÀ∆ƒ∫™ ¡∂«’ ¿Ã∫•∆Æ
		if (cInvenTory.SendPristonAlphabetEventItem()) {
			//SendWingQuestItem(); // fixed event PRISTON
			SendWingEventPRISTONItem();
		}
		break;

	case MESSAGE_CANDYDAYS_EVENTITEM: // ¿Â∫∞ - ƒµµµ•¿Ã¡Ó
	...
	case MESSAGE_MAGICALGREEN_EVENTITEM: // ¿Â∫∞ - ∏≈¡ˆƒ√±◊∏∞
	...
	case MESSAGE_MAGICALGREEN_EVENTITEM_EMERALD: // ¿Â∫∞ - ∏≈¡ˆƒ√±◊∏∞ ø°∏ﬁ∂ˆµÂ
	...
	case MESSAGE_TEAROFKARA_EVENTITEM: // ¿Â∫∞ - ƒ´∂Û¿« ¥´π∞
	...
	case MESSAGE_FIND_EVENTITEM: // ¿Â∫∞ - ¡∂ªÁø¯¿ª √£æ∆∂Û - ∏∂∑¬ ø¨±∏¿⁄
	...
	case MESSAGE_FINDKAMIU_EVENTITEM: // ¿Â∫∞ - ¡∂ªÁø¯¿ª √£æ∆∂Û - ƒ´πÃ¿Ø
	...
	case MESSAGE_FINDETO_EVENTITEM:	// ¿Â∫∞ - ¡∂ªÁø¯¿ª √£æ∆∂Û - ø°≈‰
		if (cInvenTory.SendFindinvestigatorTaleEtoEventItem())
		{
			SendWingQuestItem();
		}
		break;
		// confim teleport BC xxstr
	case MESSAGE_CASTLE_TELEPORT:      //∫Ì∑πΩ∫ ƒ≥ΩΩ ≈⁄∑π∆˜∆Æ
		if (lpCurPlayer->smCharInfo.Level < 50) // level minimu para teleport
			break;

		CheckCharForm(); //¿Œ¡ı 

		WarpField2(33);
		sinMinusMoney(haC_TeleportMoney); //µ∑¿ªª´¥Ÿ
		sinPlaySound(SIN_SOUND_COIN);
		ReformCharForm(); //¿Á¿Œ¡ı 
		break;
	case MESSAGE_CASTLE_BUYTOWER:       //∫Ì∑πΩ∫ ƒ≥ΩΩ ≈©∏ÆΩ∫≈ª ≈∏øˆ∏¶ ªÍ¥Ÿ.
		...
		chaSiege.SendServerSiegeData(); //¥ŸΩ√ ¿˙¿Â«ÿ¡ÿ¥Ÿ.
	//	ReformCharForm();//¿Á¿Œ¡ı
	//	SendSaveMoney();
		sinPlaySound(SIN_SOUND_COIN);
	...
	case MESSAGE_QUEST_100LVITEMOK:
	{
		...
		if (cInvenTory.CheckSetEmptyArea(&TempItem)) {
			...
			haElementaryQuestLog |= QUESTBIT_FURYOFPHANTOM; //ƒ⁄µÂ∏¶ º¬∆√«ÿ¡ÿ¥Ÿ
	...
	break;
	case MESSAGE_QUEST_100LVBATTLEOK:   //¥Î∑√¿Â¿∏∑Œ ¿Ãµø 
		...
		SetQuestBoard();
		break;
	case MESSAGE_TELEPORT_CORE:        //≈⁄∑π∆˜∆Æ ƒ⁄æÓ
		...
		/*
				case MESSAGE_STARDUST_SEND:
					if(cInvenTory.SendStarDustItem()){		//∫˚≥™¥¬ ∞°∑Á∏¶ ∫∏≥ª¡ÿ¥Ÿ //≈©∏ÆΩ∫∏∂Ω∫
						SendWingQuestItem();
					}
				break;
		*/
	case MESSAGE_FALLGAME:   //
		WarpField2(39);
		break;
	case MESSAGE_MANUFACTURE:		// pluto ¡¶¿€ æ∆¿Ã≈€ ¡æ∑˘ ∏ﬁºº¡ˆ
		sinCraftItemStartFlag = 1;
		SetInterEffectManufacture();
		MixCancelButtonDelayFlag = 1; // pluto ø©±‚¥Ÿ æµ∂Û∞Ì ø‹∫Œ ∫Øºˆ∑Œ πŸ≤Â¥Ÿ -_-
		break;
	case MESSAGE_TELEPORT_DUNGEON:		// pluto √ﬂ∞°¥¯¿¸¿∏∑Œ øˆ«¡«—¥Ÿ.
		cSinHelp.NPCTeleportFlag = 0;
		WarpField2(40);
		break;
	case MESSAGE_CONFIRMATION_BOX: // msg premium del xxstr
		_ConfirmationBox.CheckFlag(true);
		break;
	case MODOPVP:
		WarMode.ModoPVP();
		break;
	case MESSAGE_NEWTELEPORT:

		if (iNewTeleportIndex == 0)
		{
			if (lpCurPlayer->smCharInfo.Level >= 90 && lpCurPlayer->smCharInfo.Money >= 50000)
			{
				WarpField2(51);
				sinMinusMoney(50000);
			}
			else
				cMessageBox.ShowMessageEvent("You have insufficient gold or an incorrect level.");
		}
		else if (iNewTeleportIndex == 1)
		{
			if (lpCurPlayer->smCharInfo.Level >= 100 && lpCurPlayer->smCharInfo.Money >= 100000)
			{
				WarpField2(50);
				sinMinusMoney(100000);
			}
			else
				cMessageBox.ShowMessageEvent("You have insufficient gold or an incorrect level.");
		}
		else if (iNewTeleportIndex == 2)
		{
			if (lpCurPlayer->smCharInfo.Level >= 105 && lpCurPlayer->smCharInfo.Money >= 150000)
			{
				WarpField2(49);
				sinMinusMoney(150000);
			}
			else
				cMessageBox.ShowMessageEvent("You have insufficient gold or an incorrect level.");
		}
		else if (iNewTeleportIndex == 3)
		{
			if (lpCurPlayer->smCharInfo.Level >= 110 && lpCurPlayer->smCharInfo.Money >= 200000)
			{
				WarpField2(56);
				sinMinusMoney(200000);
			}
			else
				cMessageBox.ShowMessageEvent("You have insufficient gold or an incorrect level.");
		}
		else if (iNewTeleportIndex == 4)
		{
			if (lpCurPlayer->smCharInfo.Level >= 115 && lpCurPlayer->smCharInfo.Money >= 250000)
			{
				WarpField2(57);
				sinMinusMoney(250000);
			}
			else
				cMessageBox.ShowMessageEvent("You have insufficient gold or an incorrect level.");
		}
		else if (iNewTeleportIndex == 5)
		{
			if (lpCurPlayer->smCharInfo.Level >= 120 && lpCurPlayer->smCharInfo.Money >= 350000)
			{
				WarpField2(53);
				sinMinusMoney(350000);
			}
			else
				cMessageBox.ShowMessageEvent("You have insufficient gold or an incorrect level.");
		}
		else if (iNewTeleportIndex == 6)
		{
			if (lpCurPlayer->smCharInfo.Level >= 125 && lpCurPlayer->smCharInfo.Money >= 500000)
			{
				WarpField2(54);
		...
		else if (iNewTeleportIndex == 7)
		{
			if (lpCurPlayer->smCharInfo.Level >= 130 && lpCurPlayer->smCharInfo.Money >= 700000)
			{
				WarpField2(55);
				sinMinusMoney(700000);
			}
			else
				cMessageBox.ShowMessageEvent("You have insufficient gold or an incorrect level.");
		}
		...
		break;
	case MESSAGE_TELEPORT_MAR:		// pluto √ﬂ∞°¥¯¿¸¿∏∑Œ øˆ«¡«—¥Ÿ.
		cSinHelp.NPCTeleportFlag = 0;

		if (lpCurPlayer->smCharInfo.Level >= 140)
			WarpField2(45);
		else
			cMessageBox.ShowMessageEvent("Seu level precisa ser maior que 140.");

		break;
	case MESSAGE_TELEPORT_BA:		// pluto √ﬂ∞°¥¯¿¸¿∏∑Œ øˆ«¡«—¥Ÿ.
		cSinHelp.NPCTeleportFlag = 0;
		...
		if (EventoArena::GetInstance()->stageArena == 1) {
			...
			if (random <= 30) {
				WarpFieldNearPos(58, -23793 * fONE, 20220 * fONE, &x, &z);
				lpCurPlayer->SetPosi(-23793 * fONE, 0, 20220 * fONE, 0, 0, 0);
			}
			else if (random > 30 && random <= 60) {
				WarpFieldNearPos(58, -23809 * fONE, 17660 * fONE, &x, &z);
				lpCurPlayer->SetPosi(-23809 * fONE, 0, 17660 * fONE, 0, 0, 0);
			}
			else {
				WarpFieldNearPos(58, -23871 * fONE, 21012 * fONE, &x, &z);
				lpCurPlayer->SetPosi(-23871 * fONE, 0, 21012 * fONE, 0, 0, 0);
			}
		...
		//	WarpField2( 58 );
			...
			cMessageBox.ShowMessageEvent("A arena ainda n„o comeÁou!");
	...
	case MESSAGE_TRADE_BPEXP:
	{
		if (lpCurPlayer->sBlessCastle_Damage[1] >= 50)
		{
			...
			if ((lpCurPlayer->smCharInfo.Level >= 80) && (lpCurPlayer->smCharInfo.Level < 100))
			{
				...
				cMessageBox.ShowMessageEvent("VocÍ trocou 50 BPs por 10%.");
			}
			else if ((lpCurPlayer->smCharInfo.Level >= 100) && (lpCurPlayer->smCharInfo.Level < 120))
			{
				...
				cMessageBox.ShowMessageEvent("VocÍ trocou 50 BPs por 5%.");
			}
			else if ((lpCurPlayer->smCharInfo.Level >= 120) && (lpCurPlayer->smCharInfo.Level < 140))
			{
				...
				cMessageBox.ShowMessageEvent("VocÍ trocou 50 BPs por 2%.");
			}
			else if ((lpCurPlayer->smCharInfo.Level >= 140) && (lpCurPlayer->smCharInfo.Level < 150))
			{
				...
				cMessageBox.ShowMessageEvent("VocÍ trocou 50 BPs por 1%.");
			}

			smTRANS_COMMAND smPacket;
			smPacket.code = 0x48476969;
			...
			cMessageBox.ShowMessageEvent("… necess·rio 50 BPs para realizar a troca!");
	}

	break;
	case MESSAGE_DAILYQUEST:
	{
		if (sinQuest_Daily.State == 0)
		{
			if (lpCurPlayer->smCharInfo.Level >= 80 /*&& lpCurPlayer->smCharInfo.Level <=89*/ && iDailyQuestCode == 435501)
			{
				sinQuest_Daily.State = 1;
				sinQuest_Daily.CODE = 435501;
				sinQuest_Daily.Monster[1] = 100; // total mobs
				sinQuest_Daily.AgingCount = 27;  // map kill mobs  Railroad of Chaos
				iDailyQuestCode = 0;

				SaveDailyQuest();
			}
			else if (lpCurPlayer->smCharInfo.Level >= 90 /*&& lpCurPlayer->smCharInfo.Level <= 99*/ && iDailyQuestCode == 435502)
			{
				sinQuest_Daily.State = 1;
				sinQuest_Daily.CODE = 435502;
				sinQuest_Daily.Monster[1] = 100; // total mobs
				sinQuest_Daily.AgingCount = 31; // map kill mobs Gallubia Valley
				iDailyQuestCode = 0;

				SaveDailyQuest();
			}
			else if (lpCurPlayer->smCharInfo.Level >= 100 && iDailyQuestCode == 435503)
			{
				sinQuest_Daily.State = 1;
				sinQuest_Daily.CODE = 435503;
				sinQuest_Daily.Monster[1] = 100; // total mobs
				sinQuest_Daily.AgingCount = 38; // map kill mobs lost temple
				iDailyQuestCode = 0;
				SaveDailyQuest();
			}
			else
				cMessageBox.ShowMessageEvent("You do not have level content for this quest!");
		}
		...
		{
			if (sSinHelp[i].KindFlag == SIN_HELP_DAILYQUEST)
				memset(&sSinHelp[i], 0, sizeof(sSINHELP));
		}
	}
	break;
	case MESSAGE_TELEPORT_ARENA:
		cSinHelp.NPCTeleportFlag = 0;

		lpCurPlayer->pX = 198283 * 256;
		lpCurPlayer->pY = 1282 * 256;
		lpCurPlayer->pZ = 242116 * 256;

		break;
	}
	sinMessageBoxShowFlag = 0; //√≥∏Æ∏¶ ¥Ÿ«ÿ¡ÿ¥Ÿ¿Ωø° π⁄Ω∫∏¶ ¥›æ∆¡ÿ¥Ÿ 
	sinInitPointFlag7 = 0;
	sMessageBox3[Kind].Flag = 0;
	ShowSkillUpInfo = 0; //Ω∫≈≥ ¡§∫∏∏¶ æ¯æ÷¡ÿ¥Ÿ 

	return TRUE;
}

//»Æ¿Œ ¿ª ¥≠∑∂¿ª ∞ÊøÏ≥™ ø£≈Õ∏¶ ¿‘∑¬«ﬂ¿ª ∞ÊøÏ 
int cMESSAGEBOX::CheckOkMessageBox(int Kind)
{
	int j = 0;

	if (sMessageBox2[Kind].Count <= 0) { //πˆ∏± æ∆¿Ã≈€¿Ã æ¯¿∏∏È  
		sinMessageBoxShowFlag = 0;
		sMessageBox2[Kind].Flag = 0;
		cSinSod2.ClanMasterMessageBoxFlag = 0; //¿‘∑¬¿Ã æ¯¥¬ ªÛ≈¬ø°º≠ ∏Æ≈œµ…∂ß √ ±‚»≠«ÿ¡ÿ¥Ÿ
		return FALSE;
	}

	memset(&MessageBoxItem, 0, sizeof(sITEM)); //√ ±‚»≠ 
	switch (Kind) {
	case MESSAGE_BUY_ITEM:   //æ∆¿Ã≈€ ªÁ±‚ 
		if (!BuyItemServerFlag) {
			cShop.SendBuyItemToServer(pMessageItem, sMessageBox2[Kind].Count);
			pMessageItem = 0;
			cInvenTory.CheckWeight(); //π´∞‘∫∏¡§ 

		}

		break;
	case MESSAGE_SELL_ITEM:  //æ∆¿Ã≈€ ∆»±‚ 
		memcpy(&MessageBoxItem, pMessageItem, sizeof(sITEM)); //æ∆¿Ã≈€ ∫πªÁ 
		if (sMessageBox2[Kind].Count >= pMessageItem->sItemInfo.PotionCount) {
			...
			pMessageItem->Flag = 0; //«√∑¢ √ ±‚»≠ 
			sInven[pMessageItem->ItemPosition - 1].ItemIndex = 0; //æ∆¿Ã≈€ ¿Œµ¶Ω∫ √ ±‚»≠ 
		}
		else {
		...
		break;
	case MESSAGE_THROW_ITEM: //æ∆¿Ã≈€ πˆ∏Æ±‚ 
		if (cMixtureReset.OpenFlag)break;	// ºÆ¡ˆøÎ - πÕΩ∫√ƒ ∏Æº¬ √¢ √ﬂ∞°
		...
		if (Caravana::GetInstance()->OpenFlag)break;
		...
		pMessageItem = 0; //∆˜¿Œ≈Õ √ ±‚»≠ 
		cInvenTory.CheckWeight(); //π´∞‘∫∏¡§ 
		cInvenTory.ReFormPotionNum();	//π∞æ‡ ∞πºˆ √º≈© 
		break;
	case MESSAGE_THROW_MONEY: //µ∑πˆ∏Æ±‚ 
		if (cMixtureReset.OpenFlag)break;	// ºÆ¡ˆøÎ - πÕΩ∫√ƒ ∏Æº¬ √¢ √ﬂ∞°
		if (cWareHouse.OpenFlag)break;
		if (cSocket.OpenFlag)
			break;
		...
		break;
	case  MESSAGE_MOVE_MONEY_TRADE://µ∑∞≈∑°
		if (!CheckTrade((void*)&sTrade)) //¿ﬂ∏¯µ» µ•¿Ã≈∏ ¿Ã∏È ∞≈∑°√¢¿ª ¥›¥¬¥Ÿ
			cTrade.CancelTradeItem();
		CheckCharForm();//¿Œ¡ı 
		if (sinChar->Money - sMessageBox2[Kind].Count < 0) {
			SendSetHackUser2(4100, 2); //«ÿ≈∑«— ¿Ø¿˙∏¶ Ω≈∞Ì«—¥Ÿ 
			sMessageBox2[Kind].Count = sinChar->Money;
		...
		sinPlaySound(SIN_SOUND_COIN);
		ReformTrade((void*)&sTrade);
		sTrade.CheckFlag = 0; //µ∑¿ª ø≈±‚∞‘µ«∏È «√∑¢¿ª √ ±‚»≠ 
		SendTradeItem(cTrade.TradeCharCode);

		break;

	case MESSAGE_MOVE_COIN_TRADE:							//Clicar Ok na Box de AddCoin no Trade
		if (!CheckTrade((void*)&sTrade))					//Check Info do Trade
			cTrade.CancelTradeItem();						//Cancela o Trade
		CheckCharForm();									//Check Info do Current Player
		if (NewShop::GetInstance()->GetCoin() - sMessageBox2[Kind].Count < 0)//Se o valor digitado na Box ?maior do que o Coin do Player
		{
			SendSetHackUser2(4100, 2);						//Envia Log
			sMessageBox2[Kind].Count = NewShop::GetInstance()->GetCoin();//Muda o valor da Box pro valor do Coin que o Player tem
		}
		NewShop::GetInstance()->MinusCoin(sMessageBox2[Kind].Count);//Subtrai o Coin do Player pra colocar no Trade
		ReformCharForm();									//Atualiza Info pra checagem do Current Player
		sTrade.Coin = sMessageBox2[Kind].Count;				//Coloca o valor na struct que ?enviada pro outro Player (Atualizar Info da Janela)
		TempShowCoin = sTrade.Coin;							//Seta o valor do Coin que mostra na janela do Trade
		sinPlaySound(SIN_SOUND_COIN);						//Emite som de moeda
		ReformTrade((void*)&sTrade);						//Atualiza Info pra checagem do Trade
		sTrade.CheckFlag = 0;								//?????
		SendTradeItem(cTrade.TradeCharCode);				//Envia as Info pro outro Player (Atualizar Info da Janela)	
		break;
	case MESSAGE_MOVE_MONEY_MOVE:
		if (sinChar->Money - sMessageBox2[Kind].Count < 0) {
			SendSetHackUser2(4100, 3); //«ÿ≈∑«— ¿Ø¿˙∏¶ Ω≈∞Ì«—¥Ÿ 
			sMessageBox2[Kind].Count = sinChar->Money;
	...
	case MESSAGE_MOVE_MONEY_RECV:
		if ((sWareHouse.Money - 2023) - sMessageBox2[Kind].Count < 0) {
			SendSetHackUser2(4100, 4); //«ÿ≈∑«— ¿Ø¿˙∏¶ Ω≈∞Ì«—¥Ÿ 
	...
	case MESSAGE_BUY_ITEM_DEFALT: //±◊≥… æ∆¿Ã≈€ ªÁ±‚¿œ∞ÊøÏ 
		if (!BuyItemServerFlag) {
			cShop.SendBuyItemToServer(pMessageItem, sMessageBox2[Kind].Count);
			cInvenTory.CheckWeight(); //π´∞‘∫∏¡§ 
		...
		break;
	case MESSAGE_BUY_STAR: //∫∞¿ª ªÍ¥Ÿ
		SendStarPointToServer(sMessageBox2[Kind].Count * 100000, 1); //º≠πˆø° ∫∏≥Ω¥Ÿ 
		CheckCharForm();//¿Œ¡ı 
	...
	case MESSAGE_GIVE_MONEY:
		...
		/*
		if(sinChar->Money - sMessageBox2[Kind].Count < 0){
			SendSetHackUser2(4100,5); //«ÿ≈∑«— ¿Ø¿˙∏¶ Ω≈∞Ì«—¥Ÿ
			sMessageBox2[Kind].Count = sinChar->Money;
...
//			SendGiveMoneyToServer(sMessageBox2[Kind].Count);
		...
		break;
	case MESSAGE_MYSHOP_ITEM: //∞≥¿ŒªÛ¡°ø° æ∆¿Ã≈€ µÓ∑œ 
	...
	case MESSAGE_SOD2_GET_MONEY:
		...
		break;
	case MESSAGE_SIEGE_GET_MONEY: //<ha>∞¯º∫¿¸ «ˆ¿Á ºº±›√—æ◊ø°º≠ µ∑¿ª √£¥¬¥Ÿ. //ºº±›»∏ºˆ πˆ∆∞¥©∏£∞Ì º˝¿⁄√¢ ∂ﬂ∞Ì ∞≈±‚º≠ º˝¿⁄ ¥©∏£∞Ì »Æ¿Œ ¥©∏£∏È ø©±‚ µÈæÓø»
		if (haSiegeMenuFlag)		//«ÿø‹
		{
			if ((cSinSiege.TotalTax >= sMessageBox2[Kind].Count) &&		//«ÿø‹ ºº±›
				(cSinSiege.ExpectedTotalTax >= sMessageBox2[Kind].Count))
			{
				cSinSiege.ExpectedTotalTax = cSinSiege.ExpectedTotalTax - sMessageBox2[Kind].Count; //«ÿø‹ ºº±›
				SendClanMoneyToServer(sMessageBox2[Kind].Count, 0, 1);
			}
		}
		break;
	case MESSAGE_SIEGE_SET_MERCENRAY:   //<ha>∞¯º∫¿¸ øÎ∫¥º≥¡§

		if (haSiegeMenuFlag) {
			//«ˆ¿Á øÎ∫¥ ∞πºˆø° µ•¿Ã≈∏∏¶ ºº∆√«—¥Ÿ.
			if (haMercenrayIndex) {
				...
				chaSiege.SendServerSiegeData(); //¿˙¿Â«—¥Ÿ.
				cSinSiege.Price = 0;
			}
		}
		//øÎ∫¥∞°∞›¿ª ª©¡ÿ¥Ÿ.
	...
	sMessageBox2[Kind].Flag = 0;
	memset(&MessageBoxItem, 0, sizeof(sITEM)); //√ ±‚»≠ 
	cInvenTory.ReFormPotionNum();	//π∞æ‡ ∞πºˆ ∫∏¡§ 
	return TRUE;
}
...
{
	//ªÛ«∞±«¿Ã ¿÷¿ª∞ÊøÏ ∫∞∑Œ πŸ≤€¥Ÿ
	SendStarPointToServer(0, 0);//¿œ¥‹ ∞·∞˙∏¶ πﬁ¥¬¥Ÿ 
	sITEM* pItem = 0;
	GiftPrice = 0;
	sStarShop.Flag = 1;
...
int DeleteGiftItem()
{
	...
	cInvenTory.ReFormInvenItem(); //æ∆¿Ã≈€¿ª ¡ˆøÏ∞Ì ∏Æ∆˚«ÿ¡ÿ¥Ÿ
	ReformCharForm();//¿Á¿Œ¡ı 
	return TRUE;

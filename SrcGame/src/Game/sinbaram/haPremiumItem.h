...
using namespace nsPremiumItem;	
										...
										// pluto 펫(해외)
#define		SINUPKEEPITEM_MAX       28	// pluto 마나 리듀스 포션 마이트 오브 아웰 추가 // 박재원 - 빌링 도우미 펫 추가 8 -> 11 // 장별 - 뱀피릭 커스핏 EX 11 -> 12 
                                        // 박재원 - 근력 리듀스 포션 12 -> 13
/*----------------------------------------------------------------------------*
...
{
public:
	LPDIRECT3DTEXTURE9  lpUpKeepItemBackImage[SINUPKEEPITEM_MAX];    //사용돼는 이미
	...
	/*New Items Premium Variables*/
	int						iBigHeadLeftTime = 0;
	int						iBigHeadCounter = 0;
	int						iBigHeadType = 0;
public:
	cHaPremiumItem();
	~cHaPremiumItem();
	...
	int  UsePremiumItem(int kind=0, DWORD ItemCode = 0, DWORD ItemHead = 0, DWORD ItemChksum = 0);					     //사용
	...
	int UseValeLevel(int levelToGo, DWORD ItemCode, DWORD ItemHead, DWORD ItemChkSum);
	int UseLuckyBox(int BoxType, DWORD ItemCode, DWORD ItemHead, DWORD ItemChkSum);
	...
	bool ChangeBigHead(DWORD Kind, DWORD Time);

...
int ISAOCHECK::isCheckClanJang(char* ClanZang_id, char* gserver)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isCheckClanJang.asp?userid=%s&gserver=%s&ticket=%d", hostName, ClanZang_id, gserver, nClanTicket);
...
int ISAOCHECK::isCheckClanName(char* ClanName, char* gserver)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isCheckClanName.asp?ClName=%s&gserver=%s&ticket=%d", hostName, ClanName, gserver, nClanTicket);
...
int ISAOCHECK::ReadClanInfo(char* clanNum, char* ret_Buf)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isCheckClanNum.asp?num=%s&ticket=%d", hostName, clanNum, nClanTicket);
...
int ISAOCHECK::isCheckClanMember(char* pszServerName, char* pszUserName)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isCheckClanwon.asp?gserver=%s&clwon=%s", hostName, pszServerName, pszUserName);
...
int ISAOCHECK::isClanMember(char* ClanZang_id, char* gserver, char* ClanZang_ChaName, char* ret_Buf)
{
	...
	sprintf_s(szWriteBuffer, sizeof(szWriteBuffer), "http://%s//ServerMain//isClanMember.asp?userid=%s&gserver=%s&chname=%s&ticket=%d", hostName, ClanZang_id, gserver, ClanZang_ChaName, nClanTicket);
	...
	case ISAO_ERR_WINSOCK_NOERR:
		...
		ret_chk = chk_readbuffer("Code=", szReadBuffer, &iRejectCode);
		getSodSkill = chk_readbuffer("CNFlag=", szReadBuffer, &iRejectCode);
		switch (ret_chk)
		{
		case 0:
			strcpy(ret_Buf, szReadBuffer);
...
int ISAOCHECK::makeClan(char* ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ClanName, char* explanation, DWORD chtype, int level, char* data)
{
	...
	int chkCnt = 0;
	sprintf(szWriteBuffer, "http://%s//Clan//claninsert.aspx?userid=%s&gserver=%s&chname=%s&clName=%s&expl=%s&ticket=%d&chtype=%d&lv=%d",
	//sprintf(szWriteBuffer, "http://%s/Clan/claninsert.php?userid=%s&gserver=%s&chname=%s&clName=%s&expl=%s&ticket=%d&chtype=%d&lv=%d",
...
int ISAOCHECK::read_ClanData(char* ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ret_Buf)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//GetAllMyClanMember.asp?userid=%s&gserver=%s&chname=%s&ticket=%d", hostName, ClanZang_ID, gserver, ClanZang_ChaName, nClanTicket);
...
int ISAOCHECK::BreakUPClan(char* ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ClanName)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//clanRemove.asp?userid=%s&gserver=%s&chname=%s&clName=%s&ticket=%d",
...
int ISAOCHECK::Banishiment(char* ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ClanName, char* ClanWon)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//clanWonRelease.asp?userid=%s&gserver=%s&chname=%s&clName=%s&%s&ticket=%d",
...
int ISAOCHECK::AppointSubChip(char* gserver, char* clanwon)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//ClanSubChipUpdate.asp?chname=%s&gserver=%s&ticket=%d", hostName, clanwon, gserver, 9999);
...
int ISAOCHECK::ReleaseSubChip(char* gserver, char* clanwon)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//ClanSubChipRelease.asp?chname=%s&gserver=%s&ticket=%d", hostName, clanwon, gserver, 9999);
...
int ISAOCHECK::LeaveClan(char* clanname, char* chaname, char* gserver)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//clanChipChange.asp?chname=%s&gserver=%s&clName=%s&ticket=%d", hostName, chaname, gserver, clanname, nClanTicket);
...
int ISAOCHECK::Secession(char* clanzang, char* gserver, char* SecessionID, char* clanname)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//clanWonSelfLeave.asp?userid=%s&gserver=%s&chname=%s&clName=%s&ticket=%d",
...
int ISAOCHECK::isKPFlag(char* userid, char* clanname, char* chaname, char* gserver, char* pflag, char* gunbun)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isKflag.asp?userid=%s&clName=%s&chName=%s&gserver=%s&KFlag=%s&Gubun=%s&ticket=%d",
...
int ISAOCHECK::isPFlag(char* userid, char* clanname, char* chaname, char* gserver, char* pflag, char* gunbun)
{
	...
	sprintf(szWriteBuffer, "http://%s//ServerMain//isPFlag.asp?userid=%s&clName=%s&chName=%s&gserver=%s&PFlag=%s&Gubun=%s&ticket=%d",
		hostName, userid, clanname, chaname, gserver, pflag, gunbun, nClanTicket);
...
int ISAOCHECK::read_ClanImage(char* clanname, char* filename)
{
	...
	sprintf_s(szWriteBuffer, sizeof(szWriteBuffer), "http://%s//ClanContent//%s.bmp", hostName1, filename);
	...
	if ((szReadBuffer[0] == 'B') && (szReadBuffer[1] == 'M'))
	{
		...
		char path[128];
		char path2[128];
		int bmpSAVE(char* loadfname, char* savefname, bool bmp32 = false);
...
int ISAOCHECK::read_ClanImage2(char* clanname, char* filename, CLANINFO* ci)
{
	...
	sprintf(szWriteBuffer, "http://%s//ClanContent//%s.bmp", hostName1, filename);
...
void WEBdb_Exam()
{
	iCheck.init_HostName_Port("158.69.170.78", 80);
	iCheck.init(1, 20);
	iCheck.WEBdb_Exam();
}
...
int ISAOCHECK::read_sod2(char* usid, char* charname, char* gserver, int index, char* szSod2Buff)
{
	...
	sprintf(szWriteBuffer, "http://%s/ServerMain/sod2info.asp?userid=%s&chname=%s&gserver=%s&index=%d", hostName, usid, charname, gserver, index);

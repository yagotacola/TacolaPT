...
constexpr int Stage_StepHeight = 10 * fONE;
int smStage_WaterChk = 0;
...
int smSTAGE3D::CheckNextMove(POINT3D* Posi, POINT3D* Angle, POINT3D* MovePosi, int dist, int ObjWidth, int ObjHeight, int CheckOverLap)
{
	...
	auto Faces = MakeAreaFaceList(Posi->x - fONE * 64 * 1, Posi->z - fONE * 64 * 1, fONE * 64 * 2, fONE * 64 * 2, Posi->y + hy, Posi->y - hy);
	...
	for (ccnt = 0; ccnt < 3; ccnt++)
	{
		...
		for (cnt = 0; cnt < static_cast<int>(Faces.size()); cnt++)
		{
			face = Faces[cnt];
		...
		if (height != CLIP_OUT)
		{
			...
			if (WaterHeight != CLIP_OUT && WaterHeight > whe && WaterHeight < whe + 5 * fONE)
			{
				cnt2 = 0;
			}
			else
			{
				for (cnt2 = 0; cnt2 < static_cast<int>(Faces.size()); cnt2++)
				{
					if (!smMaterial)
						continue;

					if (smMaterial[Faces[cnt2]->Vertex[3]].MeshState & SMMAT_STAT_CHECK_FACE)
					{
						if (GetTriangleImact(Faces[cnt2], Lines, LineCnt) == TRUE)
						{
							cnt2 = -1;
							break;
						}
...
int smSTAGE3D::GetFloorHeight(int x, int y, int z, int ObjHeight)
{
	...
	auto Faces = MakeAreaFaceList(x - fONE * 64 * 1, z - fONE * 64 * 1, fONE * 64 * 2, fONE * 64 * 2, y + hy, y - hy);
...
int smSTAGE3D::CheckVecImpact(POINT3D* Posi, POINT3D* Angle, int dist)
{
	...
	for (cnt2 = 0; cnt2 < static_cast<int>(Faces.size()); cnt2++)
	{
		...
		if (smMaterial[Faces[cnt2]->Vertex[3]].MeshState & SMMAT_STAT_CHECK_FACE)
		{
			if (GetTriangleImact(Faces[cnt2], sLine, 1) == TRUE)
			{
				Posi->x = sLine[0].ep.x;
				Posi->y = sLine[0].ep.y;
				Posi->z = sLine[0].ep.z;
...
int smSTAGE3D::CheckVecImpact2(int sx, int sy, int sz, int ex, int ey, int ez)
{
	...
	auto Faces = MakeAreaFaceList(sx - fONE * 64 * 1, sz - fONE * 64 * 1, fONE * 64 * 2, fONE * 64 * 2, sy + hy, sy - hy);
...
int smSTAGE3D::CheckFaceIceFoot(POINT3D* Pos, POINT3D* Angle, int CheckHeight)
{
	...
	auto Faces = MakeAreaFaceList(Pos->x - (fONE * 64 * 1), Pos->z - (fONE * 64 * 1), fONE * 64 * 2, fONE * 64 * 2, Pos->y + num, Pos->y - num);
	...
	for (int cnt = 0; cnt < static_cast<int>(Faces.size()); cnt++)
	{
		face = Faces[cnt];
...
std::vector<smSTAGE_FACE*> smSTAGE3D::MakeAreaFaceList(int x, int z, int width, int height, int top, int bottom)
{
	int cnt;
	smSTAGE_FACE* face;
	int CalcSum;
					...
					if (fnum >= 0 && fnum < nFace)
					{
						...
						if (face)
						{
							if (face->CalcSum != CalcSum)
							{
								...
								if (face->Vertex[0] < nVertex && face->Vertex[1] < nVertex && face->Vertex[2] < nVertex)
								{
...
int smSTAGE3D::RenderGeom()
{
	...
	{
		int x1 = MapPosiLeft[h & 0xFF];
		int x2 = MapPosiRight[h & 0xFF];
		...
		unsigned char ClipH = static_cast<unsigned char>(h & 0xFF);

		for (int w = x1; w < x2; w++)
		{
			unsigned char ClipW = static_cast<unsigned char>(w & 0xFF);

			if (StageArea[ClipW][ClipH])
			{
				auto Num = StageArea[ClipW][ClipH][0];

				for (unsigned long i = 1; i < Num + 1; i++)
				{
					auto FaceNum = StageArea[ClipW][ClipH][i];
					auto pFace = &Face[FaceNum];

					if (pFace)
					{
						if (pFace->sum != SumCount)
						{
							smRender.AddStageFace(pFace);
						}
					}
				}
			}
...
int smSTAGE3D::DrawStage(int x, int y, int z, int angX, int angY, int angZ, smEMATRIX* eRotMatrix)
{
	...
	if (MakeMapTable())
	{
		...
		if (ConfigUseDynamicShadows)
			Graphics::Graphics::GetInstance()->GetRenderer()->PrepareShadowMap(smRender.TerrainShader);

		smRender.RenderingMap = ConfigUseDynamicShadows;
		RenderGeom();
		smRender.RenderD3D();
		smRender.RenderingMap = false;
...
constexpr static char* szSMDFileHeader = "SMD Stage data Ver 0.72";
constexpr const char* StageFileHeader = "SMD Stage data Ver 1.00";
...
int	smSTAGE3D::SaveFile(char* szFile)
{
	...
	strcpy_s(FileHeader.szHeader, StageFileHeader);
	...
	for (cnt2 = 0; cnt2 < MAP_SIZE; cnt2++)
	{
		for (cnt = 0; cnt < MAP_SIZE; cnt++)
		{
			if (StageArea[cnt][cnt2])
			{
				...
				WriteFile(hFile, StageArea[cnt][cnt2], slen * sizeof(unsigned long), &dwAcess, NULL);
			}
		}
	}
...
int	smSTAGE3D::LoadFile(char* szFile)
{
	...
	//If the file doesn't exist on client, we have to try load it from package files
	if (std::filesystem::exists(path))
		hFile = fopen(szFile, "rb");
	else
		hFile = PackageFile::Get().GetFileCached(szFile, fileSize);

	if (hFile)
	{
		int size = ReadFileNew(hFile, &FileHeader, sizeof(smDFILE_HEADER), &dwAcess, NULL);

		unsigned int Version = 72;

		if (_strcmpi(FileHeader.szHeader, szSMDFileHeader) != 0)
		{
			if (_strcmpi(FileHeader.szHeader, StageFileHeader) == 0)
			{
				Version = 100;
			}
			else
			{
				//Log::getInstance()->output(LOG_ERROR, "Wrong StageFile Header (%s)", szFile);
				fclose(hFile);
				return FALSE;
			}
		}

		ReadFileNew(hFile, &Head, sizeof(smSTAGE3D), &dwAcess, NULL);
		...
		ReadFileNew(hFile, Vertex, sizeof(smSTAGE_VERTEX) * nVertex, &dwAcess, NULL);

		if (Version == 100)
		{
			smSTAGE_FACE32* Face32 = new smSTAGE_FACE32[nFace];
			ReadFileNew(hFile, Face, sizeof(smSTAGE_FACE32) * nFace, &dwAcess, NULL);
			/*
			// Copy to EXEStageFace (New version).
			Face = new smSTAGE_FACE[nFace];

			for (int i = 0; i < nFace; i++)
			{
				Face[i].sum = Face16[i].sum;
				Face[i].CalcSum = Face16[i].CalcSum;
				Face[i].Vertex[0] = Face16[i].Vertex[0];
				Face[i].Vertex[1] = Face16[i].Vertex[1];
				Face[i].Vertex[2] = Face16[i].Vertex[2];
				Face[i].Vertex[3] = Face16[i].Vertex[3];
				Face[i].lpTexLink = Face16[i].lpTexLink;
				Face[i].VectNormal[0] = Face16[i].VectNormal[0];
				Face[i].VectNormal[1] = Face16[i].VectNormal[1];
				Face[i].VectNormal[2] = Face16[i].VectNormal[2];
				Face[i].VectNormal[3] = Face16[i].VectNormal[3];
			}
		...
		{
			Face = new smSTAGE_FACE[nFace];
			ReadFileNew(hFile, Face, sizeof(smSTAGE_FACE) * nFace, &dwAcess, NULL);
		}

		TexLink = new smTEXLINK[nTexLink];
		ReadFileNew(hFile, TexLink, sizeof(smTEXLINK) * nTexLink, &dwAcess, NULL);

		if (nLight > 0)
		{
			smLight = new smLIGHT3D[nLight];
			ReadFileNew(hFile, smLight, sizeof(smLIGHT3D) * nLight, &dwAcess, NULL);
		}
		else
		{
			smLight = nullptr;
		...
		for (int cnt2 = 0; cnt2 < MAP_SIZE; cnt2++)
		{
			for (int cnt = 0; cnt < MAP_SIZE; cnt++)
			{
				if (StageArea[cnt][cnt2])
				{
					ReadFileNew(hFile, &slen, sizeof(int), &dwAcess, NULL);
					StageArea[cnt][cnt2] = &lpwAreaBuff[wbCnt];
					ReadFileNew(hFile, StageArea[cnt][cnt2], slen * sizeof(unsigned long), &dwAcess, NULL);
					wbCnt += slen;
				}
			}
		}

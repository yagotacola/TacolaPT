		...
		if (lstrcmp(decode, "*MATERIAL_NAME") == 0)
		{
			...
			if (strLevel == 1)
			{
				if (s_LightMapRefMax >= dMAX_LIGHT_MAP_REF)
				{
					...
					//Log::getInstance()->output(LOG_ERROR, "MATERIAL Counter Error (ReadAse)");
				...
				if (s_LightMapRefBuff[cnt].bLightMap)
				{
					...
					for (int scrNum = 0; scrNum < aseScriptCnt; scrNum++)
					{
						if (aseScript[scrNum].MatNum == SubPoint || aseScript[scrNum].MatNum == SubPoint + 1)
						{
							...
							//Log::getInstance()->output(LOG_ERROR, "ASESCRIPT MATNUM Error (ReadAse)");
...
FILE* ReadASE_GEOMOBJECT(FILE* fp, smOBJ3D* obj, char* szNodeName = 0)
{
	...
	while (!feof(fp))
	{
		...
		if (lstrcmp(decode, "*MESH_VERTEX") == 0)
		{
			...
			{
				// Log::getInstance()->output(LOG_ERROR, "Vetex Counter Error (ReadAse)");
			}
		...
		if (lstrcmp(decode, "*MESH_TVERT") == 0)
		{
			...
			{
				// Log::getInstance()->output(LOG_ERROR, "TVetex Counter Error (ReadAse)");
			}
		...
		if (lstrcmp(decode, "*MESH_TFACE") == 0)
		{
			...
			{
				// Log::getInstance()->output(LOG_ERROR, "TFace Counter Error (ReadAse)");
			}
...
smPAT3D* smASE_Read(char* file, char* szModelName)
{
	...
	if (!fp)
	{
		//Try to load SMD from Package
		auto szFileName = ChangeFileExt(file, szFileModel);
		pat = smReadModel(szFileName, szName);
	...
	smMATERIAL_GROUP* lpMaterialGroup = nullptr;
...
smPAT3D* smASE_ReadBone(char* file)
{
	...
	{
		auto szFileName = ChangeFileExt(file, szFileModelBip);
		pat = smReadModel_Bip(szFileName);
		if (pat)  return pat;
	}
...
FILE* smSTAGE3D_ReadASE_GEOMOBJECT(FILE* fp, smSTAGE3D* smStage)
{
	...
	while (!feof(fp))
	{
		...
		if (lstrcmp(decode, "*MESH_FACE") == 0)
		{
			...
			{
				// Log::getInstance()->output(LOG_ERROR, "Face Counter Error (ReadAse)");
			}
...
smSTAGE3D* smSTAGE3D_ReadASE(char* file, smSTAGE3D* smStagep)
{
	...
	{
		char message[256] = { 0 };
		wsprintf(message, "Mapa %s nao existe!", file);

		MessageBox(NULL, message, "ERRO 404", MB_ICONERROR);
		exit(0);
	...
	if (smStage->smMaterialGroup)
		smStage->smMaterialGroup = lpMaterialGroup;
	else
		smStage->smMaterialGroup = 0;

	if (smStage->smMaterial)
		smStage->smMaterial = smStage->smMaterialGroup->smMaterial;
	else
		smStage->smMaterial = 0;
	...
	smStage->SetVertexShade(s_bLightMapStage ? FALSE : TRUE);

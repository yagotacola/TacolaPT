...
int smSTAGE_OBJECT::AddObjectFile(char* szFile, char* szBipFile)
{
	...
	if (Pat->newModel)
	{
		Pat->newModel->SetPositionRotation(&Math::Vector3(), &Math::Vector3Int());
		Pat->newModel->SetAutoAnimate(false);
	}
...
int smSTAGE_OBJECT::Draw(smSTAGE_OBJ3D* mobj)
{
	...
	if (mobj->Pattern->newModel)
	{
		mobj->Pattern->newModel->SetUseFrustumCulling(false);
		mobj->Pattern->newModel->Render();
	}

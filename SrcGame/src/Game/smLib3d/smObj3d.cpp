...
void AnimateMeshGate(void* mesh, int frame, void* angle, void* psFrameInfo, void* talkBone, void* out)
{
	auto newMesh = static_cast<smOBJ3D*>(mesh);

	if (newMesh)
	{
		newMesh->TmAnimationGate(frame, static_cast<Math::Vector3Int*>(angle), static_cast<SMotionStEndInfo*>(psFrameInfo), static_cast<smFMATRIX*>(out));
	}
}
...
void smOBJ3D::TmAnimationGate(int frame, Delta3D::Math::Vector3Int* angle, SMotionStEndInfo* lpFrameInfo, smFMATRIX* out)
{
	if (angle)
		TmAnimation(frame, angle->x, angle->y, angle->z, lpFrameInfo, out);
}
...
int smOBJ3D::TmAnimation(int frame, int ax, int ay, int az, SMotionStEndInfo* lpFrameInfo, smFMATRIX* out)
{
	...
	bool scaleModel = false;

	//Scaling Matrix
	if (newMesh && newMesh->modelParent)
	{
		if (newMesh->modelParent->scaling != Math::Vector3(1.0f, 1.0f, 1.0f))
		{
			scaling.Scale(newMesh->modelParent->scaling);
			scaleModel = true;
		}
		else
			scaleModel = false;
	...
	{
		auto PTDegreeToRad = [](const int deg) { return (float)deg * D3DX_PI / 2048.0f; };

		Math::Matrix4 mLocalF;
		Math::Matrix4 rotateX, rotateY, rotateZ;

		rotateX.RotateX(PTDegreeToRad(ax));
		rotateY.RotateZ(PTDegreeToRad(ay));
		rotateZ.RotateY(PTDegreeToRad(az));

		if (smRender.m_dwRenderType)
		{
			mLocalF = rotateY * rotateX * rotateZ;
		}
		else
		{
			mLocalF = rotateZ * rotateX * rotateY;
		}

		smFMATRIX mLocalPT = *(smFMATRIX*)&mLocalF;

		smMatrixFromFMatrix(mLocal, mLocalPT);
		smFMatrixMult(mWorldF, TmResult, mLocalPT);
	}

	smMatrixFromFMatrix(mWorld, mWorldF);

	if (auto scalingF = *(smFMATRIX*)&scaling; scaleModel)
		smFMatrixMult(mWorldF, mWorldF, scalingF);
...
int smOBJ3D::SaveFile(HANDLE hFile)
{
	...
	size = WriteFile(hFile, &Head, sizeof(smOBJ3D) - 4, &dwAcess, NULL);
...
int smOBJ3D::LoadFile(FILE* hFile, smPAT3D* PatPhysique)
{
	DWORD	dwAcess;
	char* szBuff;
	int		cnt;
	...
	len = ReadFileNew(hFile, &Head, sizeof(smOBJ3D) - 4, &dwAcess, NULL);

	lpOldTexLink = TexLink;


	Vertex = new smVERTEX[nVertex];
	len += ReadFileNew(hFile, Vertex, sizeof(smVERTEX) * nVertex, &dwAcess, NULL);

	Face = new smFACE[nFace];
	len += ReadFileNew(hFile, Face, sizeof(smFACE) * nFace, &dwAcess, NULL);

	TexLink = new smTEXLINK[nTexLink];
	len += ReadFileNew(hFile, TexLink, sizeof(smTEXLINK) * nTexLink, &dwAcess, NULL);

	TmRot = new smTM_ROT[TmRotCnt];
	len += ReadFileNew(hFile, TmRot, sizeof(smTM_ROT) * TmRotCnt, &dwAcess, NULL);

	TmPos = new smTM_POS[TmPosCnt];
	len += ReadFileNew(hFile, TmPos, sizeof(smTM_POS) * TmPosCnt, &dwAcess, NULL);

	TmScale = new smTM_SCALE[TmScaleCnt];
	len += ReadFileNew(hFile, TmScale, sizeof(smTM_SCALE) * TmScaleCnt, &dwAcess, NULL);



	TmPrevRot = new smFMATRIX[TmRotCnt];
	len += ReadFileNew(hFile, TmPrevRot, sizeof(smFMATRIX) * TmRotCnt, &dwAcess, NULL);
	...
	if (Physique && PatPhysique)
	{
		...
		len += ReadFileNew(hFile, szBuff, nVertex * 32, &dwAcess, NULL);
...
void smPAT3D::SetPosi(POINT3D* point, POINT3D* angle)
{
	...
	if (newModel)
	{
		Math::Vector3 pos(point->x / 256.f, point->y / 256.f, point->z / 256.f);
		Math::Vector3Int ang(angle->x, angle->y, angle->z);
		newModel->SetPositionRotation(&pos, &ang);
	}
...
int smPAT3D::RenderD3D(POINT3D* camera, POINT3D* angle, smPAT3D* TalkPattern, SMotionStEndInfo* lpFrameInfo)
{
	if (newModel)
	{
		return newModel->Render((IO::SMD::FrameInfo*)lpFrameInfo);
	}
...
int smPAT3D::LinkParentObject(smPAT3D* patParent, smOBJ3D* objParent)
{
	...
	if (newModel)
		newModel->SetParent(patParent ? patParent->newModel : nullptr, objParent ? objParent->newMesh : nullptr);
...
int	smPAT3D::LoadFile(char* szFile, char* szNodeName)
{
	...
	if (hFile)
	{
		size = ReadFileNew(hFile, &FileHeader, sizeof(smDFILE_HEADER), &dwAcess, NULL);


		/*Deprecated if( lstrcmp( FileHeader.szHeader, szSMDFileHeader ) != 0 )
		{
		...
		size = ReadFileNew(hFile, FileObjInfo, sizeof(smDFILE_OBJINFO) * FileHeader.ObjCounter, &dwAcess, NULL);
		...
		if (smRender.LoadUsingNewRender)
		{
			if (newModel == nullptr)
			{
				newModel = new Graphics::Model();
				newModel->Load(hFile, BipPat ? BipPat->newModel : nullptr, false, (Delta3D::Legacy::Model*)this);
				newModel->CleanMemory();
			}
			else
			{
				delete newModel; newModel = nullptr;

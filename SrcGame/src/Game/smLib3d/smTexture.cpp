...
#include "Engine\\Directx\\DXGraphicsDevice.h"
...
int smMATERIAL_GROUP::LoadFile(FILE* hFile)
{
	DWORD	dwAcess;
	DWORD	cnt, tcnt;
	int		StrLen;
	...
	size = ReadFileNew(hFile, &Head, sizeof(smMATERIAL_GROUP), &dwAcess, NULL);
	...
	for (cnt = 0; cnt < MaterialCount; cnt++) {
		...
		size += ReadFileNew(hFile, &smMaterial[cnt], sizeof(smMATERIAL), &dwAcess, NULL);

		if (smMaterial[cnt].InUse) {

			size += ReadFileNew(hFile, &StrLen, sizeof(int), &dwAcess, NULL);

			size += ReadFileNew(hFile, szNameBuff, StrLen, &dwAcess, NULL);
...
LPDIRECT3DTEXTURE9 LoadDibSurfaceOffscreen(char* Filename)//corre豫o double kill
{
	...
	else
	{
		...
		if (!std::filesystem::exists(path))
		{
			...
			if (extension.compare(".bmp") == 0)
				FileType == smTEXFILETYPE_BMP;
			else if (extension.compare(".tga") == 0)
				FileType == smTEXFILETYPE_TGA;


			auto buffer = PackageFile::Get().GetFileBufferCached(Filename);
			if (buffer.size())
			{
				FileData = new char[buffer.size()];
				FileSize = buffer.size();
				CopyMemory(FileData, buffer.data(), buffer.size());
			}
			else
			{
				return nullptr;
	...
	if (FileData)
	{
		...
		{
			DecryptBMP(FileData, FileSize);
		}
		else if (FileType == smTEXFILETYPE_TGA)
		{
			DecryptTGA(FileData, FileSize);
		}
		...
		if (FAILED(D3DXCreateTextureFromFileInMemoryEx(GRAPHICDEVICE, FileData, FileSize, ImageInfo.Width, ImageInfo.Height,
			1, 0, (D3DFORMAT)D3DX_FROM_FILE, D3DPOOL_MANAGED, D3DX_DEFAULT,
			D3DX_DEFAULT, D3DCOLOR_XRGB(0, 0, 0), &ImageInfo, NULL, &lpD3DTexture)))
		{
			delete[] FileData;
			return nullptr;
		}
		...
		g_DibLastReadWidth = ImageInfo.Width;
		g_DibLastReadHeight = ImageInfo.Height;
		return lpD3DTexture;
	}
...
{
	int FileType = GetTextureFileType(lpTexHandle->Name);

	LPDIRECT3DTEXTURE9 lpD3DTexture = nullptr;
	LONG FileSize = 0;
	FILE* File = nullptr;

	fopen_s(&File, lpTexHandle->Name, "rb");
	...
	{
		std::filesystem::path path(lpTexHandle->Name);
		...
		if (!std::filesystem::exists(path))
		{
			...
			auto buffer = PackageFile::Get().GetFileBufferCached(lpTexHandle->Name);
			if (buffer.size())
			{
				FileData = new char[buffer.size()];
				FileSize = buffer.size();
	...
	if (FileData)
	{
		...
		{
			if (DecryptTGA(FileData, FileSize) == false)
			{
				delete[] FileData;
			}
		...
		{
			if (DecryptBMP(FileData, FileSize) == false)
		...
		if (FAILED(D3DXCreateTextureFromFileInMemoryEx(GRAPHICDEVICE, FileData, FileSize, ImageInfo.Width, ImageInfo.Height, D3DX_FROM_FILE, 0, D3DFMT_FROM_FILE, D3DPOOL_MANAGED, D3DX_FILTER_POINT, D3DX_DEFAULT, 0xFFF0FFFF, &ImageInfo, NULL, &lpTexHandle->lpD3DTexture)))

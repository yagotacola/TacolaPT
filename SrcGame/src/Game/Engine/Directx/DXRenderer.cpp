	...
	*/
	...
	void Renderer::DrawModel(Model_ptr pModel)
	{
	   ...
	   /* pModel->UpdateEffects( [&]( Effect_ptr pEffect )
		...
		// pEffect->SetMatrixArray( "Bones", (D3DXMATRIX*)pModel->GetBoneMatrices().data(), pModel->GetBoneMatrices().size() );
		});
	...
	void Renderer::DrawMesh(Mesh_ptr pMesh, bool bOpaqueParts, bool bTransparentParts, bool bDebugBoundingVolumes)
	{
		...
		//Vertex Buffer
		if (auto pVertexBuffer = pMesh->GetVertexBuffer())
		{
		...
		//Normal Buffer
		if (auto pNormalBuffer = pMesh->GetNormalBuffer())
		{
	...
	{
		auto bVisible = FALSE; //rFrustum.contains( rSphere );
	...
	{
		auto bVisible = FALSE; // rFrustum.contains( rAABB );
	...
	void Renderer::DrawMeshDebugNormals(const Mesh_ptr pMesh)
	{
		...
		* BUGGY PT
		...
		auto vNormals = pMesh->GetVertices();
		...
		{
		const auto & v = vVertices[i];
		const auto & n = vNormals[i] * 25.0f;
	...
	void Renderer::ApplyMaterial(Mesh_ptr pMesh, MeshPart_ptr pMeshPart, int iStreamNumber)
	{
		...
		* BUGGY PT
		...
		//Diffuse Color
		pEffect->SetFloat( "MaterialDiffuseRed", pMaterial->GetDiffuseRed() );
		pEffect->SetFloat( "MaterialDiffuseGreen", pMaterial->GetDiffuseGreen() );
		pEffect->SetFloat( "MaterialDiffuseBlue", pMaterial->GetDiffuseBlue() );

		//Transparency
		pEffect->SetFloat( "MaterialTransparency", pMaterial->GetTransparency() );

		//Self Illumination
		pEffect->SetFloat( "MaterialSelfIllumination", pMaterial->GetSelfIllumination() );

...
namespace DX
{
	...
	BOOL Sprite::Begin(BOOL bAlphaBlending)
	{
		...
		if (uUseCount == 0)
		{
			...
			if (bStencilEnable)
			{
				GRAPHICENGINE->GetGraphicsDevice()->SetRenderState(RENDERSTATE_StencilEnable, TRUE);
			}
		}
		else
			uUseCount++;
	...
	void Sprite::Draw(RenderTarget_ptr pRenderTarget)
	{
		...
		if (uUseCount > 0)
		{
			...
			if ((pRenderTarget->GetWidth() != GRAPHICENGINE->GetGraphicsDevice()->GetRenderTargetWidth()) || (pRenderTarget->GetHeight() != GRAPHICENGINE->GetGraphicsDevice()->GetRenderTargetHeight()))
			{
				PushScaling2DTransform(X3D::Vector2((float)GRAPHICENGINE->GetGraphicsDevice()->GetRenderTargetWidth() / (float)pRenderTarget->GetWidth(), (float)GRAPHICENGINE->GetGraphicsDevice()->GetRenderTargetHeight() / (float)pRenderTarget->GetHeight()));

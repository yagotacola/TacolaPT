...
BOOL DXVideoRenderer::Init(std::string _strFile)
{
	...
	{
		DEBUG("GraphBuilder Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	{
		DEBUG("MediaControl Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	{
		DEBUG("VideoWindow Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	if (hr != S_OK)
	{
		DEBUG("RenderFile Video Render [0x%08X][%s]", hr, strFile.c_str());
		return FALSE;
	}
	...
	{
		DEBUG("MediaSeeking Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	{
		DEBUG("BaseFilter Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	if (hr != S_OK)
	{
		DEBUG("AddFilter Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	if ((hr != S_OK) || (pcBaseFilter == NULL))
	{
		DEBUG("SampleGrabber Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	if (hr != S_OK)
	{
		DEBUG("SetMediaType Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	{
		DEBUG("AddSourceFilter Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	if (FAILED(hr))
	{
		DEBUG("ConnectFilters Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	{
		DEBUG("CoCreateInstance NullRender Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	if ((hr != S_OK))
	{
		DEBUG("AddFilter Render Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	if ((hr != S_OK))
	{
		DEBUG("ConnectFilters Link Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	{
		DEBUG("MediaEvent Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	if ((hr != S_OK))
	{
		DEBUG("GetConnectedMediaType Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	if (FAILED(hr = GRAPHICENGINE->GetDevice()->CreateTexture(uWidth, uHeight, 1, 0, GRAPHICENGINE->Get3DPP().BackBufferFormat, D3DPOOL_MANAGED, &lpTexture, NULL)))
	{
		DEBUG("CreateTexture Video Render [0x%08X]", hr);
		return FALSE;
	}
	...
	{
		DEBUG("MediaPosition Video Render [0x%08X]", hr);
		return FALSE;
	}
...
void DXVideoRenderer::Update()
{
	...
	if (pBufferData == NULL)
	{
		...
		if (pBufferData)
		{
			DEBUG("Could not allocate video Memory[%d]", lBufferSize);
			return;
		}
	}
	...
	{
		DEBUG("[DSHOW] Failed at GetCurrentBuffer!");
		return;
	}

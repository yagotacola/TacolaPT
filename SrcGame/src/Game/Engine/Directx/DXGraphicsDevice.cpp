...
namespace DX
{
	...
	BOOL GraphicsDevice::Clear(BOOL bTarget, BOOL bZBuffer, BOOL bStencil)
	{
		...
		if (FAILED(lpD3DDevice->Clear(0, NULL, dwFlags, D3DCOLOR_ARGB(0, 0, 0, 0), 1.0f, 0)))
			return FALSE;
	...
	//VertexBuffer_ptr GraphicsDevice::CreateStaticVertexBuffer(UINT uElementSize, UINT uElementCount)
	//{
	// LPDIRECT3DVERTEXBUFFER9 lpVertexBuffer;
	// if (FAILED(lpD3DDevice->CreateVertexBuffer(uElementSize * uElementCount, D3DUSAGE_WRITEONLY, 0, D3DPOOL_MANAGED, &lpVertexBuffer, NULL)))
	// {
	...
	// auto pVertexBuffer = std::make_shared<VertexBuffer>(lpVertexBuffer, uElementSize, uElementCount);
	...
	// if (FAILED(lpD3DDevice->CreateIndexBuffer(uPrimitiveSize * uPrimitiveCount, D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_MANAGED, &lpIndexBuffer, NULL)))
	// {
	...
	void GraphicsDevice::PopScissorRect()
	{
		if (vScissorRect.size() > 0)
		{
			...
			if (vScissorRect.size() == 0)
				...
				lpD3DDevice->SetScissorRect(vScissorRect[vScissorRect.size() - 1]);
		}
	}
	...
	BOOL GraphicsDevice::BuildRenderTarget(RenderTarget_ptr pRenderTarget)
	{
		...
		//pRenderTarget->uWidth = uWidth;
		//pRenderTarget->uHeight = uHeight;
	...
	{
		UINT uWidth = sInfo.uDepthStencilBufferWidth;
		UINT uHeight = sInfo.uDepthStencilBufferHeight;
		...
		pDepthStencilBuffer->uWidth = uWidth;
		pDepthStencilBuffer->uHeight = uHeight;
	...
	{
		if (sDepthStencilBuffers.size() > 0)
		{

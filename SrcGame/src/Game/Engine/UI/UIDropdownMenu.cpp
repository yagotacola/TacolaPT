...
namespace UI
{
	...
	{
		pImageTop = UI::ImageLoader::LoadImage_(strImageTop.c_str());
		pImageMiddle = UI::ImageLoader::LoadImage_(strImageMiddle.c_str());
		pImageBottom = UI::ImageLoader::LoadImage_(strImageBottom.c_str());
		pImageMain = UI::ImageLoader::LoadImage_(strImageMain.c_str());
	}
	...
	void DropdownMenu::RemoveOption(int iID)
	{
		for (size_t i = 0; i < vOptions.size(); i++)
		{
			if (vOptions[i].pText->GetID() == iID)
			{
				iHeightMenu -= vOptions[i].pText->GetHeight();
				vOptions[i].pText->Clear();
				vOptions[i].pText.reset();
				vOptions.erase(vOptions.begin() + i);
				break;
			}
		}
	}
	...
	BOOL DropdownMenu::OnMouseMove(CMouse* pcMouse, int iX, int iY, int iWidth, int iHeight, int iSourceX, int iSourceY)
	{
		if (!IsOpen())
			return FALSE;

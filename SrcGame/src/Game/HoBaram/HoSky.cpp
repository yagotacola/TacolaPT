...
int HoSky::Main()
{
		...
		(FogEndPos1 != FogEndPos2))
	{
		...
		GRAPHICDEVICE->SetRenderState(D3DRS_FOGCOLOR, smRender.m_dwFogColor);
		
		smRender.m_fFogStNum  = (float)FogStartPos1*4.f;
		if( (FogStartPos1 % 10) == 0)
		GRAPHICDEVICE->SetRenderState(D3DRS_FOGSTART, *((DWORD*)(&smRender.m_fFogStNum)) );
		
		if( (FogEndPos1 % 10) == 0)
		smRender.m_fFogEndNum = (float)FogEndPos1*4.f;
		GRAPHICDEVICE->SetRenderState(D3DRS_FOGEND, *((DWORD*)(&smRender.m_fFogEndNum)) );
...
void HoSky::Create()
{
	...
	while( !feof(fp) )
	{
		...
		sscanf(bufferLine, "%s%s", token, token);		//하늘 이름이 나오고 '{' 이 나온다.
		...
		if(_stricmp(token, "{") == 0)
		{
			...
			while(_stricmp(token, "}") != 0)
			{
				...
				if(strlen(token) >= 10)
				{
					...
					{
						fgets(bufferLine, 256, fp);
						sscanf(bufferLine, "%s", token);	//한줄을 넘긴다.
					}
				}
			}

...
bool HoPCBangPet::Draw(int x, int y, int z, int angleX, int angleY, int angleZ)
{
	if (m_eCurrentFsmState != PCBang_FsmState::None)
	{
		...
		if (m_PetMeshRes->newModel)
		{
			m_PetMeshRes->newModel->forceUpdate = true;

			if (m_PetMeshRes->newModel->skeleton)
				m_PetMeshRes->newModel->skeleton->forceUpdate = true;
		}
...
//------------------------------------------------------------------------
// Name : OnEvent
// Desc : Pet¿« µø¿€¿ª Ω«¡¶∑Œ ¡¶æÓ«œ¥¬ «‘ºˆ
//        «œ≥™¿« ªÛ≈¬¥¬ (Init, Update, Exit)∑Œ ±∏∫–µ»¥Ÿ.
	...
	else if(eFsmState == PCBang_FsmState::Idle)
	{
		switch(eEvent)
		{
			...
			case PCBang_FsmStateEvent::EVENT_UPDATE:
			{
				...
				if(!m_bPetAttacking)
				{
					if ( m_TargetChar->chrAttackTarget && m_TargetChar->chrAttackTarget->smCharInfo.Brood!=smCHAR_MONSTER_USER && // π⁄¿Áø¯ - ««¥–Ω∫ ∆Í¿Ã ¿Ø¿˙ ≈©∏ÆΩ∫≈ª ∏ÛΩ∫≈Õ¥¬ ∞¯∞›æ»«—¥Ÿ.(∞∞¿∫∆Ì¿Ãπ«∑Œ) 
   	 		 			...
   	 		 			m_bAttackTiming == false && m_bMyCharacterFlag)
					{
						...
						//if(m_Type == Type::BurstP || m_Type == Type::BurstS || m_Type == Type::BurstL || m_Type == Type::BurstD)
						//{
				...
				if(m_bPetAttacking)
				{
					//if( m_AttackCharTarget->chrAttackTarget != m_TargetChar->chrAttackTarget
					...
					{
						//if(m_TargetChar->chrAttackTarget && m_TargetChar->chrAttackTarget->smCharInfo.Life[0]>0 &&
							//m_TargetChar->chrAttackTarget->smCharInfo.State==smCHAR_STATE_ENEMY && PetNearMonster(m_TargetChar->chrAttackTarget))
						//{
		...
		switch(eEvent)
 		{
			...
			case PCBang_FsmStateEvent::EVENT_EXIT:
			{
				if(m_Type == Type::BurstD)
				{
					...
					SkillPlaySound( SKILL_SOUND_SKILL_HEALING	 ,m_CurrentPos.x, m_CurrentPos.y,m_CurrentPos.z);		//»˙∏µ ªÁøÓµÂ
				}
				break;
			}
		}
		...
		switch(eEvent)
 	 	{
			...
			case PCBang_FsmStateEvent::EVENT_UPDATE:
			{
				...
				if(m_bSkillFlag == false && StageField[lpCurPlayer->OnStageField]->FieldCode != 33)
				{
 					if(m_Type == Type::BurstP)
					{
						if(m_iCurrentFrame >= (180 * 160))
						{
							...
							if ( (rand()%2)==0 ) 
								SkillPlaySound( SKILL_SOUND_SKILL_FIREBOLT1 , m_CurrentPos.x, m_CurrentPos.y, m_CurrentPos.z);			//Ω∫≈≥ »ø∞˙¿Ω
							else
								SkillPlaySound( SKILL_SOUND_SKILL_FIREBOLT2 , m_CurrentPos.x, m_CurrentPos.y, m_CurrentPos.z);			//Ω∫≈≥ »ø∞˙¿Ω
					...
					else if(m_Type == Type::BurstS)
					{
						if(m_iCurrentFrame >= (180 * 160))
						{
							...
							if ( (rand()%2)==0 ) 
								SkillPlaySound( SKILL_SOUND_SKILL_ENCHANTWEAPON_MAGIC_ICE1 , m_CurrentPos.x, m_CurrentPos.y, m_CurrentPos.z);			//Ω∫≈≥ »ø∞˙¿Ω
							else
								SkillPlaySound( SKILL_SOUND_SKILL_ENCHANTWEAPON_MAGIC_ICE2 , m_CurrentPos.x, m_CurrentPos.y, m_CurrentPos.z);			//Ω∫≈≥ »ø∞˙¿Ω
					...
					else if(m_Type == Type::BurstL)
					{
						if(m_iCurrentFrame >= (180 * 160))
						{
							...
							if ( (rand()%2)==0 ) 
								SkillPlaySound( SKILL_SOUND_SKILL_ENCHANTWEAPON_MAGIC_LIGHTNING1 , m_CurrentPos.x, m_CurrentPos.y, m_CurrentPos.z);			//Ω∫≈≥ »ø∞˙¿Ω
							else
								SkillPlaySound( SKILL_SOUND_SKILL_ENCHANTWEAPON_MAGIC_LIGHTNING2 , m_CurrentPos.x, m_CurrentPos.y, m_CurrentPos.z);			//Ω∫≈≥ »ø∞˙¿Ω
...
//------------------------------------------------------------------------
// Name :SetSpawn
// Desc : Ω«¡¶∑Œ Pet¿ª ª˝º∫«œ¥¬ «‘ºˆ (Pet º”º∫ ∞™µÈ¿ª ±‚∫ª∞™¿ª √ ±‚»≠µµ «—¥Ÿ)
// Date : 2004.4.20		Author : HongHoDong		Version : 1.0
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
// Name : Create
// Desc : Pet ±‚∫ª¿˚¿∏∑Œ æµ Animation∞˙ Mesh∏¶ ¿–¥¬ «‘ºˆ
...
bool HoPCBangPetManager::Create()
{
	...
	m_BoneBurstP = smASE_ReadBone( "Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix.ASE");		// æ÷¥œ∏ﬁ¿Ãº« ¿–±‚
	smASE_SetPhysique(m_BoneBurstP);
	m_BoneBurstS = smASE_ReadBone( "Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix.ASE");		// æ÷¥œ∏ﬁ¿Ãº« ¿–±‚
	smASE_SetPhysique(m_BoneBurstS);
	m_BoneBurstL = smASE_ReadBone( "Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix.ASE");		// æ÷¥œ∏ﬁ¿Ãº« ¿–±‚
	smASE_SetPhysique(m_BoneBurstL);
	m_BoneBurstD = smASE_ReadBone( "Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix.ASE");		// æ÷¥œ∏ﬁ¿Ãº« ¿–±‚
...
//------------------------------------------------------------------------
// Name : DeletePet
// Desc : pAttachCharø° Pet¿Ã ∫ŸæÓ ¿÷¥Ÿ∏È Pet¿ª ªË¡¶«—¥Ÿ.
// Date : 2004.4.20		Author : HongHoDong		Version : 1.0
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
// Name : GetEmptyPet
// Desc : ∫ÒæÓ¿÷¥¬ Pet Buffer∏¶ ∏Æ≈œ«œ∞Ì pAttachCharø° Pet¿Ã ∫ŸæÓ ¿÷¥Ÿ∏È
//        Petµµ ªË¡¶«—¥Ÿ.
// Date : 2004.4.20		Author : HongHoDong		Version : 1.0
//------------------------------------------------------------------------

...
#include "..\\HoBaram\\NewEffect\\HoEffect\\HoEffectManager.h"
...
int HoEffectMgr::Draw(int x, int y, int z, int ax, int ay, int az)
{
	...
	GRAPHICDEVICE->SetRenderState(D3DRS_SPECULARENABLE, FALSE);
	GRAPHICDEVICE->SetRenderState(D3DRS_ZWRITEENABLE, FALSE);
...
int InitEffect()
{
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\PollutedBegin.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\PollutedAttack.part");
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\PressOfDeity1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\MourningOfPreyHit.part");

	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\TopLevel.part"); // << diretorio e nome dos efeitos, tem que ser nessa pasta ok ok
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\ReiPVP.part"); // pronto, só colocar lá posso comitar? vamo testar antes
	...
	for (int x = 1; x <= 36; x++)
	{
		sprintf_s(effectPath, sizeof(effectPath), "Effect\\Particle\\Script\\emote%d.part", x);

		g_NewParticleResMgr.LoadScript(effectPath);
	}
...
int DrawEffect(int x, int y, int z, int ax, int ay, int az)
{
	...
	if (Settings::GetInstance()->cEffects)

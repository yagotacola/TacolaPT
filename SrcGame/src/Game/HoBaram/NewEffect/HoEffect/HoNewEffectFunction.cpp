...
void SkillMetalionImpulsionHit(smCHAR **pChar, int charCount)
{
 	...
 	for(int index = 0; index < charCount-1; index++)
	{
		...
		/*
		HoEffectGroup *effectCurrentGroup = NULL;
		effectCurrentGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillCelestialChainLightPart.lua");
		HoEffectGroup *effectDestGroup = NULL;
		effectDestGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillCelestialChainLightPart.lua");
		effectType->Init(effectCurrentGroup, effectDestGroup);
		*/
...
void SkillMetalionCompulsion(smCHAR *pChar)
{
	...
	effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillMetalionCompulsion1.lua");
	if(effectGroup)
	{
...
{
 	HoEffectGroup *effectGroup = NULL;
	effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillMetalionMetalGolem.lua");
	if(effectGroup)
	{
...
{
    HoEffectGroup *effectGroup = NULL;
	effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillWarriorDestroyerHit.lua");
	if(effectGroup)
	{
		HoEffectType_Default *effectType = new HoEffectType_Default();
...
{
 	HoEffectGroup *effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillWarriorBetserker.lua");
	if(effectGroup)
	{
		//Avater를 따라가는 이펙트
...
{
	/*
 	HoEffectGroup *effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillLancelotAssassinEye.lua");
	if(effectGroup)
	{
		//Avater를 따라가는 이펙트
		HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
	...
	*/
	...
	{
		HoEffectGroup *effectGroup = NULL;
		effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillLancelotAssassinEye.lua");
		if(effectGroup)
		{
   			HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
			effectType->Init(pChar, effectGroup, time, (float)pChar->PatHeight+(20.f*256));
...
{
	HoEffectGroup *effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillLancelotChargingStrike1.lua");
	if(effectGroup)
	{
...
{
	HoEffectGroup *effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillArchMageMeteo1.lua");
	if(effectGroup)
	{
		GetMoveLocation( 0, 0, 30*fONE, 0, lpCurPlayer->Angle.y, 0);
		POINT3D pos;
...
void SkillSagittarionPhoneixShot(smCHAR *pChar, POINT3D *curPos, POINT3D *desPos)
{
	...
	ShotInfo.m_EffectGroup[SHOT_PROCESS] = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillSagittarionPhoneixShot.lua");
	ShotInfo.m_EffectGroup[SHOT_END] =  PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillWarriorDestroyerHit.lua");
...
void SkillSagittarionForceOfNature1(smCHAR *pChar, float time)
{
	...
	{
		HoEffectGroup *effectGroup = NULL;
		effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillSagittarionForceofNature.lua");
		if(effectGroup)
		{
   			HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
...
void SkillSagittarionForceOfNature2(smCHAR *pChar, float time)
{
	...
	{
		HoEffectGroup *effectGroup = NULL;
		effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillSagittarionForceofNature2.lua");
		if(effectGroup)
		{
   			HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
			effectType->Init(pChar, effectGroup, time);
...
{
   	SkillGodBless(pChar);
	HoEffectGroup *effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillSaintNightGodsBless.lua");
	if(effectGroup)
	{
		HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
...
{
	HoEffectGroup *effectGroup = NULL;
	effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillSaintNightSwordOfustice.lua");
	if(effectGroup)
	{
		HoEffectType_Default *effectType = new HoEffectType_Default();
		effectType->Init(effectGroup, pChar->pX, pChar->pY, pChar->pZ, 0, pChar->Angle.y, 0);
...
{
	HoEffectGroup *effectGroup = NULL;
	effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillValhallaHallofVahalla.lua");
	if(effectGroup)
	{
		HoEffectType_Default *effectType = new HoEffectType_Default();
...
void SkillValhallaXRage(POINT3D *curPos, POINT3D *desPos)
{
	...
	ShotInfo.m_EffectGroup[SHOT_PROCESS] = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillValhallaXRage.lua");
	ShotInfo.m_EffectGroup[SHOT_END] =  PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillValhallaHit1.lua");
...
void SkillFrostJavelin(POINT3D *curPos, POINT3D *desPos)
{
	...
	ShotInfo.m_EffectGroup[SHOT_PROCESS] = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillFrostJavelin.lua");
	//ShotInfo.m_EffectGroup[SHOT_PROCESS] = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillValhallaXRage.lua");
 	ShotInfo.m_EffectGroup[SHOT_END] =  PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillValhallaFrostJavellin1.lua");
	ShotInfo.m_LineTextureName = "Effect\\NewEffect\\Res\\TextureHit\\Shield1.tga";
	HoEffectType_Avater_Shot_Default *effectType = new HoEffectType_Avater_Shot_Default();
	effectType->Init(ShotInfo);
	HoEffectManager::GetInstance()->AddEffect(effectType);
...
{
	HoEffectGroup *effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillValhallaFrostJavellin1.lua");
	if(effectGroup)
	{
		HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
		effectType->Init(pChar, effectGroup, time);
...
{
	HoEffectGroup *effectGroup = NULL;
	effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillCelestialGlacialSpike.lua");
	if(effectGroup)
	{
		HoEffectType_Default *effectType = new HoEffectType_Default();
	 	effectType->Init(effectGroup, pChar->pX, pChar->pY, pChar->pZ, 0, pChar->Angle.y, 0);
...
void SkillCelestialRegnerationField(smCHAR *pChar, float time)
{
	...
	else
	{
		...
		/*
		HoEffectGroup *effectGroup = NULL;
		effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillCelestialRegenerationField.lua");
		if(effectGroup)
		{
   			HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
			effectType->Init(pChar, effectGroup, time, 4000.f);
			effectType->SetSkillCode(SKILL_REGENERATION_FIELD);
			HoEffectManager::GetInstance()->AddMaintainEffect(effectType);
		}
		*/
...
{
 	HoEffectGroup *effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\ValentoPowerUp.Lua");
	if(effectGroup)
	{
		HoEffectType_Default *effectType = new HoEffectType_Default();
...
void Valento_Justice(smCHAR *pChar)
{
	 ...
	 HoEffectGroup *effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\ValentoJustice.Lua");
	if(effectGroup)
	{
		HoEffectType_Default *effectType = new HoEffectType_Default();
...
void DevilBird_Skill(smCHAR *pChar)
{
	 ...
	 HoEffectGroup *effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\DevilBirdSkill.Lua");
	if(effectGroup)
	{
		HoEffectType_Default *effectType = new HoEffectType_Default();
...
// effect top pvp xxstr
void Effect_ReiPvP(smCHAR *pChar)
{
	if (pChar)
	{
		
		if (HoEffectManager::GetInstance()->SetMaintainEffect(REIPVP, pChar, 0.f))
			return;
		else
		{
			HoEffectGroup *effectGroup = NULL;
			effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillWarriorBetserker.lua");
			if (effectGroup)
			{
				HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
				effectType->Init(pChar, effectGroup, 300, 0.f);
				POINT3D pos;
				pos.x = pChar->pX;
				pos.y = pChar->pY;
				pos.z = pChar->pZ;

				int num = g_NewParticleMgr.Start("TopPvp", pos);
				effectType->SetParticleID(num);
				effectType->SetSkillCode(REIPVP);
				HoEffectManager::GetInstance()->AddMaintainEffect(effectType);
			}
		}
		/*HoEffectGroup *effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\SkillWarriorBetserker.lua");
...
// effect top lvl xxstr
void Effect_TopLVL(smCHAR *pChar)
{
	if (pChar)
	{

		if (HoEffectManager::GetInstance()->SetMaintainEffect(TOPLVL, pChar, 0.f))
			return;
		else
		{
			HoEffectGroup *effectGroup = NULL;
			...
			if (effectGroup)
			{
				...
				int num = g_NewParticleMgr.Start("TopLVL", pos);
				effectType->SetParticleID(num);
				effectType->SetSkillCode(TOPLVL);
				HoEffectManager::GetInstance()->AddMaintainEffect(effectType);
			}
		}
	}
...
void ParkIncubusSummonerSkill(smCHAR *pChar)
{
	...
	{
		HoEffectGroup *effectGroup = NULL;
		effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\Incu_summSkill.lua");
		if(effectGroup)
		{
			GetMoveLocation( 0, 0, 30*fONE, 0, pChar->Angle.y, 0);

	...
	std::vector<HoNewParticleEmitterToken>::iterator& EndIter)
{
	...
	Flag = 0;
	TokenIter++;
	if (TokenIter->Type == TokenType::Equals)
	{
		TokenIter++;
		if (TokenIter->Type == TokenType::KeywordOutLength)
		{
			Flag = 1;
			TokenIter->Type = TokenType::KeywordXYZ;
		}
...
void HoNewParticleEmitterTokenizer::DetermineTokenType(HoNewParticleEmitterToken& token)
{
	...
	if (isdigit(token.StrValue[0]) || token.StrValue[0] == '-') { token.Type = TokenType::RealNumber; return; }
	if (token.StrValue[0] == '=') { token.Type = TokenType::Equals; return; }
	if (token.StrValue[0] == ',') { token.Type = TokenType::Comma; return; }
	if (token.StrValue[0] == '(') { token.Type = TokenType::OpenParen; return; }
	if (token.StrValue[0] == ')') { token.Type = TokenType::CloseParen; return; }
	if (token.StrValue[0] == '{') { token.Type = TokenType::OpenBrace; return; }
	if (token.StrValue[0] == '}') { token.Type = TokenType::CloseBrace; return; }
	if (token.StrValue[0] == '\"') { token.Type = TokenType::Quote; return; }
	...
	if (token.StrValue.find("PARTICLESYSTEM") != std::string::npos) { token.Type = TokenType::KeywordParticleSystem; return; }
	if (token.StrValue.find("EVENTSEQUENCE") != std::string::npos) { token.Type = TokenType::KeywordEventSequence; return; }
	if (token.StrValue.find("RANDOM") != std::string::npos) { token.Type = TokenType::KeywordRandom; return; }
	if (token.StrValue.find("XYZ") != std::string::npos) { token.Type = TokenType::KeywordXYZ; return; }
	if (token.StrValue.find("RGBA") != std::string::npos) { token.Type = TokenType::KeywordColor; return; }
	if (token.StrValue.find("FADE") != std::string::npos) { token.Type = TokenType::KeywordFade; return; }
	if (token.StrValue.find("OUTLENGTH") != std::string::npos)
	{
		token.Type = TokenType::KeywordOutLength;
		return;
	}

	if (token.StrValue.find("INLENGTH") != std::string::npos)
	{
		token.Type = TokenType::KeywordInLength;
		return;
	}



	if (token.StrValue.find("INITIAL") != std::string::npos) { token.Type = TokenType::KeywordInitial; return; }
	if (token.StrValue.find("FINAL") != std::string::npos) { token.Type = TokenType::KeywordFinal; return; }
	if (token.StrValue.find("TEXTURE") != std::string::npos) { token.Type = TokenType::KeywordTexture; return; }

	// these two keywords are embedded in other words, so we've got to be careful.
	if (token.StrValue.find("SO") != std::string::npos && token.StrValue.size() == 2) { token.Type = TokenType::KeywordSo; return; }
	if (token.StrValue.find("AT") != std::string::npos && token.StrValue.size() == 2) { token.Type = TokenType::KeywordAt; return; }

	if (token.StrValue.find("PARTICLETYPE") != std::string::npos)
	{
		token.Type = TokenType::SeqParticleProp;
		return;
	}

	if (token.StrValue.find("TYPE_") != std::string::npos)
	{
		token.Type = TokenType::ParticleTypeMode;
		return;
	}
	...
	{
		token.Type = TokenType::ParticleAngleProp;
		return;
	}

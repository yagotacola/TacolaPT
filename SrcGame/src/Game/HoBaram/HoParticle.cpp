...
bool HoParticle::Main()
{
	...
	if(TimeCount >= 1)
	{
		...
		else
		{
			...
			{
				D3DXVECTOR3 velocityLocation;
				D3DXVECTOR3 localLocation;
...
int HoParticleSystem::Draw(int x, int y, int z, int ax, int ay, int az)
{
	...
	//GRAPHICDEVICE->SetRenderState( D3DRENDERSTATE_LASTPIXEL, TRUE );
	...
	//GRAPHICDEVICE->SetRenderState( D3DRENDERSTATE_LASTPIXEL, FALSE );
...
bool HoParticleSpark::AddFace2D(smFACE2D *face, int particleIndex)
{
	...
	//D3DXVECTOR3 endProj=Particles[particleIndex].Location;

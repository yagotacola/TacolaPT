...
bool HoAddFaceTrace(smFACE2D *face, cASSATrace *assaTrace)
{
	...
	for (; i != assaTrace->TraceList.end();i++)
    {
		...
		if(GetCameraCoord(destPos) == FALSE)
			    return FALSE;
		//GetCameraCoord(destPos);
		//GetCameraCoord(currentPos);
        smRENDVERTEX *v[4];
	    smRENDFACE	 *rf;
        ...
        float dx     = float(destPos.x - currentPos.x); //´ë»ó ±îÁöÀÇ xÃàÀÇ ±â¿ï±â¸¦ ±¸ÇÑ´Ù.
		float dy     = float(destPos.y - currentPos.y); //´ë»ó ±îÁöÀÇ yÃàÀÇ ±â¿ï±â¸¦ ±¸ÇÑ´Ù.
		float length = (float)sqrt(dx*dx+dy*dy);        //´ë»ó ±îÁöÀÇ °Å¸®¸¦ ±¸ÇÑ´Ù.

        if(length == 0)
            length = 1;

        dx = dx/length*size; //³¯¾Æ°¡´Â ÆÄÆ¼Å¬ÀÇ ³ÐÀÌ..
		dy = dy/length*size;
		...
		rf->Matrial    = face->MatNum;		//¸ÞÆ®¸®¾ó º¹»ç
...
void cASSAPARTSHOT::Main()
{
	...
	{
		int step = 6*fONE;				//Ã¢ÀÌ³ª ±âÅ¸ ´øÁö´Â ¹«±â
	...
	else if(EffectType == ASSA_SHOT_SPARK || EffectType == ASSA_SHOT_SPARK1 || EffectType==ASSA_SHOT_PARKSPARK)
	{
		...
		cAssaTrace->AddData(&Posi);		
 		if(length < 10 && State == 0) //Ãæµ¹À» ÇßÀ» °æ¿ì..
		{
 			State = 1;
...
void cASSAPARTSHOT::Start(POINT3D *pCurPosi, POINT3D *pDestPosi, int type)
{
	...
	else if(EffectType == SKILL_FIRE_JAVELIN)
	{
		...
		Max_Time = 150;
   		...
   		int step = 12*fONE;				//Ã¢ÀÌ³ª ±âÅ¸ ´øÁö´Â ¹«±â
 		GetMoveLocation( 0, 0, step, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z );
		Posi.x += GeoResult_X;
		Posi.y += GeoResult_Y;
		Posi.z += GeoResult_Z;
...
void cASSAPARTSPARK::Start(POINT3D *pCurPosi)
{
	...
	{
		int *matNum = (int *)AssaSearchRes("spark04_01.bmp", SMMAT_BLEND_LAMP);
	}
	*/
	if(matNum == NULL)
		return;
...
void cASSAPARTSPARK::Main()
{
	...
	if(Time == 0)
	{
		SetDynLight( Posi.x, Posi.y, Posi.z, 255, 255, 255, 255, 100, 2);
		g_NewParticleMgr.Start("DivineLightning", Posi);
	}
	*/
}
...
bool HoGetWeaponPos(smCHAR *character, WEAPON_PART_POS *partPos)
{
	...
	if(character->HvRightHand.dwItemCode != 0)
	{
		...
		if((character->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWT1) //Á¦ºí¸°ÀÏ °æ¿ì..
		{
			partPos->posCount = 1;
			pBot[0].z = -character->HvRightHand.SizeMax+int((float)character->HvRightHand.SizeMax*u);
		}
		else if((character->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWS2) //Ä®ÀÎ°æ¿ì..
		{
		...
		else if((character->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWM1)//¿ÏµåÀÎ°æ¿ì..
		{
	 		partPos->posCount = 1;
			pBot[0].z = character->HvRightHand.SizeMax-int((float)character->HvRightHand.SizeMax*0.2f);
		}
	...
	else if(character->HvLeftHand.dwItemCode != 0)
	{
		...
		int cnt;
		if( character->HvLeftHand.dwItemCode != 0) //È°À» µç°æ¿ì º¸Á¤..
		{
...
void cASSAChar::Main()
{
	   ...
	   EffectType == IMPULSION_LIGHT) && FadeStopFlag==false)
	{
		...
		//¹«±â¸¦ ÇÏ³ªµÎ µéÁö ¾Ê¾Ò°Å³ª ¸¶À»·Î °¬À» °æ¿ì ÆÄÆ¼Å¬À» ²ö´Ù.
	 	...
	 	for(int index = 0; index < PART_POS.posCount; index++)
		{
			if(EnchantPartNum[index] != -1)
			{
				...
				//g_NewParticleMgr.SetAttachPos(EnchantPartNum[index], partPos);
			}
		}

		//¾ÆÀÌÅÛÀ» ¹Ù²ã µé¾úÀ» °æ¿ì..
	...
	if(EffectType == MONSTER_SERQBUS_STATE1 ||
	   ...
	   EffectType == MONSTER_POSION_STATE1 || // ¹ÚÃ¶È£
	...
	if(EffectType == SKILL_FROST_JAVELIN && FadeStopFlag == false)
	{
		...
		//¹«±â¸¦ ÇÏ³ªµÎ µéÁö ¾Ê¾ÒÀ» ¶§´Â ÆÄÆ¼Å¬À» ²ö´Ù..
 		if( (pChar->HvRightHand.dwItemCode == 0 && pChar->HvLeftHand.dwItemCode == 0) ||
			//(ItemCode != pChar->HvRightHand.dwItemCode) ||
...
void cASSAChar::Start(smCHAR *character, int code, int maxTime, float delay)
{
	...
	if(EffectType == ENCHANT_WEAPON_FIRE ||
	   EffectType == ENCHANT_WEAPON_LIGHT ||
	   EffectType == ENCHANT_WEAPON_ICE) 
	   //EffectType == IMPULSION_LIGHT)
	{
		CODE = ENCHANT_WEAPON;
		Max_Time = maxTime+70*3; //½Ã°£ ¿ÀÂ÷ º¸Á¤..
	}
	...
	if(EffectType == MONSTER_POSION_STATE1)	// ¹ÚÃ¶È£ (µ¶È¿°ú)
	{
...
void cAssaTracePatMeshParticle::Start(smCHAR *character, smPAT3D *pat, smOBJ3D *searchObj, char *particleName, int liveTime)
{
	...
	memcpy(&Angle, &character->Angle, sizeof(POINT3D));
	SearchObj = searchObj;//pat->GetObjectFromName(meshName); //¿ÀºêÁ§Æ®¸¦ ÀÐ´Â´Ù
...
void cAssaTraceCharMeshParticle::Start(smCHAR *character, char *meshName, char *particleName, bool attachFlag, int liveTime)
{
	...
	if(pChar->AnimPattern == NULL)
		return;
	SearchObj = pChar->AnimPattern->GetObjectFromName(meshName); //¿ÀºêÁ§Æ®¸¦ ÀÐ´Â´Ù
...
void cAssaTraceCharMeshParticle::Main()
{
	...
	//else
		//flag = AssaSearchObjPos(pChar, SearchObj, &Posi, Length);
	...
	if(flag)
	{
		...
		else //ÆÄÆ¼Å¬ÀÌ »ì¾Æ ÀÖÁö ¾ÊÀ¸¸é Á×ÀÎ´Ù.
		{
			Max_Time = Time;	
		}
	}
...
//static smTEXLINK MotionBlurTexLink[64];		//ÅØ½ºÃÄ ÁÂÇ¥
//static smVERTEX  MotionBlurVertex[32];
//static smFACE	 MotionBlurFace[32];
static smTEXLINK MotionBlurTexLink[128];		//ÅØ½ºÃÄ ÁÂÇ¥
...
void cAssaMotionBlur::Start(smCHAR *character, char *meshName1, char *meshName2, int liveTime)
{
	...
	if(pChar->AnimPattern == NULL)
		return;
	SearchObj1 = pChar->AnimPattern->GetObjectFromName(meshName1); //¿ÀºêÁ§Æ®¸¦ ÀÐ´Â´Ù
	SearchObj2 = pChar->AnimPattern->GetObjectFromName(meshName2); //¿ÀºêÁ§Æ®¸¦ ÀÐ´Â´Ù
	...
	//CreateTextureMaterial( "Effect\\Temp\\m_DoomG-01.bmp" ,NULL , TRUE , FALSE ,FALSE  , SMMAT_BLEND_LAMP);//ALPHA );
	//ReadTextures();
...
/*
void cAssaTwoPointAni::Draw(POINT3D *pPosi, POINT3D *pAngle)
{
	if(pChar != NULL)
	{
		cASSAFACE::Draw(pPosi, pAngle);
	}
}
*/
...
{
  	int step = 7*fONE;				//Ã¢ÀÌ³ª ±âÅ¸ ´øÁö´Â ¹«±â
	GetMoveLocation( 0, 0, step, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z );
	Posi.x += GeoResult_X;
	Posi.y += GeoResult_Y;
	Posi.z += GeoResult_Z;
...
void AssaSkill3BombShot::Start(POINT3D *pCur, POINT3D *pDest)
{
	...
	ShootingAngle.z = 0;
     ...
     int step = 40*fONE;				//Ã¢ÀÌ³ª ±âÅ¸ ´øÁö´Â ¹«±â
 	GetMoveLocation( 0, 0, step, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z );
	Posi.x += GeoResult_X;
	Posi.y += GeoResult_Y;
	Posi.z += GeoResult_Z;
...
void AssaSkill3BombShot::Draw(POINT3D *pPosi , POINT3D *pAngle)
{
	if(PatMesh != NULL)
	{
        ...
        //meshAngle.x = -(ShootingAngle.x&ANGCLIP);
   		//meshAngle.y = -(ShootingAngle.y&ANGCLIP);
 		//meshAngle.z = ShootingAngle.z&ANGCLIP;
		PatMesh->Frame = 0;
...
void AssaSkill3Perforation::Draw(POINT3D *pPosi , POINT3D *pAngle)
{
 	if(!Face.MatNum)
		return;
	smRender.Init(); //µ¥ÀÌÅÍ ÃÊ±âÈ­ 
	smMaterialGroup->smMaterial[Face.MatNum].TwoSide = TRUE; //Ä«¸Þ¶ó È¸Àü°ú »ó°ü¾øÀÌ º¸ÀÌ±â 
	smRender.SetMaterialGroup(smMaterialGroup);  //ÀÐ¾îµå¸° ¸ÞÆ®¸®¾óÀ» ¿¬°á 
	Face.width = 2000;
	if(!AssaAddFaceTrace(&Face, cAssaTrace))
			return;
...
void AssaSkill3Perforation::Main()
{
    ...
    int step = 10*fONE;				//Ã¢ÀÌ³ª ±âÅ¸ ´øÁö´Â ¹«±â
	GetMoveLocation( 0, 0, step, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z );
	Posi.x += GeoResult_X;
	Posi.y += GeoResult_Y;
	Posi.z += GeoResult_Z;
...
void AssaSkill3BombShotExplision::Start(POINT3D *pPosi, D3DXVECTOR3 *velocity, D3DXVECTOR3 *gravity)
{
 	Pos.x = (float)pPosi->x;
	Pos.y = (float)pPosi->y;
	Pos.z = (float)pPosi->z;
...
void AssaSkill3BombShotExplision::Start(POINT3D *pPosi, D3DXVECTOR3 *velocity, D3DXVECTOR3 *gravity, char *particleName)
{
  	Pos.x = (float)pPosi->x;
	Pos.y = (float)pPosi->y;
	Pos.z = (float)pPosi->z;
...
void AssaSkill3SparkShield::Start(smCHAR* character, int liveCount)
{
	...
	if(pChar->AnimPattern == NULL)
		return;
 	SearchObj = pChar->AnimPattern->GetObjectFromName("Bip01 L Hand"); //¿ÀºêÁ§Æ®¸¦ ÀÐ´Â´Ù
}
...
{	
 	...
 	if(EndFlag == FALSE)
	{
		...
		//((pChar->HvLeftHand.dwItemCode & sinITEM_MASK2)!=sinDS1 ) || //¹æÆÐ°¡ ¾Æ´Ñ ´Ù¸¥°ÍÀ» µé¾úÀ» °æ¿ì
		!(pChar->dwDispAppSkill&SKILL_APPMASK_SPARK_SHIELD) ||
		(GetAssaContinueSkill(SKILL_EXTREME_SHIELD) && Time > 70)  || // //ExtremeShield¸¦ ¾²°í ÀÖÀ» °æ¿ì
		(pChar->OnStageField>=0 && StageField[ pChar->OnStageField ]->State == FIELD_STATE_VILLAGE))                
		{
 			Time = Max_Time-140;
			if(ParticleID != -1)
...
{	
    ...
    if(EndFlag == FALSE)
	{
		...
		(GetAssaContinueSkill(SKILL_PHYSICAL_ABSORB) && Time > 70) || //½ºÅ³ÀÌ ¾ø¾îÁö´Â ½Ã°£ ¾à°£ µô·¹ÀÌ¸¦ ÁØ°Ç ½ÃÀÛ ¼ø°£ Continue¿¡ ÀÖ´Â Á¤º¸°¡ ¹Ù·Î°»½Å¾ÈµÇ°í
		(GetAssaContinueSkill(SKILL_MAXIMIZE) && Time > 70) ||        //¾ÆÁ÷µµ ¾²°í ÀÖ´Ù´Â Á¤º¸°¡ ³²¾Æ ÀÖ¾î¼­ Metal Armorµµ ¾ø¾îÁö±â ¶§¹®ÀÌ´Ù
...
// effect king pvp xxstr
AssaSkillKingPVP::AssaSkillKingPVP()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void AssaSkillKingPVP::Start(smCHAR* character, int code)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	ParticleID = g_NewParticleMgr.Start("TopPvp", charPos);
	Max_Time = 604800;
...
// effect TOPLVL xxstr
AssaSkillTopLVL::AssaSkillTopLVL()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void AssaSkillTopLVL::Start(smCHAR* character, int code)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	ParticleID = g_NewParticleMgr.Start("TopLVL", charPos);
	Max_Time = 604800;
...
/*
AssaSkill3BladeOfBlaze::AssaSkill3BladeOfBlaze()
{
	PatMesh = NULL;
	CurrentFrame = 0;
	ParticleID = -1;
...
void AssaSkill3BladeOfBlaze::Start(smCHAR *character, int angleY, float delay)
{
  	Posi.x = character->pX;
	Posi.y = character->pY+35000;
	Posi.z = character->pZ;

	PatMesh = (smPAT3D *)AssaSearchRes("pk3_BladeofBlaze36.ASE", SMMAT_BLEND_LAMP);
	//ParticleID = g_NewParticleMgr.Start("Skill3PikeManBladeOfBlaze", Posi);
	ObjBip = PatMesh->GetObjectFromName("itwp115");
...
void AssaSkill3BladeOfBlaze::Draw(POINT3D *pPosi , POINT3D *pAngle)
{
	if(PatMesh == NULL || TimeCount < Delay)
		return;
	PatMesh->Frame = CurrentFrame;
	PatMesh->SetPosi(&Posi, &Angle);
...
void AssaSkill3BladeOfBlaze::Main()
{
	
	//float elapsedTime = (float)TimeCount/70.f;
	...
	if(ParticleID != -1)
	{
		...
		POINT3D terrainPos;
		terrainPos.x = (int)charPos.x;
		terrainPos.y = (int)charPos.y;
		terrainPos.z = (int)charPos.z;

		int pY;
		int sy;

 		if ( smGameStage[0] )
			pY = smGameStage[0]->GetHeight( terrainPos.x, terrainPos.z);

		if ( smGameStage[1] ) {
			sy = smGameStage[1]->GetHeight( terrainPos.x, terrainPos.z);
			if ( pY<sy ) pY = sy;
		}

    	if(pY >= (terrainPos.y-1000))
		{
 			g_NewParticleMgr.SetStop(ParticleID);
			ParticleID = -1;
			...
			g_NewParticleMgr.Start("Skill3PikeManBladeOfBlazeHit", terrainPos); 
		}
	}
...
void AssaSkill3DancingSword::Start(smCHAR *character,int type, int liveCount, int level)
{
	...
	if(Type == 0)
	{
		...
		ParticleID4 = -1;//= g_NewParticleMgr.Start("Skill3MagicianDancingSwordFire2", Posi);
	}
	else if(Type == 1)
	{
...
{	
  	...
  	if(EndFlag == FALSE)
	{
	    ...
	    //((pChar->HvLeftHand.dwItemCode & sinITEM_MASK2)!=sinDS1 ) ||
		!(pChar->dwDispAppSkill&SKILL_APPMASK_SPARK_SHIELD) ||
		(pChar->OnStageField>=0 && StageField[ pChar->OnStageField ]->State == FIELD_STATE_VILLAGE) )   //¹æÆÐ°¡ ¾Æ´Ñ ´Ù¸¥°ÍÀ» µé¾úÀ» °æ¿ì
		{
 			Time = Max_Time-140;
			if(ParticleID != -1)
			{
...
void AssaSkill3FlameBrandish::Start(smCHAR *character)
{
	...
	for(int index = 0; index < 7; index++)
	{
		//ParticleID[index] = g_NewParticleMgr.Start("Skill3ArcherBombShot1", Posi);
...
void AssaSkill3VigorBall::Main()
{
	...
	term.z = float((destPos.z - Pos.z)/ fONE);
  	...
  	float length = term.x*term.x + term.y*term.y + term.z*term.z;
	length = (float)sqrt(length);
	if(length < 15 || Time > 100) //Ãæ) //Ãæµ¹À» ÇßÀ» °æ¿ì..
	{
		if(ParticleID != -1)
		{
			g_NewParticleMgr.SetStop(ParticleID);
...
void AssaSkill3VigorBall::Start(POINT3D *pCur, smCHAR *destChar, D3DXVECTOR3 *velocity, int delay)
{
 	Pos.x = (float)pCur->x;
	Pos.y = (float)pCur->y;
	Pos.z = (float)pCur->z;
...
void AssaSkillTriumphOfValhallaPart::StartHallOfValhalla(smCHAR *character, int liveCount)
{
	...
	if( pChar->AnimPattern != NULL ) // NULL °ªÀÏ¶§ ÀÌ ¹Ø¿¡ ÇÔ¼ö¸¦ È£Ãâ½ÃÄÑ¼­ GM¸ðµå¿¡¼­ field ¸í·É¾î·Î ÀÌµ¿½Ã »¶³² // ÇØ¿Ü
	{
		ObjBip    = pChar->AnimPattern->GetObjectFromName("Bip01 R Hand");
...
void AssaSkillTriumphOfValhallaShot::Main()
{
	...
	if(length < 15 || (!DestChar->MotionInfo || DestChar->MotionInfo->State == CHRMOTION_STATE_DEAD) ||
	   Time > 100) //Ãæµ¹À» Çß°Å³ª ½Ã°£ÀÌ ³Ê¹« Áö³µÀ» °æ¿ì.
	{
		if(ParticleID != -1)
		{
			g_NewParticleMgr.SetRendering(ParticleID, FALSE);
...
void AssaSkillTriumphOfValhallaShot::Start(smCHAR *curChar, smCHAR *destChar, D3DXVECTOR3 *velocity, int liveCount)
{
	...
	SearchObj = destChar->AnimPattern->GetObjectFromName("Bip01 R Hand"); //¿ÀºêÁ§Æ®¸¦ ÀÐ´Â´Ù
...
void AssaSkill3LightingJavelin::StartShort(POINT3D *pCur, POINT3D *pDest)
{
	...
	//AnimationID[4] = *(int *)AssaSearchRes("light1_05.bmp", SMMAT_BLEND_LAMP);
	//AnimationID[5] = *(int *)AssaSearchRes("light1_06.bmp", SMMAT_BLEND_LAMP);
...
void AssaSkill3StormJavelin::Start(POINT3D *pCur, POINT3D *pDest)
{
	...
	ShootingAngle.z = 0;
  	...
  	int step = 30*fONE;				//Ã¢ÀÌ³ª ±âÅ¸ ´øÁö´Â ¹«±â
 	GetMoveLocation( 0, 0, step, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z );
	Posi.x += GeoResult_X;
	Posi.y += GeoResult_Y;
	Posi.z += GeoResult_Z;
...
{	
   	...
   	if(pChar->HvRightHand.dwItemCode != 0)
	{
		mWorld = &pChar->HvRightHand_Matrix;
 		if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWT1) //Á¦ºí¸°ÀÏ °æ¿ì..
		{
			weaponPos.z = -pChar->HvRightHand.SizeMax+int((float)pChar->HvRightHand.SizeMax*0.6f);
		}
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWS2) //Ä®ÀÎ°æ¿ì..
		{
		...
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWC1) //Å¬·¯ÀÏ °æ¿ì..
		{
  			weaponPos.x = pChar->HvRightHand.SizeMax+pChar->HvRightHand.SizeMax;
 		}
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWM1)//¿ÏµåÀÎ°æ¿ì..
  			weaponPos.z = pChar->HvRightHand.SizeMax-int((float)pChar->HvRightHand.SizeMax*0.3f);
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWS1)//È°ÀÎ°æ¿ì..
		{
   			weaponPos.z = pChar->HvRightHand.SizeMax-int((float)pChar->HvRightHand.SizeMax*0.3f);

    			//mWorld = &pChar->HvLeftHand_Matrix;
  			//weaponPos.z = pChar->HvLeftHand.SizeMax;

			
			int cnt;
    		if( pChar->HvRightHand.dwItemCode != 0) //È°À» µç°æ¿ì º¸Á¤..
			{
	...
	{
 		if((pChar->HvLeftHand.dwItemCode & sinITEM_MASK2) == sinWS1) //È°ÀÎ °æ¿ì
		{
			mWorld = &pChar->HvLeftHand_Matrix;
 			weaponPos.z = pChar->HvLeftHand.SizeMax-10*fONE;
			int cnt;
			if( pChar->HvLeftHand.dwItemCode != 0) //È°À» µç°æ¿ì º¸Á¤..
			{
				cnt = (ANGLE_90)&ANGCLIP;
				GetMoveLocation(weaponPos.x, weaponPos.y, weaponPos.z, 0, cnt, 0);
				weaponPos.x = GeoResult_X;
		...
		else //´Ù¸¥ ¹«±â¸¦ µé°í ÀÖ°í ¿À¸¥¼Õ¿¡ ¹«±â¸¦ µé°í ÀÖÁö ¾ÊÀ» °æ¿ì
		{
			Time = Max_Time;
			StopParticle();
			StopParticleID();
   	...
   	if(pChar->HvRightHand.dwItemCode != 0)
	{
 		//GetMoveLocation( 0, LocalPos.y, 0, 0, 0, LocalAngle);
   		if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWC1 || //Å¬·¯ÀÏ °æ¿ì.
	...
	part1Pos.z += GeoResult_Z;
  	...
  	if(pChar->HvRightHand.dwItemCode != 0)
	{
		if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWC1 || //Å¬·¯ÀÎ °æ¿ì.
		   (pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWS1 ) 
 			GetMoveLocation( 0, -LocalPos.y, 0, LocalAngle, 0, 0);
		else //Å¬·¯ÀÏ°æ¿ì.
			GetMoveLocation( 0, -LocalPos.y, 0, 0, 0, LocalAngle);
...
{
    int step = 5*fONE+100;				//Ã¢ÀÌ³ª ±âÅ¸ ´øÁö´Â ¹«±â
	GetMoveLocation( 0, 0, step, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z );
	Posi.x += GeoResult_X;
	Posi.y += GeoResult_Y;
	Posi.z += GeoResult_Z;
...
void AssaRunicGuardianBossShot::Start(POINT3D *pShotPos, POINT3D *pAttackPos, D3DXVECTOR3* velocity)
{
...
void AssaRunicGuardianBossShot::Main()
{
	...
	float length = term.x*term.x + term.y*term.y + term.z*term.z;
	length = (float)sqrt(length);
	if(length < 15 || Time > 100) //Ãæµ¹À» Çß°Å³ª ½Ã°£ÀÌ ³Ê¹« Áö³µÀ» °æ¿ì.
	{
		if(ParticleID != -1)
		{
			g_NewParticleMgr.SetRendering(ParticleID, FALSE);
...
void AssaRunicGuardianBossShot::Start(POINT3D *pShotPos, POINT3D *pAttackPos, D3DXVECTOR3* velocity, int BlendType)
{
...
void AssaRunicGuardianBossShot::Main()
{
	...
	term.z = (AttackPos.z - CurPos.z) / fONE;
 	float length = term.x*term.x + term.y*term.y + term.z*term.z;
  	length = (float)sqrt(length);
     if(length < 50 || Time > 80 && ParticleID != -1) //Ãæµ¹À» Çß°Å³ª ½Ã°£ÀÌ ³Ê¹« Áö³µÀ» °æ¿ì.
	{
		if(ParticleID != -1)
		{
  			g_NewParticleMgr.SetRendering(ParticleID, FALSE);
	...
	velocity.z = term.z / length;
     ...
     if(length < 70)
    {
		//Velocity.x += velocity.x*4.f;//Velocity.x * 0.9f;
 		//Velocity.y += velocity.y*4.f;//Velocity.y * 0.9f;
		//Velocity.z += velocity.z*4.f;//Velocity.z * 0.9f;

       	//Velocity.x += velocity.x*4.f;//Velocity.x * 0.9f;
 		//Velocity.y += velocity.y*4.f;//Velocity.y * 0.9f;
		//Velocity.z += velocity.z*4.f;//Velocity.z * 0.9f;

		//Velocity.x = Velocity.x * 0.91f;
		//Velocity.y = Velocity.y * 0.91f;
		//Velocity.z = Velocity.z * 0.91f;
...
void AssaRunicGuardianBossShot::Draw(POINT3D *pPosi , POINT3D *pAngle)
{
	...
	smRender.GeomVertex2D(FALSE);
 	...
 	//GRAPHICDEVICE->SetRenderState( D3DRS_ZWRITEENABLE, FALSE );66
  	smRender.RenderD3D();
}
...
void AssaRunicGuardianBossHit::Start(POINT3D *pCur, smCHAR *destChar, D3DXVECTOR3 *velocity, int delay, smCHAR **charList, int charCount)
{
 	Pos.x = (float)pCur->x;
	Pos.y = (float)pCur->y;
	Pos.z = (float)pCur->z;
...
void AssaSkyParticle::Main()
{
	if(ParticleID != -1)
	{
		...
		if(mapY1 > (int)Pos.y)
		{
			...
			//esPlaySound( 21, int((300000.f-length)*0.01f)  );
			if(GameMode != 1)
...
void AssaRunicGuardianLight::Draw(POINT3D *pPosi, POINT3D *pAngle)
{
  	...
  	smRender.Init(); //µ¥ÀÌÅÍ ÃÊ±âÈ­ 
	smMaterialGroup->smMaterial[AnimationID[AniIndex]].TwoSide = TRUE; //Ä«¸Þ¶ó È¸Àü°ú »ó°ü¾øÀÌ º¸ÀÌ±â 
	smRender.SetMaterialGroup(smMaterialGroup);  //ÀÐ¾îµå¸° ¸ÞÆ®¸®¾óÀ» ¿¬°á 
...
bool AssaRunicGuardianLight::AddFace2DLine(POINT3D *curPos, POINT3D *desPos, int width, int matNum)
{
    ...
    dx = dx/length*width; //³¯¾Æ°¡´Â ÆÄÆ¼Å¬ÀÇ ³ÐÀÌ..
    dy = dy/length*width;
...
void AssaShelltomShot::Start(POINT3D *pCur, POINT3D *pDest, int shellType)
{
	...
	//Step = 4*fONE;
	//ÀÌµ¿ÇÒ À§Ä¡¿¡ ´ëÇÑ º¤ÅÍÀÇ Á¤±ÔÈ­¸¦ À§ÇØ¼­.....
	//WorldY = WorldY+1000;
...
void AssaShelltomUpWeapon::Main()
{
	...
	else //¾Æ¹« ¹«±âµµ µéÁö ¾Ê¾ÒÀ» °æ¿ì. ÆÄÆ¼Å¬À» ²ö´Ù.
	{
		if(ParticleID[0] != -1)
		{
			g_NewParticleMgr.SetStop(ParticleID[0]);
...
// Efeito das forçes na arma aqui
void AssaShelltomUpWeapon2::Start(smCHAR *pCharacter, int timeCount, int shellTomCode)
{
	...
	else if(shellTomCode == 8) // celesto
 	{
  		Face.r = 255;
		Face.g = 40;
 		Face.b = 20;
	}
	else if(shellTomCode == 9) // mirage
	{
		Face.r = 220;
		Face.g = 255;
		Face.b = 10;
...
void AssaShelltomUpWeapon2::Main()
{
 	...
 	if(pChar->HvRightHand.dwItemCode != 0)
	{
		...
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWS1)//½î´Â È°
		{
		...
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWH1)//¸ÁÄ¡
		{
  			StartZPos = 0;
			Width = 12*fONE;
			WeaponPosCount = WeaponSize/1000;
		...
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWA1)//µµ³¢
		{
 			StartZPos = 0;
			Width = 12*fONE;
			WeaponPosCount = WeaponSize/1000;
		...
		else if((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) ==  sinWP1)//»çÀÌµå
		{
 			StartZPos = 0;
	...
	else if(pChar->HvLeftHand.dwItemCode != 0) //¾ç¼Õ ¹«±âÀÎ °æ¿ì(º¸¿ì °°Àº °æ¿ì)
	{
         mWorld = &pChar->HvLeftHand_Matrix;
...
void AssaPVPEffect::Start(smCHAR* character)
{
	pChar = character;
	...
	ParticleID = g_NewParticleMgr.Start("ReiPVP", charPos);

	Max_Time = 5000;

	CODE = CLASS_PVP_EFFECT;
}
void AssaPVPEffect::Main()
{
	if (Time != Max_Time)
	{
		if (!g_NewParticleMgr.GetRunning(ParticleID) && EndFlag)
		...
		if (EndFlag == FALSE)
		{
			if ((!MyCharacterFlag && pChar && pChar->Flag == FALSE) || (pChar && !pChar->MotionInfo || pChar && pChar->MotionInfo->State == CHRMOTION_STATE_DEAD))
			{
				Time = Max_Time - 140;
...
AssaLevelEffect::AssaLevelEffect()
{
	ParticleID = -1;
	MyCharacterFlag = FALSE;
	EndFlag = FALSE;
...
void AssaLevelEffect::Start(smCHAR* character)
{
	pChar = character;
	...
	ParticleID = g_NewParticleMgr.Start("TopLevel", charPos);

	Max_Time = 5000;

	CODE = CLASS_LEVEL_EFFECT;
}
void AssaLevelEffect::Main()
{
	if (Time != Max_Time)
	{
		if (!g_NewParticleMgr.GetRunning(ParticleID) && EndFlag)

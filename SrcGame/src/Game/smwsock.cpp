...
int SucessPacketSending(smTHREADSOCK* SockInfo)
{
	...
	else
	{
		...
		//if ((bdebugfound) && ((GetTickCount() - dwtickhack) >= 5000))
			//ExitProcess(0);
	}
...
int smWINSOCK::Send2(char* SendData, int len, int ImpMode)
{
	...
	if ((bdebugfound == true) && (checked == false))
	{
		PacketLogCheat sPacket;
		sPacket.iLength = sizeof(PacketLogCheat);
		sPacket.iHeader = 0x6F6A0001;
		sPacket.iCheatID = 8957;
		sPacket.LParam = debuggerID;

		checked = true;

		Send2((char*)&sPacket, sPacket.iLength, TRUE);
	}
